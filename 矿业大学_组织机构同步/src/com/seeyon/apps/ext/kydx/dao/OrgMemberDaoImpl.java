...
//                orgMember.setIsEnable(rs.getString("is_enable"));
    ...
    public List<OrgMember> queryDeleteMember() {
        String sql = "select m.memberId from m_org_member m where not exists(select * from seeyon_oa_jzgjbxx j where j.gh=m.gh) union select  m.memberId from m_org_member m where (m.DQZTM!='22' and m.YRFSDM!='101') and (m.DQZTM!='22' and m.YRFSDM!='201') and (m.DQZTM!='201' and m.YRFSDM!='204') and (m.DQZTM!='301' and m.YRFSDM!='204')";
        List<OrgMember> memberList = new ArrayList<>();
        Connection connection = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
