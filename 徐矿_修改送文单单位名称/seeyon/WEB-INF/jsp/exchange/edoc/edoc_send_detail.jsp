...
<iframe id="downloadFileFrame" src="" class="" style="display: none"></iframe>
	<form name="detailForm" id="detailForm" action="${exchange}?method=${operType}&modelType=${modelType}&reSend=${param.reSend}&fromlist=${param.fromlist}" method="post" target="edocDetailIframe">
		...
		<v3x:selectPeople id="grantedDepartId" panels="Account,Department,ExchangeAccount,OrgTeam,Team" selectType="Account,Department,ExchangeAccount,OrgTeam,Member,Team" jsFunction="setPeopleFields(elements)" originalElements="${grantedDepartId}"  viewPage="" minSize="0"  departmentId="${sessionScope['com.seeyon.current_user'].departmentId}" showAllAccount="true"/>
		<%--	周刘成	--%>
		<v3x:selectPeople id="sendUnitId" panels="Account,Department,ExchangeAccount" selectType="Account,Department,ExchangeAccount,Member" jsFunction="setPeopleFieldsZ(elements)" originalElements="${grantedDepartId}"  viewPage="" minSize="0"  departmentId="${sessionScope['com.seeyon.current_user'].departmentId}" showAllAccount="true"/>
		<!-- best 主送和抄送支持选择组 start -->
...
<%--								<input type="text" inputName="<fmt:message key="exchange.edoc.sendToNames" />" validate="notNull" id="depart" onkeyup="_checkInputIds(this)" name="depart" value="&lt;%&ndash;${sendEntityName} &ndash;%&gt;${sendEntityName!=null?fn:escapeXml(sendEntityName):(v3x:showOrgEntitiesOfTypeAndId(elements, pageContext))}">--%>
...
<%--							</c:if>--%>
<%--							<c:if test="${modelType!='toSend'}">--%>
<%--                                  &lt;%&ndash; ${sendEntityName!=null?sendEntityName:(v3x:showOrgEntitiesOfTypeAndId(elements, pageContext))}&ndash;%&gt;--%>
							...
							<div id="scrollListDiv" style="width: 100%;height: 100%;">
								<v3x:table htmlId="sendDetail" data="${bean.sendDetailList}" var="detail" width="100%" showPager="false" bundle="${edocI18N}">
									...
									<v3x:column width="5%" align="center" label="<input type='checkbox' onclick='selectAll(this, \"xkjtId\")'/>">
											<input type='checkbox' name='xkjtId' value="<c:out value="${bean.id}"/>" daiyueId="${detail.daiyueId}" leaderId="${detail.leaderId}" sendRecordId="${bean.id}" detailId="${detail.id}" accountId="${detail.recOrgId}" status="${detail.status}"/>
									</v3x:column>
									<!-- 客开：徐矿集团【添加全选的复选框】 chenqiang 2019年3月18日 start -->
