	...
	private static String getOpinionUserName(Long userId,String proxyName,OrgManager orgManager, FormOpinionConfig displayConfig,
		...
		try {
			...
			if(edocOpinion.isBound() && 
					...
					(OpinionShowNameTypeEnum.SIGN.getValue().equals(displayConfig.getShowNameType())||"true".equals(PropertiesConfiger.getInstance().getProperty("system.gz.edition")))
			){//电子签名显示方式
			    ...
			    if(vSign != null){
			    	...
			    	if(FormUtil.isPhoneLogin()){
			    		...
			    		imgStr = "/seeyon/edocController.do?method=showInscribeSignetPic&id="+vSign.getId();
			    	}
			        doUserName = "<IMG alt=\""+Strings.toHTML(doUserName)+"\" style=\"vertical-align: text-bottom;max-width:100%\" src=\"" + imgStr + "\" >";
			    }else{
			    	//贵州专版 -- 旧数据保存签名的机制和新公文不一样，因此要做判断
			    	if("true".equals(PropertiesConfiger.getInstance().getProperty("system.gz.edition")) && edocOpinion!= null){
			    	AffairManager affairManager = (AffairManager)AppContext.getBean("affairManager");
		               CtpAffair affair1 = affairManager.get(edocOpinion.getAffairId());
		               if(affair1!=null && (affair1.getApp()==19 || affair1.getApp()==20 || affair1.getApp()==21) ){
					    	return getOpinionImgUserNameForGZZB(userId, proxyName,orgManager);
		               }
			    	}
		...
		for(Iterator<String> it = map.keySet().iterator();it.hasNext();){
			...
			for(EdocOpinion opinion : opinions) {
				...
				for(Comment comment :commentList){
					if(comment.getAffairId() != null && comment.getAffairId().longValue()==opinion.getAffairId()&&
							tempAttitude.equals(comment.getExtAtt4())){
					...
					for (Attachment att : tempAtts) {
						...
						if(FormUtil.isPhoneLogin()){
							...
							if(!"".equals(attType) && attType.contains(".")){
								String miniType = attType.split("\\.")[0];
							...
							s = com.seeyon.ctp.common.filemanager.manager.Util.AttachmentToHtmlWithShowAllFileName(att,true, false);
						}
						sb.append(s);
						attSb.append(s);
					}
...
//		if(userName.indexOf("IMG")==-1 || attrStr != null){
...
//		}
	...
	public static String optionToJs(Map<String, Object> hs){
        ...
        opinionsJs.append("var edocDetailURL=\"/seeyon/edocController.do\";");
		...
		private static String getOpinionImgUserNameForGZZB(Long userId,String proxyName,OrgManager orgManager){
			...
			try {
				...
				if (member.getIsAdmin()) {
					...
					//huzy 图片显示
					String namePath = "seeyon/apps_res/edoc/images/signets/";
					//往本地保存图片
					if(ssi!=null){
						//签名图片样式
