		...
		if((updateMtId==a.meetingid) && (status==2 || status==null || ""==status || "undefined"==status)&&(timeout!=2 && timeout!=3)){//更改后的设置
			...
			$.ajax({
				...
				success: function (res) {
					var m = res.data;
					var html = "";
					html += "申请人：" + m.sqr + "；申请人电话：" + (typeof (m.sqrdh)!='undefined' && m.sqrdh!=0?m.sqrdh:'无') + "；申请部门：" + m.deptname + "；参会领导："+(typeof (m.ldname)!='undefined' && m.ldname !=null ?m.ldname:"无")+"；会议时间：" + m.time + "；用途：" + m.description+"；会场要求："+(typeof (m.hcyq)!='undefined' && m.hcyq !=0?m.hcyq:"");
