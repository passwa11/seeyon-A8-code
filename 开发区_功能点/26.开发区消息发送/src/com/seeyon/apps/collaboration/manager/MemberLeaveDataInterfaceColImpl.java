 ...
 * @author tanmf
 */
public class MemberLeaveDataInterfaceColImpl extends AbstractMemberLeaveData {
    
    private static Log LOG = CtpLogFactory.getLog(MemberLeaveDataInterfaceColImpl.class);
    ...
    public String getLabel(){
        return "member.leave.freecollaboration.title";
    }
    ...
    public Integer getCount(long memberId) {
        ...
        condition.put(ColQueryCondition.CollType.name(), "Self"); //Templete Self
    ...
    public boolean doHandle(long leaveMemberId, long agentMemberId) throws BusinessException {
        try{
            ...
            Agent agent = new Agent();
            {
                ...
                AgentUtil.setAgentOption(agent, AgentConstants.Agent_Option.COL.name(), true);
            ...
            for (Agent v3xAgent : oldList) {
                if(agent.getId().longValue() == v3xAgent.getId().longValue()
                        || !AgentUtil.hasXXX(v3xAgent, AgentConstants.Agent_Option.COL.name())){
                    continue;
                }
                
                AgentUtil.setAgentOption(v3xAgent, AgentConstants.Agent_Option.COL.name(), false);
                ...
                if(!AgentUtil.isHasAgentOption(v3xAgent)) {
                    ...
                    if(AgentUtil.isHasTemplate(v3xAgent)){
                        ...
                        List<AgentDetail> agentList = v3xAgent.getAgentDetails();
                        if(agentList != null){
                            details.addAll(agentList);
                        }
                    }
                    
                    agentIntercalateManager.update(v3xAgent, details,null);
                }
            }
            
            //消息要带上后缀：(来自离职办理)
            MessageContent c = MessageContent.get("agent.setting.msg.remind.from.memberleave", Functions.showMemberNameOnly(leaveMemberId), 0);
    ...
    public Long getAgentMemberId(long leaveMemberId){
        AgentModel agentModel = AgentUtil.getAgent4MemberLeave(leaveMemberId, AgentConstants.Agent_Option.COL.name());
        if(agentModel != null){
