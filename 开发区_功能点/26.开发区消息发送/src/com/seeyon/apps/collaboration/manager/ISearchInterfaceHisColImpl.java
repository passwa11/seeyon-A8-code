 ...
 *
 */
public class ISearchInterfaceHisColImpl extends ISearchManager{
    private transient OrgManager orgManager;
    private transient ISearchInterfaceColDao iSearchInterfaceColDaoFK;
	...
	public  ISearchInterfaceColDao getISearchInterfaceColDao() {
		if(iSearchInterfaceColDaoFK==null){
			iSearchInterfaceColDaoFK = (ISearchInterfaceColDao)AppContext.getBean("iSearchInterfaceColDaoFK");
		}
		return iSearchInterfaceColDaoFK;
	}
    ...
    public String getAppShowName() {
        return ISEARCH_MANAGER_DUMPDATA_APPKEY;
    }
    @Override
    ...
    public List<ResultModel> iSearch(ConditionModel cModel) {
        ...
        if(getISearchInterfaceColDao() == null){
        	return ret;
        }
        // 1. 解析条件
        // 2. 分页查询
        List<CtpAffair> list = getISearchInterfaceColDao().transSearch(cModel);
        // 3. 组装数据，返回
        if(list != null){
            for(CtpAffair affair : list){
                Integer resentTime = affair.getResentTime();
                String forwardMember = affair.getForwardMember();
                String title = ColUtil.mergeSubjectWithForwardMembers(affair.getSubject(), 80, forwardMember, resentTime, null);
                V3xOrgMember member = null;
                try {
                    member = getOrgManager().getMemberById(affair.getSenderId());
