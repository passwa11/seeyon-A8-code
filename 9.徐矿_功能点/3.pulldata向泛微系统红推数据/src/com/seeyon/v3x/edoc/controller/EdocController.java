...
//            }
//        }
//        //客开：徐矿集团【是否显示单选框】 chenqiang start 2019年3月29
//        if (StringUtils.isBlank(s_summaryId)) {
//            if ("register".equals(comm)) {
//                modelAndView.addObject("xkjtAttachmentHasRadio", true);
//                modelAndView.addObject("canClick", true);
//            } else {
//                modelAndView.addObject("xkjtAttachmentHasRadio", false);
//                modelAndView.addObject("canClick", true);
//            }
//
//
...
//                    && edocRegister.getState() == RegisterState.Registed.ordinal()) {
//                // 去掉待登记
...
//        } else {
//            if (edocRegister.getState() != null
//                    && edocRegister.getState() == RegisterState.DraftBox.ordinal()) {// 草稿
...
//                if (Strings.isNotBlank(errorMsg)) {
//                    // 当人员调出到另外的单位时，可以继续处理在之前单位的待登记文
//                    if (edocRegister.getRegisterUserId() != null && edocRegister.getRegisterUserId().equals(recordRegisterUserId)) {
//                        // 登记公文，判断指定的登记人是否可以登记此公文
//                        String errorMsg2 = canSendRecEdoc(user.getLoginAccount(), recordRegisterUserId, roleType, app);
//                        if (Strings.isNotBlank(errorMsg2)) {
//                            // 还是使用 errorMsg 的值返回
...
//        //客开：徐矿集团【保存主附件】 chenqiang 2019年3月27日  start
//        try {
//            String mainAttachmentId = request.getParameter("mainAttachmentId");
//            String mainAttachmentValue = request.getParameter("mainAttachmentValue");
//            if (StringUtils.isNotBlank(mainAttachmentId) && StringUtils.isNotBlank(mainAttachmentValue)) {
//                XkjtManager xkjtManager = (XkjtManager) AppContext.getBean("xkjtManager");
//                XkjtSummaryAttachment xkjtSummaryAttachment1 = xkjtManager.findMainAttachmentBySummaryId(edocSummary.getId());
//                XkjtSummaryAttachment xkjtSummaryAttachment = new XkjtSummaryAttachment();
//                if (xkjtSummaryAttachment1 == null) {
//
//                    xkjtSummaryAttachment.setIdIfNew();
//                    xkjtSummaryAttachment.setSummaryId(edocSummary.getId());
//                    xkjtSummaryAttachment.setAttachmentId(Long.valueOf(mainAttachmentId));
//                    xkjtSummaryAttachment.setValue(Integer.valueOf(mainAttachmentValue));
//                    xkjtManager.saveMainAttachment(xkjtSummaryAttachment);
//                } else {
//                    xkjtSummaryAttachment = xkjtSummaryAttachment1;
//                    xkjtSummaryAttachment.setAttachmentId(Long.valueOf(mainAttachmentId));
//                    xkjtManager.updateMainAttachment(xkjtSummaryAttachment);
//                }
//
//            }
//        } catch (Exception e) {
//            LOGGER.error("chenq----保存主附件错误", e);
//        }
//
//        //客开：徐矿集团【保存主附件】 chenqiang 2019年3月27日  end
...
//            if (!isQuickSend && Strings.isNotBlank(advanceRemind2) && !"0".equals(advanceRemind2)) {
...
//            if (Strings.isEmpty(request.getParameter("my:create_person"))) {
...
//            }
//        }
//        //使用中发现finally出现未执行的情况，这里在
//        this.edocLockManager.unlock(edocSummary.getSubject(), user.getId());
//        if (lockDocMarkSubject == null) {
//            this.edocMarkLockManager.unlock(lockDocMark);
//        }
...
//        //
//        if (Strings.isNotBlank(request.getParameter("templeteId"))) {
//            summary.setTempleteId(Long.parseLong(request.getParameter("templeteId")));
//        }
//        if ("1".equals(edocType)) {
//            if (registerId != -1) {
//                edocRegister = edocRegisterManager.findRegisterById(registerId);
...
//
//        // bset
//        mav.addObject("isShow", request.getParameter("isShow"));
//
//        return mav;
//    }
//
...
//            /**项目：徐矿集团  【屏蔽公文详情按钮】 作者：jiangchenxi 时间：2019年3月13日 start*/
//            boolean open = xkjtManager.isOpen(affairId);
//            mav.addObject("open", open);
//            /**项目：徐矿集团  【屏蔽公文详情按钮】 作者：jiangchenxi 时间：2019年3月13日 end*/
//            //同步消息
//            userMessageManager.updateSystemMessageStateByUserAndReference(AppContext.currentUserId(), affair.getId());
//            //客开：徐矿集团【查看公文详情时，主附件选中，且不能点击】 chenqiang 2019年3月27日  start
//            XkjtManager xkjtManager = (XkjtManager) AppContext.getBean("xkjtManager");
//            XkjtSummaryAttachment xkjtSummaryAttachment = xkjtManager.findMainAttachmentBySummaryId(summary.getId());
//            if (xkjtSummaryAttachment != null) {
//                mav.addObject("mainAttachmentId", xkjtSummaryAttachment.getAttachmentId());
//                mav.addObject("xkjtAttachmentHasRadio", true);
//                mav.addObject("canClick", false);
//            }
//
//            //客开：徐矿集团【查看公文详情时，主附件选中，且不能点击】 chenqiang 2019年3月27日  end
//
//            // best 公文在办理流程时如果是最后一个节点默认把提交换为办结 start
//            if (affair != null) {
//                try {
//                    mav.addObject("isShow", request.getParameter("isShow"));
//                    Map<String, Object> params = new HashMap<String, Object>();
//                    params.put("affairId", affair.getId());
//                    Map<String, Object> isLast = xkjtManager.judgeAffairIsLast(params);
//                    if (isLast != null) {
//                        mav.addObject("isLast", isLast.get("isLast"));
//                    }
//                } catch (Exception e) {
//                    logger.error("", e);
//                }
//            }
//            // best 公文在办理流程时如果是最后一个节点默认把提交换为办结 end
//
//            return mav;
//        } catch (Exception e) {
//            LOGGER.error("查看公文时抛出异常：", e);
...
//                    oldAttsMap.put(att.getFileUrl(), att);
//                    maxSort = maxSort > att.getSort() ? maxSort : att.getSort();
//                }
//                for (Long url : importAttIds) {
//
//                    Attachment newAtta = importAttsMap.get(url);
//                    //兼容名称一致的情况
//                    boolean flagName = false;
//                    for (Map.Entry<Long, Attachment> entity : oldAttsMap.entrySet()) {
//                        if (entity.getValue().getFilename().equals(newAtta.getFilename())) {
//                            flagName = true;
//                            if (!oldAttIds.contains(url)) {
//                                //已经修改的
//                                newAtta.setSort(entity.getValue().getSort());
//                                addAtts.add(newAtta);
//                                break;
//                            }
//                        }
...
//                }
//
//                String attaFlag = attachmentManager.create(addAtts, false);
...
//        }
//        //改为调queryByCondition-->queryByConditionDumplation同一流程只取一条的
//        //组装查询条件到EdocSearchModel中
//        EdocSearchModel em = new EdocSearchModel();
//        em.setGourpBy("true");
//        int edocType = 0;
//        if (ApplicationCategoryEnum.edocRec.key() == appEnum.getKey()) {
//            edocType = 1;
//        } else if (ApplicationCategoryEnum.edocSign.key() == appEnum.getKey()) {
//            edocType = 2;
//        }
//        em.setEdocType(edocType);
//        if (condition != null) {
//            if ("subject".equals(condition) && Strings.isNotBlank(textfield)) {
//                em.setSubject(textfield);
//            } else if ("docMark".equals(condition) && Strings.isNotBlank(textfield)) {
//                em.setDocMark(textfield);
//            } else if ("docInMark".equals(condition) && Strings.isNotBlank(textfield)) {
//                em.setSerialNo(textfield);
//            } else if ("startMemberName".equals(condition) && Strings.isNotBlank(textfield)) {
//                em.setCreatePerson(textfield);
//            } else if ("createDate".equals(condition)) {
//                if (StringUtils.isNotBlank(textfield)) {
//                    Date stamp = Datetimes.getTodayFirstTime(textfield);
//                    em.setCreateTimeB(stamp);
//                }
//                if (StringUtils.isNotBlank(textfield1)) {
//                    Date stamp = Datetimes.getTodayFirstTime(textfield1);
//                    em.setCreateTimeE(stamp);
//                }
//            }
//        }
//        long curUserId = AppContext.getCurrentUser().getId();
//        List<EdocSummaryModel> queryList = edocManager.queryByCondition(curUserId, em, true);
