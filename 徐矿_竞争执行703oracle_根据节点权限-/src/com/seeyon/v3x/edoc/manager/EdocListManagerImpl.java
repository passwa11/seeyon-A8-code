...
//            	zhou  竞争账号处理时间显示问题
				String sql="select max(COMPLETE_TIME) as dealTime from  CTP_AFFAIR where SUBJECT =? and state=4 and (NODE_POLICY ='转送' or NODE_POLICY='办理')";
				...
				try {
					...
					ps.setString(1,summary.getSubject());
					rs=ps.executeQuery();
					Timestamp time=null;
					while(rs.next()){
						time=rs.getTimestamp("dealTime");
					}
					if(null !=time){
						model.setDealTime(new Date(time.getTime()));
					}

				}catch (Exception e){
					e.printStackTrace();
					log.error("edocListManagerImpl文件中bindListDataToEdocBo方法中的jdbc查询出错了："+e.getMessage());
				}finally {
					try {
						if(null !=rs){
							rs.close();
