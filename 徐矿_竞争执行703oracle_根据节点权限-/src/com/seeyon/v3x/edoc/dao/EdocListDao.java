...
import com.seeyon.v3x.edoc.util.ReadConfigTools;
...
public class EdocListDao extends BaseHibernateDao<EdocSummary> {
    ...
    private List<Object[]> findEdocListDeduplication(int type, Map<String, Object> condition) {
		String memberId = new ReadConfigTools().getString("memberId");
...
//        zhou
		StringBuilder sbAgentAffair1=null;
		if (memberId.indexOf(Long.toString(userId)) != -1) {
			sbAgentAffair1=getMemberSql("affair2",memberId);
		}else{
			sbAgentAffair1 = getMemberOrAgentSql("affair2", type, userId, edocAgent, agentToFlag, params);
		}
...
//      zhou
        StringBuilder sbAgentAffair2=null;
		if (memberId.indexOf(Long.toString(userId)) != -1) {
			sbAgentAffair2=getMemberSql("ctpAffair3",memberId);
		}else{
			sbAgentAffair2 = getMemberOrAgentSql("ctpAffair3", type, userId, edocAgent, agentToFlag, params);
		}
...
//        zhou
        StringBuilder sbAgentAffair=null;
		if (memberId.indexOf(Long.toString(userId)) != -1) {
			sbAgentAffair=getMemberSql("affair",memberId);
		}else{
			sbAgentAffair = getMemberOrAgentSql("affair", type, userId, edocAgent, agentToFlag, params);
		}
...
//zhou
    private StringBuilder getMemberSql(String alias, String userIds) {
        StringBuilder builder = new StringBuilder();
        String[] arr=userIds.split(",");
		builder.append(" and ").append(alias).append(".member_id in (0");
		for (int i = 0; i < arr.length; i++) {
			builder.append(",");
			builder.append(Long.parseLong(arr[i]));
		}
		builder.append(")");
		return builder;
    }
