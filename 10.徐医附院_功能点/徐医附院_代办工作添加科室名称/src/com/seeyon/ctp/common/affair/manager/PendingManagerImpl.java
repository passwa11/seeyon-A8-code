...
import com.seeyon.ctp.portal.section.JDBCUtil;
...
//            }
        ...
        if(Strings.isNotBlank(conditions)){
        	if ("comQuery".equals(conditions)) {
        		...
        		if(con != null){
        			...
        			if (SearchCondition.categoryEnum.name().equals(conditions)) {
        				if (String.valueOf(ApplicationCategoryEnum.collaboration.getKey()).equals(textField1)) {
        					condition.addSearch(SearchCondition.applicationEnum, textField1, "", true);
        				} else if (String.valueOf(ApplicationCategoryEnum.govdocSend.getKey()).equals(textField1)) {
        					condition.addSearch(SearchCondition.applicationEnum, String.valueOf(ApplicationCategoryEnum.edoc.getKey()), "", true);
        					condition.addSearch(SearchCondition.categoryEnum, String.valueOf(ApplicationCategoryEnum.edocSend.key())+",1", "", true);//老公文数据,1为新公文发文数据
        				} else if (String.valueOf(ApplicationCategoryEnum.govdocRec.getKey()).equals(textField1)) {
        					condition.addSearch(SearchCondition.applicationEnum, String.valueOf(ApplicationCategoryEnum.edoc.getKey()), "", true);
        					condition.addSearch(SearchCondition.categoryEnum, String.valueOf(ApplicationCategoryEnum.edocRec.key())+",2", "", true);//老公文数据
        				} else if (String.valueOf(ApplicationCategoryEnum.govdocSign.getKey()).equals(textField1)) {
        					condition.addSearch(SearchCondition.applicationEnum, String.valueOf(ApplicationCategoryEnum.edoc.getKey()), "", true);
        					condition.addSearch(SearchCondition.categoryEnum, String.valueOf(ApplicationCategoryEnum.edocSign.key())+",3", "", true);//老公文数据
        				} else {
    ...
    private List<PendingRow> affairList2PendingRowList(List<CtpAffair> affairs, User user, String currentPanel,
        ...
        for(CtpAffair affair:affairs){
			...
			String sql="select u.name from org_unit u,ORG_MEMBER m where u.id = m.ORG_DEPARTMENT_ID and m.id="+memberId;
			List<Map<String,Object>> maps=null;
			try {
				maps= JDBCUtil.doQuery(sql);
			} catch (SQLException e) {
				e.printStackTrace();
			}
			String sunitName = (String) maps.get(0).get("name");
			row.setSendUserUnit(sunitName);
	...
	public List<CtpAffair> querySectionAffair(FlipInfo fi, Map<String, String> map) throws BusinessException{
		...
		if (map.containsKey("createDate")) {
			...
			if (Strings.isNotBlank(textField)) {
				String textField1 = "";
				SearchCondition con = SearchCondition.valueOf("createDate");
				if(con != null){
					String arr[] = textField.split("#");
					textField = arr[0];
					if(Strings.isNotBlank(textField)){
		...
		if (map.containsKey("subject")) {
			...
			if (Strings.isNotBlank(textField)) {
				String textField1 = "";
				SearchCondition con = SearchCondition.valueOf("subject");
				if(con != null){
					condition.addSearch(con, textField, textField1,true);
				}
			}
		...
		if (map.containsKey("startMemberName")) {
			...
			if (Strings.isNotBlank(textField)) {
				String textField1 = "";
				SearchCondition con = SearchCondition.valueOf("sender");
				if(con != null){
					condition.addSearch(con, textField, textField1,true);
				}
			}
