...
public class WorkFlowEventListener extends AbstractEventListener {
	...
	public void setColTraceWorkflowManager(
			TraceWorkflowDataManager colTraceWorkflowManager) {
...
//			affairManager.updateAffair(affair);
    ...
    public boolean onProcessFinished(EventDataContext context) {
        ...
        try {
             ...
             */
             boolean isTemplateHasPrePigholePath =  colManager.isTemplateHasPrePigholePath(colSummary.getTempleteId());
            if(colSummary.getArchiveId() != null && !isTemplateHasPrePigholePath){
                affairManager.updateSentPigeonholeInfo(colSummary.getId(), colSummary.getArchiveId());
                appLogManager.insertLog(user, AppLogAction.Coll_Pigeonhole_delete, colSummary.getSubject());
            }
            ...
            if(Strings.isNotBlank(colSummary.getAdvancePigeonhole()) && AppContext.hasPlugin("doc")){
    			...
    			try {
    				...
    				if(Strings.isNotBlank(archiveFolder)){
    				    ...
    				    //不一样时才移动
    				    if(!StrArchiveFolder.equals(archiveFieldValue) && !"true".equals(archiveText) && null!=realFolderId){
    				...
    				if("true".equals(archiveText)){
    					...
    					if(ctpContentAll.get(0).getContent()!=null && !"".equals(ctpContentAll.get(0).getContent())){
    						String keyword = null;
    						...
    						List<Attachment> attachment = attachmentManager.getByReference(ctpContentAll.get(0).getContentDataId(),Long.parseLong(ctpContentAll.get(0).getContent()));
    						if(!attachment.isEmpty()){
    							V3XFile file = fileManager.getV3XFile(attachment.get(0).getFileUrl());
            ...
            if(colSummary.getAttachmentArchiveId() != null &&  AppContext.hasPlugin("doc")){
            	...
            	List<Attachment> formAttachments = capFormManager.getAllFormAttsByModuleId(colSummary.getId(),colSummary.getFormAppid());//表单控件里面的附件
				...
				for (EventDataContext context : contextList) {
					...
					if (ColUtil.isNotBlank(context.getDealTerm())) {
						if(context.getDealTerm().matches("^\\d\\d\\d\\d-\\d\\d-\\d\\d \\d\\d:\\d\\d$")){
							...
							nodeDeaLineRunTime = workTimeManager
							.getCompleteDate4Nature(now, deadline,
    ...
    public boolean onWorkitemCanceled(EventDataContext context) {
    	...
    	try{
    		...
    		if(operationType == TAKE_BACK || operationType == WITHDRAW || operationType == SPECIAL_BACK_RERUN){
    			...
    			if(Strings.isNotEmpty(workItems)){
    				...
    				for (WorkItem workItem0 : workItems) {
    					if(m.keySet().contains(workItem0.getId())) {
    						...
    						if((operationType == WITHDRAW || operationType == SPECIAL_BACK_RERUN)
    				            ...
    				            WorkflowTracePO workflowTracePO= colTraceWorkflowManager.createWorkflowTracePO(af,af.getSenderId(),AppContext.getCurrentUser().getId(),trackType);
    				            dynamicNewDataWorkflowTraceList.add(workflowTracePO);
    							//colTraceWorkflowManager.createStepBackTrackData(af,af.getSenderId(),AppContext.getCurrentUser().getId(),trackType);
    				...
    				if(Strings.isNotEmpty(dynamicOldDataAffairList)){
    					colTraceWorkflowManager.deleteBatch(dynamicOldDataAffairList);
    				}
    				
    				if(Strings.isNotEmpty(dynamicNewDataWorkflowTraceList)){
    					colTraceWorkflowManager.saveBatch(dynamicNewDataWorkflowTraceList);
    				}
    		...
    		boolean isCompetition = PROCESS_MODE_COMPETITION.equals(context.getProcessMode());
    		boolean operationSo = (operationType == COMMONDISPOSAL 
        			|| operationType == ZCDB 
        			|| operationType == AUTOSKIP 
        			|| operationType == SPECIAL_BACK_SUBMITTO);
...
//                }
    ...
    public boolean onWorkitemWaitToReady(EventDataContext context) {
        try {
            ...
            List<WorkItem> workItems = context.getWorkitemLists();
            for (WorkItem wi : workItems) {
                transDeal4SepcailSubmit(wi,SubStateEnum.col_pending_unRead,summary);
            }
        } catch (Exception e) {
            log.error("",e);
        }
	...
	private void saveListMap(AffairData affairData,Date receiveTime,Boolean isCover,List<MessageReceiver> receivers,List<MessageReceiver> receivers1) {
       ...
       private void transDeal4SepcailSubmit(WorkItem wi,SubStateEnum subState,ColSummary summary){
           ...
           try {
               ...
               Long agentMemberId = colMessageManager.isAgent(affair.getTempleteId(), affair.getMemberId(), affair.getReceiveTime());
