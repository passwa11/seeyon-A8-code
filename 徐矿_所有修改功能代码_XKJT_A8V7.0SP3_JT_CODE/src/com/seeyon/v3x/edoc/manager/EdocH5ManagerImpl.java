...
public class EdocH5ManagerImpl implements EdocH5Manager {
	...
	public EdocSummaryBO getEdocSummaryBO(Long summaryId, Long affairId, String openFrom, Map<String,Object> params) throws BusinessException {
		...
		//借阅不进行权限校验，和PC保持一致
		if(!"lenPotent".equals(openFrom) && !"glwd".equals(openFrom)){
		  //权限校验
		...
		edocSummary.setAffairId(affairId);
         ...
         if (sub_state == null || sub_state.intValue() == SubStateEnum.col_pending_unRead.key()) {
             ...
             try {
               ...
               //更新第一次查看时间
                 ...
                 // 待签收数据移动端查看时affair没有accountId，增加当前人员id mxs
                 long firstViewTime = workTimeManager.getDealWithTimeValue(ctpAffair.getReceiveTime(), nowTime, ctpAffair.getOrgAccountId() != null ? ctpAffair.getOrgAccountId() : user.getLoginAccount());
