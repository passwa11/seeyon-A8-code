...
XMLHttpRequestCaller.prototype.serviceRequest = function() {
    ...
    try{
        ...
        // Chrome 禁止了在 unload的时候发送同步请求， 这里兼容一下
        if(!this.async && navigator.sendBeacon){
            var headers = {
                "type": 'application/x-www-form-urlencoded;charset=UTF-8'
              };
            var param = {};
            if (sendContent && sendContent.length > 0) {
                var strs = sendContent.split("&");
                for (var i = 0; i < strs.length; i++) {
                    var con = strs[i].split("=");
                    param[con[0]] = decodeURIComponent(con[1]);
                }
            }
            var blob = new Blob([param], headers);
            var tempStatus = navigator.sendBeacon(url, blob);
            isSendBeacon = true;
        }else{
            throw e;
        }
       ...
       for(var i=0; i<nlength; i++){
            ...
            if(s.dataName != null && s.dataName != ""){
                if(s.dataName=='公文单'){
                    ...
                    checkOption.innerHTML +="<label for='dataNameBox"+i+"'><input type=checkbox    name='dataNameBoxes' id=dataNameBox"+i+" onclick='printMain(this)'><font style='font-size:12px' color='black'>"+s.dataName+"</font></label>&nbsp;&nbsp;";
