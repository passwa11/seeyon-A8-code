...
function edocFormDisplay() {
	...
	if(navigator.userAgent.indexOf("Firefox")!=-1){
	  ...
	  bodyTypeSelector.enabled("menu_bodytype_OfficeWord");
	  document.getElementById("bodyType").value="HTML";
	}
	showEdocMark();
	return false;
}
...
function init_newedoc(){
  if (v3x.isMac) {
    alert(V5_Edoc().$.i18n("edoc.alert.support.win"));//当前功能不支持非windows系统
    clickFlag = false;
    ...
    clearWorkflowResource(getParentFrame(), currentUserId); 
    if(notOpenSave){
    	//changyi 离开当前页面
        initLeave(edocType);
    }
	...
	if($.browser.msie && $.browser.version == 8){
		$("#send").css({"height":"62px"});
	}
	var _pObj = $("#process_info");
	if(_pObj.val() != ""  && _pObj.val() != processDefaultTitle && subState != "" && subState != '1'){
		$("#isQuickS").hide();
	}
 }
         ...
         if(!recPostAccountId){
             ...
             if (subState == '16') {//指定回退给发起人[直接提交给我]查看
                 ...
                 if (processId) {
                     ...
                     top2.showWFTDiagram(getA8Top(), templeteProcessId, window, currentUserName, recPostAccountName, 'edoc', tempEdocFormId);
                 }
             } else if (isRepealFree == 'true') {//撤销回来的自由流程
                 top2.createWFPersonal(getA8Top(), getWFtype(param_edocType), currentUserId, currentUserName
                     , recPostAccountName, processId, window, policy.bianma, recPostAccountId, policy.name);
             } else if ((isFromTemplate && templateType != 'text')
                 //当后台设置开关时，设置不能自建流程时，就不能编辑流程了，只能查看流程 (最开始 按钮置灰)
                 //而且要在调用模板之后，当templeteProcessId有值时，才能点查看流程
                 || selfCreateFlow == 'false') {//调用系统模板(非格式模板)后只能查看流程，调用个人模板后可以编辑流程
...
//因为调用模板时，是从外层框架中调用的，所以需要取消离开当前新建公文时弹出的确认对话框
 ...
 function initEdocIe10AutoScroll(){
	...
	function sendCallBack_newEdoc(){
	    ...
	    //客开：徐矿集团【保存附件类型】 chenqiang 2019年3月27日  start  
	    var id = $("input[name='option']:checked").attr("id");
	    var value = $("input[name='option']:checked").val();
	    $("#mainAttachmentId").val(id);
	    $("#mainAttachmentValue").val(value);
//	    var result = callBackendMethod();
	    //客开：徐矿集团【保存附件类型】 chenqiang 2019年3月27日  end 
	...
	function showQuickSend(){
	    ...
	    if(document.getElementById("isQuickSend").checked){
		 ...
		 if(isQuickSend){
		    ...
		    //归档控件
		    if(document.getElementById("sel_label_guidang") && document.getElementById("guidang_div") && hasDoc=="true"){
	        ...
	        if((typeof(hasPluginDoc)==="undefined" || hasPluginDoc=="true") &&((jsEdocType==0 && !confirm(edocLang.edoc_quicksend_return_send))||
	...
	function insertAttachmentFn(){
		//客开：徐矿集团【是否添加单选按钮】 chenqiang 2019年3月29 start
		xkjtAttachmentHasRadio = "true";
	    insertAttachment(null, null, "resizeLayout", "false");
	    //客开：徐矿集团【是否添加单选按钮】 chenqiang 2019年3月29 end
	    //resizeLayout();
	}
	...
	function initDate(reader){
	    if(reader){
	        ...
	        //主送单位
	        var sendTo = document.getElementById("my:send_to");
	        if(sendTo)
	...
	function _init_(){
        ...
        $("#show_more").click(function () {
            if (moreShow_st==0) {
                $(this).html(V5_Edoc().$.i18n("edoc.hide")+'<span class="ico16 arrow_2_t"></span>');//收起
                $(".newinfo_more").show();
                $("#send_td").attr("rowspan",3);
                moreShow_st=1;
            }else {
                $(this).html(V5_Edoc().$.i18n("edoc.expand")+'<span class="ico16 arrow_2_b"></span>');//展开
                $(".newinfo_more").hide();
	...
	function setDefaultContentMenu(){
		var installDoc = "";
		var installWps = "";
		try{
			if(v3x.isMSIE){
			    ...
			    installDoc= ocxObj.WebApplication(".doc");
			    installWps=ocxObj.WebApplication(".wps");
			}
		}catch(e){
			 installDoc = true;
			 installWps = true;
		}
		if(!installDoc && installWps){
			document.getElementById("bodyType").value = "WpsWord";
		}
		if(typeof(bodyType)!="undefined" && bodyType){
