...
function initMoreOperation(){
	 $("#caozuo_more_btn").click(function(){
...
function unLoad(processId, summaryId,userId){
	...
	try{
    	...
    	viewParam += ',"accountId":"'+summaryOrgAccountId+'"';
    	viewParam += ',"firstViewTime":"'+affairFirstViewTime+'"';
...
function edocSubmitFunc(){
	if(submitCount){
		alert(V5_Edoc().$.i18n("edoc.alert.repeat.click.js"));//正在提交中，请不要重复点击！
		return;
	}
	submitCount = true;
	...
	if(!top2.executeWorkflowBeforeEvent("BeforeFinishWorkitem",summaryId,affair_id,processId,processId,activityId,edocJsonStr,"sendEdoc")){
		enablePrecessButtonEdoc();
        return;
    }
	...
	top2.preSendOrHandleWorkflow(top2,workitemId,tmpId,processId,activityId, currentUserId,caseId,currentUserAccount,edocJsonStr,
		      "sendEdoc",caseProcessXML,window,edocSubmitForm);
}
 //工作流回掉，固定函数名
...
function edocSubmitForm() {
    ...
    try {
        ...
        //封发节点或基础操作中存在就交换类型的节点需要判断公文文号不能为空
        if(hasExchangeType){
        	var doc_mark = contentIframe.document.getElementById("my:doc_mark");
        	if(doc_mark != null && doc_mark.value == ""){
        ...
        if ((optionType == '3' || optionType == '4') && isFlowBack != "") {
            ...
            var url = genericURL + "?method=optionSetup&optionId=" + optionId
                    ...
                    + affair_id + "&ndate=" + new Date().getTime();

            window.seletcOpinionTypeWin = getA8Top().$.dialog({
                title:V5_Edoc().$.i18n("edoc.form.flowperm.setup.js"),//意见保留设置
                transParams:{'parentWin':window},
                url: url,
            ...
            if ((wfLastInput=="true" && edocType=="0" && contentIframe.checkEdocMarkIsRequireAndIsNotNull() ) || hasExchangeType) {//流程结束或者进行公文交换时需要判断公文文号
                if (permKey != 'fengfa' && contentIframe.checkEdocWordNoWfLastInput() == false) {
                	enablePrecessButtonEdoc();
...
function _exeCheckExchangeRoleCallback(value){
    ...
    if(value){
        ...
        try {
            var subForm = document.theform;
            var bodyType = document.getElementById("bodyType").value;
            if (getBrowserFlagByRequest && bodyType!="HTML") {
        ...
        if (list != null && list != "undifined" && list != "") {
            ...
            if (listArr.length > 1) {
                ...
                window.checkExchangeRoleWin = getA8Top().$.dialog({
                    ...
                    show:true,
                      handler:function(){
                          checkExchangeRoleCallback('cancel');
                      }
                    },
                    url : _url,
                    targetWindow : getA8Top(),
...
function showDigarm(id,fromSupervis) {
	...
	if(isOpenFrom == 'supervise'||fromSupervis=='fromSupervis'){//督办默认节点权限是 阅读
		top2.editWFCDiagramEdocModalDialog(window.getA8Top(),caseId,processId,window,appNameNode,templateFlag, flowPermAccountId,defaultNodeName,defaultNodeLable,editRefreshWorkflow,true,superviseEdocLabel);
	}else{
		top2.editWFCDiagram(window.getA8Top(),caseId,processId,window,appNameNode,templateFlag, flowPermAccountId,defaultNodeName,defaultNodeLable,editRefreshWorkflow,superviseEdocLabel);
	}
}

function getEdocDefaultNode(){
	var appName=appNameNode;
	 if(appName == "sendEdoc"){
		 appName="edocSend";
	 }else if(appName == "recEdoc"){
		 appName="edocRec";
	 }else if(appName == "signReport"){
		 appName="edocSign";
	 }
...
function showPrecessAreaTd(type){
	if(type=='content'){
		...
		if(!v3x.isMSIE&&bodyType=="gd"){
			alert(V5_Edoc().$.i18n("edoc.alert.GD.use.IE.js"));//当前浏览器不支持GD正文，请您使用IE浏览器
			return;
		}
	...
	if(type == 'edocform'){
		if(edocFormFrame){
			...
			edocFormFrame.style.width="";
		}
		edocformTR.style.display ='';
		if(workflowTR)workflowTR.style.display = 'none';
		if(edocform_input)edocform_input.className = 'deal_btn_l_sel';
	}
	else if(type == 'workflow'){
		if(edocFormFrame){
			edocFormFrame.style.height="1px";
			edocFormFrame.style.width="1px";
		}
		var wfUrl= 'workflow/designer.do?method=showDiagram&isTemplate='+templateFlag+'&isDebugger=false&scene='+scene+'&isModalDialog=false&caseId='+caseId+'&showHastenButton='+showHastenButton+'&appName=edoc&currentNodeId='+activityId+"&wendanId="+wendanId + CsrfGuard.getUrlSurffix();
		if(affairState == '1' && openFrom!='supervise'){//协同-待发
		  wfUrl+="&currentUserName="+encodeURIComponent(currentUserName)+"&currentUserId="+currentUserId + CsrfGuard.getUrlSurffix();
		  if(scene=='3'){
		...
		if(contentTR && bodyType == "HTML"){
			...
			if(!v3x.isMSIE && (bodyType == "OfficeWord" || bodyType == "OfficeExcel" || bodyType == "Pdf")){
				if(v3x.isChrome && (bodyType == "OfficeWord" || bodyType == "OfficeExcel" || bodyType == "Pdf")){
					LazyloadOffice('0');
				}else{
					if(bodyType == "Pdf"){
						alert(v3x.getMessage("edocLang.edoc_IeAndChrome_pdf_Disbaled"));
					}else{
						//Office转换
						document.getElementById('htmlContentIframe').src = detailURL+"?method=getOfficeTransContent&summaryId="+summaryId;
						if(edocFormFrame){
							edocFormFrame.style.height="1px";
							edocFormFrame.style.width="1px";
						}
						...
						content_input.className = 'deal_btn_m_sel';
						$("#htmlContentDiv").width($("#hhhhhhh").width()-100);
						$("#htmlContentDiv").height($("#hhhhhhh").height());
					}
				}
			}else{
				LazyloadOffice('0');
...
function edocInsertPeople(workitemId,processId,activityId,performer,caseId){
  //OA-42082 收文待办进行加签，默认的节点权限应该是阅读，不是审批
   var defaultNode = getEdocDefaultNode();
   var defaultNodeName = defaultNode.instance.defaultNodeName;
   parent.parent.insertNode(workitemId,processId,activityId,performer,caseId,appNameNode,false,defaultNodeName,wOrgAccountId,refreshWorkflow);
}
...
//调用工作流减签接口
function edocDeletePeople(workitemId,processId,activityId,performer,caseId,isTemplate){
	parent.parent.deleteNode(workitemId,processId,activityId,performer,caseId,refreshWorkflow,summaryId,affair_id,isTemplate);
}
//调用工作流知会接口
...
function transfer(){
     ...
     //移交加锁
     var re = EdocLock.lockWorkflow(puobj.processId, puobj.currentUser,EdocLock.TRANSFER);
     if(re[0] == "false"){
    	 enablePrecessButtonEdoc();
       parent.parent.$.alert(re[1]);
       return;
     }
	selectPeopleFun_transfer();
}
//选人后进入的方法
function transferFun(elements){
	disabledPrecessButtonEdoc();//置灰按钮
	if(elements.length>1){
		alert("edocLang.edoc_alert_transfer_only_one");
		enablePrecessButtonEdoc();//启用按钮
		return;
	}
...
function TransmitToBulletin(){
	...
	//如果是书生交换的收文，不允许转发 END
	  ...
	  edocBulletinIssue(summaryId,bodyType,bulletinIssueBack);
	}
	...
	$(document).ready(function(){
		...
		//浏览器兼容设置---start
		try{
		    $("#hhhhhhh,#signAreaTable,#contentIframe,#scrollContentTd div").height($("body").height()-document.getElementById("hhhhhhh").getBoundingClientRect().top - 10);
		}catch(e){}
		try{$("#iSignatureHtmlDiv,#inputPosition").css("height","1px");}catch(e){}
		try{$("#iSignatureHtmlDiv,#newInputPosition").css("height","1px");}catch(e){}
		//流程图
		...
		$(window).resize(function(){
		    try{
			    ...
			    $("#iSignatureHtmlDiv,#newInputPosition").css("height","1px");
		    }catch(e){}
			//流程图
			$("#workflowTR table").height($("#workflowTR").height());
		});
	...
	function windowResizeTop() {
		...
		if($contentIframe.css("height") !== "1px"){
		    // 当前在流程页面， 不做切换
		    $contentIframe.height(tempHeight);
		}
		try {
			$("#iSignatureHtmlDiv,#inputPosition").css("height", "1px");
			$("#iSignatureHtmlDiv,#newInputPosition").css("height", "1px");
...
function selectDateTime(request, obj, width, height) {
	...
	}else if(v3x.isFirefox){
		whenstart(request, obj, width, height, 'datetime',null,270,270,{'callBackFun':function(){
			if (obj.value != "") {
				var now = new Date();// 当前系统时间
...
/**
 * PC端查看，未加载文单就点击pdf文件报js
 */
function _pdfFullSize(){
	try{
		contentIframe.pdfFullSize();
	}catch(e){
	}
}

function setPipeonholeCheckboxChecked(){
	var pipeonhole = document.getElementById("pipeonhole");
	if(pipeonhole && pipeonhole.checked){
		document.getElementById("showSelectPigeonholePath").style.display="";
	}else{
		document.getElementById("showSelectPigeonholePath").style.display="none";
	}
}
