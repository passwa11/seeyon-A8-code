                    ...
                    $membersList.append(
                        '<ul class="members-row">' +
                            '<li class="member" memberIdx="' + row.index + '">' +
                                '<span class="member-name seated-longname">' + row.name.substring(0,10) + '</span>'+
                                '<span class="member-dep seated-longname">' + row.name.substring(10,row.name.length) + '</span>'+
                            '</li>' +
                        '</ul>'
                    );
                }else {
...
//5.生成左侧人员信息-未报送
function createMembers1(data) {
    var $membersList1 = $(".left-list > .members-list1");
    var ulIdx = 0;
    data.users1.forEach(function (row, idx) {
    	var usersIdx = parseInt(data.users.length)+parseInt(row.index);	
    	
    	if(row.name.length>10){
    		if (idx % 3 === 0) {			
                $membersList1.append(
                    '<ul class="members-row1">' +
                        '<li class="member1" memberIdx="' + usersIdx + '">' +
                            '<span class="member-name seated-longname">' + row.name.substring(0,10) + '</span>'+
                            '<span class="member-dep seated-longname">' + row.name.substring(10,row.name.length) + '</span>'+
                        '</li>' +
                    '</ul>'
    		...
    		if (idx % 3 === 0) {			
                $membersList1.append(
                    '<ul class="members-row1">' +
                        '<li class="member1" memberIdx="' + usersIdx + '">' +
                            '<span class="member-name seated-longname">' + row.name + '</span>'+
                            '<span class="member-dep seated-longname">' + row.dep + '</span>'+
                        '</li>' +
                    '</ul>'
                );
            } else {
...
function defaultSeats(data) {
    ...
    $.each(data.users, function(i, user) {
        if(user.row && user.col) {
            hasDefaultSeat = true;
            // 左侧选中
            $(".members-row  .member").eq(user.index).addClass("seated-member").attr("memberIdx", user.index);
            // 右侧落座
           ...
           // alert($(".seats-row").eq(user.row - 1).children(".seat-li").eq(user.col - 1).children(".seat").text());
            if($(".seats-row").eq(user.row - 1).children(".seat-li").eq(user.col - 1).children(".seat").text().length>6){
            	
            	var tempText = $(".seats-row").eq(user.row - 1).children(".seat-li").eq(user.col - 1).children(".seat").text();
            	var tempTextlen = tempText.length+1;
            	//alert(tempTextlen);
            	$(".seats-row").eq(user.row - 1).children(".seat-li").eq(user.col - 1).children(".seat").removeClass("seat").addClass("seat1").addClass("seated-longname").text(tempText.substring(0,6));
            	$(".seats-row").eq(user.row - 1).children(".seat-li").eq(user.col - 1).children(".seat1").after('<span class="seat2"></span>');
            	$(".seats-row").eq(user.row - 1).children(".seat-li").eq(user.col - 1).children(".seat2").addClass("seated-member").addClass("seated-longname").text(tempText.substring(6,tempTextlen));
                ...
                $(document).on(
                    {
                        ...
                        "mouseup": function (e) {
                            ...
                            if ((e.clientX > seats_x) && (e.clientY < seats_y) && !_page.isHandling) {
                                ...
                                var memberName = _page.curCloneMember.dom.children(".member-name").text();
                                var memberDep = _page.curCloneMember.dom.children(".member-dep").text();
                                if(memberName.length>=10){
                                	_page.curCloneMember.name = memberName + memberDep;   
                                }else{
                                	_page.curCloneMember.name = memberName;   
                                }
    ...
    el.on("mouseenter", function () {
        if (_page.curCloneMember.moved) {
            ...
            if($(this).attr("memberIdx")) {
                ...
                // 安插本次座位上的人员
            	//alert(_page.curCloneMember.name);
            	if(_page.curCloneMember.name.length>6){
            		$(this).text(_page.curCloneMember.name).removeClass("seat").addClass("seat1").addClass("seated-member").addClass("seated-longname").text(_page.curCloneMember.name.substring(0,6)).attr("memberIdx", _page.curCloneMember.index);
            		$(this).after('<span class="seat2 seated-member seated-longname">'+ _page.curCloneMember.name.substring(6, _page.curCloneMember.name.length+1) +'</span>');    
            	}else{
...
function quicklySeatMembers(data) {
    ...
    var $members = $(".members-list .member"),
        $members1 = $(".members-list1 .member1"),
        $seats = $(".seats-content .seat-li"),
    	$seats1 = $(".seats-content .seat1"),
    	$seats2 = $(".seats-box .seat2");
    $(".quickly-seat").on("click", function (e) {
        ...
        $members1.addClass("seated-member");
        // 清空当前排座
        clearSeats();
        clearSeats();
        $.each(data.users, function (idx, row) {
        	if(row.name.length>6){
        		//alert("haha:" + idx);
        		//debugger;
        		$seats.eq(idx).find(".seat").removeClass("seat").addClass("seat1 seated-member seated-longname").text(row.name.substring(0,6)).attr("memberIdx", row.index);
        		$seats.eq(idx).find(".seat1").after('<span class="seat2 seated-member seated-longname">'+ row.name.substring(6,row.name.length+1) +'</span>');        		
        	}else{
        		$seats.eq(idx).find(".seat").text(row.name).addClass("seated-member").attr("memberIdx", row.index);
        	}
           
           
        	
        	if($seats.eq(idx).find(".seat").attr("row")){
        		 _page.insertedMembersSeats.push({
                     row: $seats.eq(idx).find(".seat").attr("row"),
                     col: $seats.eq(idx).find(".seat").attr("col"),
                     index: row.index
                 });
        	}
        	
//        	if($seats.eq(idx).find(".seat1").attr("row")){
//        		_page.insertedMembersSeats.push({
//                    row: $seats.eq(idx).find(".seat1").attr("row"),
//                    col: $seats.eq(idx).find(".seat1").attr("col"),
//                    index: row.index
//                });
//        	}
        ...
        var users_len = parseInt(data.users.length);
        $.each(data.users1, function (idx, row) {
        	if(row.name.length>6){
        		$seats.eq(idx + users_len).find(".seat").removeClass("seat").addClass("seat1 seated-member seated-longname").text(row.name.substring(0,6)).attr("memberIdx", parseInt(row.index + users_len));
        		$seats.eq(idx + users_len).find(".seat1").after('<span class="seat2 seated-member seated-longname">'+ row.name.substring(6,row.name.length+1) +'</span>');        		
        	}else{
        		$seats.eq(idx + users_len).text(row.name).addClass("seated-member").attr("memberIdx", row.index + users_len);
        	}
...
//            _page.insertedMembersSeats.push({
//                row: $seats1.eq(idx + users_len).attr("row"),
//                col: $seats1.eq(idx + users_len).attr("col"),
//                index: row.index + users_len
//            });
...
function cancelCurSeat() {
    ...
    $(".del-icon").on("click", function (e) {
        ...
        if(memberIdx==undefined){
        	 memberIdx = $(this).parent().siblings(".seat1").attr("memberIdx");
        }
        ...
        clearSeats($(this).parent().siblings(".seat1"));
        clearSeats($(this).parent().siblings(".seat2"));
...
function clearMembers(memberIdx) {
    var $members = $(".members-list .member"),
    	...
    	$members1_len = $members1.length;

    // memberIdx用来标记是否是批量操作
    if (memberIdx) {
    	if(memberIdx>=parseInt($members_len)){
    		for (var i = 0; i < $members1_len; i++) {
	            if (memberIdx === $members1.eq(i).attr("memberIdx")) {
	                $members1.eq(i).removeClass("seated-member");
	                return;
	            }
	        }
    	}else{
    ...
    // 一键清空
        ...
        $members1.removeClass("seated-member");
    }
...
function clearSeats(curSeat) {
    if (_page.insertedMembersSeats.length) {
        ...
        if (curSeat) {
            ...
            $(".seats-box .seat1").text("").addClass("seat").removeClass("seated-member is-part-handleing seat1 seated-longname").attr("memberIdx", null).siblings(".icon-box").children().removeClass("show");
            $(".seats-box .seat2").remove();
        ...
        if (_page.curCloneSeat.moved) {
            ...
            if(_page.insertedMembersSeats.length) {
                ...
                if(curSeatMemberIdx) {
                    ...
                    // 安插本次座位上的人员
                	if(_page.curCloneSeat.name.length>6){
                		$(this).text(_page.curCloneSeat.name.substring(0,6)).attr("memberIdx", _page.curCloneSeat.index).removeClass("seat").addClass("seated-member is-part-handleing seat1 seated-longname").siblings(".icon-box").children().addClass("show");
                		$(this).after('<span class="seat2 seated-member seated-longname is-part-handleing">'+ _page.curCloneSeat.name.substring(6,_page.curCloneSeat.name.length+1) +'</span>');    
                    ...
                    for (var o = 0; o < _page.insertedMembersSeats.length; o++) {
                        ...
                        if (_page.curCloneSeat.index === item.index) {
                            ...
                            // 清空上次座位上的text()和样式
                        	if(_page.curCloneSeat.name.length>6){
                        		//alert("text:"+$(".seats-box .seats-row").eq(item.row - 1).find(".seat").eq(item.col - 1).text());
                        		//alert("当前索引："+_page.curCloneSeat.index);
                                //alert("CLONECUR:"+item.index);
                        		
                        		
                        		$(".seats-box .seats-row").eq(item.row - 1).find(".seat1").eq(item.col - 1).text("").addClass("seat").removeClass("seated-member is-part-handleing seat1 seated-longname").attr("memberIdx", null).siblings(".icon-box").children().removeClass("show");
                        		$(".seats-box .seats-row").eq(item.row - 1).find(".seat2").eq(item.col - 1).remove();
                        	}else{
                        		//alert("text1:"+$(".seats-box .seats-row").eq(item.row - 1).find(".seat").eq(item.col - 1).text());
                        		//alert("当前索引："+_page.curCloneSeat.index);
                                //alert("CLONECUR:"+item.index);
...
function OK() {
    ...
    if(!exportData.valid) {
        ...
        for(var i = 0, seated_len = _page.insertedMembersSeats.length; i < seated_len; i++) {
        	//alert("length:"+ _page.insertedMembersSeats.length);
        	//循环报送人员
...
//                	alert("length:"+_page.resData.users[j].index);
//                	alert("name:"+_page.resData.users[j].name);
//                	alert("dep:"+_page.resData.users[j].dep);
//                	alert("row:"+_page.insertedMembersSeats[i].row);
//                	alert("col:"+_page.insertedMembersSeats[i].col);
            ...
            //循环未报送单位
            for(var k = 0, resData1_len = _page.resData.users1.length; k < resData1_len; k++) {
            	var allIndex = parseInt(_page.resData.users1[k].index) + parseInt(_page.resData.users.length);
                if(_page.insertedMembersSeats[i].index == allIndex) {
//                	alert("name:"+_page.resData.users1[k].name);
//                	alert("dep:"+_page.resData.users1[k].dep);
//                	alert("row:"+_page.insertedMembersSeats[i].row);
//                	alert("col:"+_page.insertedMembersSeats[i].col);
                    exportData.users.push({
