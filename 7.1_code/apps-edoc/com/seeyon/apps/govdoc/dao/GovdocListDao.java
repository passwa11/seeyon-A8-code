...
public class GovdocListDao {
	...
	//公文列表主表Clob字段Hql
	public static final String SummaryColbListHql = "summary.currentNodesInfo, "
			+ "summary.sendTo, "
			+ "summary.copyTo, "
			+ "summary.reportTo, "
			+ "summary.printUnit, "
	...
	//公文登记列表主表字段Hql(非Clob)
	public static final String RegSummaryHql = "summary.id"
			+ ",summary.subject"
			+ ",summary.identifier"
			+ ",summary.govdocType"
			+ ",summary.edocType"
	...
	public static final String RegClobSummaryHql = "summary.sendTo"
			+ ",summary.sendTo2"
			+ ",summary.sendToId"
			+ ",summary.sendToId2"
			+ ",summary.copyTo"
	...
	/**public static final String AdminManagerHql = "summary.id,summary.subject,summary.createTime,summary.processId,summary.caseId,summary.orgAccountId,affair.finish,summary.templeteId,summary.completeTime,summary.deadlineDatetime,summary.secretLevel,summary.currentNodesInfo" 
																		+ ",affair.app,affair.subApp,affair.deadlineDate,affair.remindDate," 
																		+ ",m.name,u.name";
	*/
	public static final String AdminManagerHql = "affair.subApp,summary.id,affair.app,summary.subject,m.name,u.name,summary.createTime,summary.processId,summary.caseId, " 
	...
	//公文登记列表登记字段Hql
	public static final String RegRecHql = "register.id";
	...
	public List<GovdocListVO> findList(FlipInfo flipInfo, Map<String, String> params) throws BusinessException {
		...
		if(flipInfo != null) {
			flipInfo.setNeedTotal(needTotal);
		}
	...
	public List<Object[]> findExchangeSignColumns(List<Long> summaryIdList) {
		String hql = "select detail.summaryId, detail.sendAccountId, detail.sendUnit, detail.recUserName, detail.recOrgName, detail.recTime from GovdocExchangeDetail detail where detail.summaryId in (:summaryId)";
		Map<String, Object> paramMap = new HashMap<String, Object>();
	...
	public List<Object[]> findOldExchangeSignColumns(List<Long> summaryIdList, List<Long> odrecieveIdList) {
		String hql = "select detail.edocId, detail.sendUnit, detail.recUserId, detail.recTime from EdocRecieveRecord detail where detail.edocId in (:summaryId) and id in (:recieveId)";
		Map<String, Object> paramMap = new HashMap<String, Object>();
		paramMap.put("summaryId", summaryIdList);
	...
	public List<WorkflowData> getAdminWfDataList(FlipInfo flipInfo,Map<String,Object> conditionParam,long accountId,boolean isPage,FlipInfo fi) {
	    ...
	    buffer.append(" from CtpAffair affair,EdocSummary summary,OrgMember m,OrgUnit u");
	    ...
	    buffer.append(" where u.id = summary.orgDepartmentId and m.id=summary.startUserId and summary.id = affair.objectId ");
...
//            flow.setEndFlag((int)end);
            ...
            if(app == ApplicationCategoryEnum.edoc.key() && sub_app != -1){
            	if(sub_app == ApplicationSubCategoryEnum.edoc_fawen.key()){//發文
            		...
            		appStr = ResourceUtil.getString("edoc.docmark.inner.send");
            		flow.setAppEnumStr(com.seeyon.ctp.common.constants.ApplicationCategoryEnum.edocSend.name());
            		flow.setEdocType(FormType.govDocSendForm.getKey());
            		flow.setNps(ApplicationCategoryEnum.edocSend.name());
            	}else if(sub_app == ApplicationCategoryEnum.edocRec.getKey()){//收文
            		appStr = ResourceUtil.getString("edoc.docmark.inner.receive");
            		flow.setAppEnumStr(com.seeyon.ctp.common.constants.ApplicationCategoryEnum.edocRec.name());
            		flow.setEdocType(FormType.govDocReceiveForm.getKey());
            		flow.setNps(ApplicationCategoryEnum.edocRec.name());
            	}else if(sub_app == ApplicationCategoryEnum.edocSign.getKey()){//签报
            		appStr = ResourceUtil.getString("edoc.docmark.inner.signandreport");
            		flow.setAppEnumStr(com.seeyon.ctp.common.constants.ApplicationCategoryEnum.edocSign.name());
            		flow.setEdocType(FormType.govDocSignForm.getKey());
	...
	private String getHqlByAffairMember(Map<String, Object> paramMap, Long memberId, boolean hasAgent, String affairTable) {
		...
		if(hasAgent) {
			...
			//if("listPendingAll".equals(listType) || "listDoneAll".equals(listType)) {
		        List<AgentModel> edocAgentList = GovdocHelper.getEdocAgentList(memberId);
		        if(Strings.isNotEmpty(edocAgentList)) {
		        	List<Integer> agentStateList = new ArrayList<Integer>();
		        	agentStateList.add(StateEnum.col_pending.key());
		        	agentStateList.add(StateEnum.col_done.key());
			        for(int i=0; i<edocAgentList.size(); i++) {
			        	buffer.append(" or ");
			        	buffer.append("( " + affairTable+".memberId = :memberId"+i + " and "+affairTable+".receiveTime >= :receiveTimeS"+i + " and "+affairTable+".state in (:agentState) )");
			        	
			        	AgentModel agent = edocAgentList.get(i);
			        	paramMap.put("memberId"+i, agent.getAgentToId());
			            paramMap.put("receiveTimeS"+i, agent.getStartDate());
			            paramMap.put("agentState", agentStateList);
			        }
		        }
			//}
		}
