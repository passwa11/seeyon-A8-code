...
public class GovdocSuperviseHandler implements SuperviseHandler{
	...
	public List<SuperviseWebModel> getSuperviseDataList(FlipInfo fi,
		...
		.append("summ.govdocType,")
		.append("summ.formAppid,")
		.append("summ.formRecordid");
        ...
        if(result != null && !result.isEmpty()){
            for(int i=0;i<result.size();i++){
               ...
               model.setCurrentNodesInfo(EdocHelper.parseCurrentNodesInfo(summary));
                ...
                model.setAppName(GovdocUtil.getNPSAppName(Integer.parseInt(govdocType), edocType));
              ...
              //设置流程是否超期标志
				...
				Object formAppId = res[j++];
				if(formAppId != null && !"".equals(formAppId)){
					model.setFormAppId(Long.valueOf(formAppId.toString()));	
				}
				Object formRecordId = res[j++];
				if(formRecordId != null && !"".equals(formRecordId)){
					model.setFormRecordId(Long.valueOf(formRecordId.toString()));
				}
                modelList.add(model);
            }
        }
	...
	@Override
	public SuperviseMessageParam getSuperviseMessageParam4SaveImmediate(Long arg0) throws BusinessException {
		EdocSummary summary = govdocSummaryManager.getSummaryById(arg0);
		if(summary == null){
			return null;
		}
	...
	public long getFlowPermAccountId(long summaryId) throws BusinessException {
		EdocSummary  s = govdocSummaryManager.getSummaryById(summaryId,false);
		if(s!=null){
	...
	public Map<String, Object> getWFParms4BugReport(Long summaryId) throws BusinessException{
		...
		int appEnumKey = EdocUtil.getApplicationCategoryEnumKeyByEdocType(summary.getEdocType());
		ApplicationCategoryEnum appEnum = ApplicationCategoryEnum.valueOf(appEnumKey);
		
		PermissionVO vo = permissionManager.getDefaultPermissionByApp(appEnum, summary.getOrgAccountId());
		if(vo != null){
			ret.put("defaultNodeName", vo.getName());
			ret.put("defaultNodeLable", vo.getLabel());
		}
		
		ret.put("nps", appEnum.name());

		ret.put("isTemplate", summary.getTempleteId() != null ? true : false);
		ret.put("appEnumStr", appEnum.name());
		ret.put("flowPermAccountId", summary.getOrgAccountId());
