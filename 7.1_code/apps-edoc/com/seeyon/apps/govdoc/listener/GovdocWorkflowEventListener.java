	...
	public boolean onWorkflowAssigned(List<EventDataContext> contextList) {
		...
		if (affairData == null && Strings.isNotBlank(processIdStr) && Strings.isDigits(processIdStr)){
			try {
				...
				if(null!=summary){
					affairData = EdocHelper.getAffairData(summary,AppContext.getCurrentUser());
				}
			} catch (Exception e) {
				LOGGER.error("", e);
			}
		...
		try {
			...
			boolean processCanAnyDealMerge = GovdocUtil.canMergeDealByType(BPMSeeyonPolicySetting.MergeDealType.DEAL_MERGE,summary);
			boolean processCanPreDealMerge = GovdocUtil.canMergeDealByType(BPMSeeyonPolicySetting.MergeDealType.PRE_DEAL_MERGE,summary);
			boolean processCanStartMerge = GovdocUtil.canMergeDealByType(BPMSeeyonPolicySetting.MergeDealType.START_MERGE,summary);
...
//					}
					...
					Long proxyMemberId = MemberAgentBean.getInstance().getAgentMemberId(ApplicationCategoryEnum.edoc.getKey(), memberId, summary.getTempleteId());
					if(proxyMemberId == null){
						affair.setProxyMemberId(0L);
					}else{
...
//		}
		//发送自动跳过的事件
		GovdocHelper.fireAutoSkipEvent(this, (Map<String, String>) global.get(WF_APP_GLOBAL_REPEAT_AFFMAP));
		return true;
	}
	...
	public boolean onWorkitemCanceled(EventDataContext context) {
		try {
			...
			if(operationType == TAKE_BACK || operationType == WITHDRAW || operationType == SPECIAL_BACK_RERUN) {
				...
				DateSharedWithWorkflowEngineThreadLocal.addToTraceDataMap("traceData_affair", traceList);// 发消息用
...
//					}
	...
	public boolean onWorkitemWaitToReady(EventDataContext context) {
		try {
        	...
        	//指定回退-提交给我 再处理时，更新当前待办人
        	List<CtpAffair> affairList = affairManager.getAffairs(summary.getId(), StateEnum.col_pending);
    ...
    public boolean onSubProcessStarted(EventDataContext context) {
        try {
            ...
            SendGovdocResult sendGovdocResult =govdocPubManager.transSendColl(EdocConstant.SendType.child, templete.getId(),
                    Long.parseLong(sendId), affairData.getFormRecordId(), oldSummaryId, null, reciveAccountId,Integer.valueOf(bodyType),hasAtts, null);
            //子流程的已发事项
