	...
	public ModelAndView newEdocRegister(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if("create".equals(comm)) {//新建
        	...
        	if(record == null && edocRegister==null && affair != null) {
        		...
        		} else if(affair.getObjectId() != null) {
...
*/
        ...
        if("create".equals(comm) && oldRegister == null) {
        	...
        	summary.setBodyType(String.valueOf(GovdocUtil.getContentType(registerBody.getContentType())));
        ...
        if(edocRecieveRecord!=null) {
	        ...
	        if("2".equals(registerState)) {
		        if(edocRegister.getState() != null && edocRegister.getState() == EdocNavigationEnum.RegisterState.Registed.ordinal()) {
		        	//去掉待登记
			...
			for (int i = 0; i < registerId.length; i++) {// 批量处理需要办理的待发文单
				...
				if (summary != null && edocRegister != null) {
					...
					try {
						...
						senderOninion.setPolicy(request.getParameter("policy"));
						senderOninion
								.setOpinionType(EdocOpinion.OpinionType.senderOpinion
										.ordinal());
						senderOninion.setNodeId(0);
						String comm = "new_form";
						Long agentToId = null;
    ...
    public ModelAndView updateFormData(HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        try{
        	...
        	edocSummary.setCanTrack(isTrack);
 	        ...
 	        baseVo.setCurrentDate(new java.util.Date());
 	        baseVo.setCurrentUser(AppContext.getCurrentUser());
...
//		}else
	...
	public ModelAndView detailIFrame(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(Strings.isNotEmpty(request.getParameter("affairId")) && !"undefined".equals(request.getParameter("affairId"))
			...
			CtpAffair affair = affairManager.get(affairId);
			if (affair == null) {
				GovdocUtil.webAlertAndClose(response, "数据已删除，你无法访问该数据！");
				return null;
			}
	...
	public ModelAndView detail(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			boolean isglwgFlag = "glwd".equals(openFrom) || "glwd".equals(from);
...
//				}
	...
	public ModelAndView edocDetailInDoc(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (Strings.isNotBlank(summaryId)) {
			...
			CtpAffair govdocAffair = affairManager.get(Long.valueOf(summaryId));
...
//        			fpm = flowPermManager.getFlowPerm(edocTypeEnum.name(), nodePermissionPolicy, flowPermAccountId);
        			...
        			permission = permissionManager.getPermission(edocTypeEnum.name(), nodePermissionPolicy, flowPermAccountId);
        			//获取当前节点名称
        			if(permission!=null){
...
//        			if(null!=fpm){
                ...
                if(Strings.isEmpty(baseActions)){
        			if(org.apache.commons.collections.CollectionUtils.isEmpty(baseActions) && "chuanyue".equals(nodePermissionPolicy)){
            			mav.addObject("nodePermissionPolicyName","阅读");
            		}
        			LOGGER.error("edocController.summary : baseActions is null! [edocTypeEnum.name():"+edocTypeEnum.name()+" nodePermissionPolicy:"+nodePermissionPolicy +" flowPermAccountId:"+flowPermAccountId+" summary.id:"+summary.getId());
        		}
        		//常用操作
...
//        }
       ...
       /*	   affair = (CtpAffair)request.getSession().getAttribute(Constant.AFFAIR_TO_SESSEION);
        	...
        	String catType= EdocUtil.getEdocMetadataNameEnum(summary != null ? summary.getEdocType() : null).name();
...
//        		}
        ...
        List<V3xHtmDocumentSignature> signatuers = htmSignetManager.findBySummaryIdAndType(summary.getId(), V3xHtmSignatureEnum.HTML_SIGNATURE_DOCUMENT.getKey());
