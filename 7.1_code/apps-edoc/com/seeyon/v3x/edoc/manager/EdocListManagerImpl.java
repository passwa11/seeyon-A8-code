...
public class EdocListManagerImpl implements EdocListManager {
  		...
  		if(EdocHelper.isG6Version() && edocType == EdocEnum.edocType.recEdoc.ordinal() 
  			...
  			for (int i = 0; i < result.size(); i++) {
	            ...
	            if(summary.getDeadlineDatetime()==null){
	            	model.setDeadlineDisplay((deadline == null || deadline == 0)? isNull : WFComponentUtil.getDeadLineNameForEdoc(deadline,summary.getCreateTime()));
	            }else{
	            	model.setDeadlineDisplay(EdocHelper.getDeadLineName(summary.getDeadlineDatetime()));
	            }
	...
	public Map<String, Object> setEdocListCondition(int type, Map<String, Object> condition) {
		...
		if(edocType == -1) {
			...
			//兼容新公文
			appList.add(ApplicationCategoryEnum.edoc.getKey());
		} else {
			appList.add(ApplicationCategoryEnum.edoc.getKey());
		}
		//公文处理状态
		...
		if(edocType == -1) {
			subAppList.add(ApplicationSubCategoryEnum.edoc_fawen.getKey());
			subAppList.add(ApplicationSubCategoryEnum.edoc_shouwen.getKey());
			subAppList.add(ApplicationSubCategoryEnum.edoc_qianbao.getKey());
			subAppList.add(ApplicationSubCategoryEnum.old_edocSend.getKey());
			subAppList.add(ApplicationSubCategoryEnum.old_edocRec.getKey());
			subAppList.add(ApplicationSubCategoryEnum.old_edocSign.getKey());
		} else {
			subAppList.addAll(GovdocUtil.getSubAppByEdocType(edocType));
		}
		...
		condition.put("subAppList", subAppList);
		return condition;
	}
	...
	private List<EdocSummaryModel> bindListDataToEdocBo(int type, Map<String, Object> condition, List<Object[]> result, EdocSearchModel em) throws BusinessException {
        ...
        for (int i = 0; i < size; i++) {
          ...
          //A8 收文待发 查询登记信息
            ...
            model.setFinshed(affair.getState() != StateEnum.col_pending.getKey() && summary.getCompleteTime()!= null);
            //公文状态
            ...
            if (isTrack != null) {
              if(EdocConstant.flowState.finish.ordinal() == summary.getState()){
	...
	public List<EdocSummaryCountVO> getCountGroupByEdocType() throws BusinessException {
		...
		Integer app, state, subState, govdocType;
