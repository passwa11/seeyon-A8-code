...
@CheckRoleAccess(roleTypes={Role_NAME.AccountAdministrator,Role_NAME.EdocManagement,Role_NAME.AccountGovdocSend,Role_NAME.AccountGovdocRec,Role_NAME.DepartmentGovdocSend,Role_NAME.DepartmentGovdocRec})
public class ExchangeEdocController extends BaseController {
	...
	public ModelAndView list(HttpServletRequest request, HttpServletResponse response)throws Exception{
		...
		*/
		...
		if(EdocHelper.isG6Version()&& !EdocSwitchHelper.isOpenRegister().equals("1")) {
	...
	public ModelAndView change(HttpServletRequest request,HttpServletResponse response)throws Exception{
		...
		//获取代理和代理的人员集合
        List<Long> agentIds = MemberAgentBean.getInstance().getAgentToMemberId(ModuleType.edoc.getKey(),user.getId());
        List<Long> pishiIds = govdocPishiManager.getEdocUserLeaderId(user.getId());
        
		//公文交换权
		isExchangeRole = GovdocRoleHelper.isExchangeRole();
		if(!isExchangeRole) {
			...
			if(!CollectionUtils.isEmpty(agentIdList)) {
				...
				for(int i=0; i<agentIdList.size(); i++) {
					...
					agentIsExchangeRole = GovdocRoleHelper.isExchangeRole(agentIdList.get(i), accountIdOfagentId);
		...
		if(!isExchangeRole) {
			try {
				...
				if (f.booleanValue()) {
					out.println("parent.doEndSign_exchange('','"+affairId+"', '"+modelType+"');"); 
				} else {
		...
		Long pishiToId = null;//被代理人ID
		String agentToName = "";
		String pishiToName = "";
		V3xOrgMember currentMember = null;
		if(Strings.isNotBlank(affairId)){
			CtpAffair affair = affairManager.get(Long.valueOf(affairId));
			currentMember = orgManager.getMemberById(affair.getMemberId());
			if(!affair.getMemberId().equals(user.getId())){
				if(agentIds != null && agentIds.contains(affair.getMemberId())){
					agentToName = currentMember.getName();
					agentToId = currentMember.getId();
				}else if(pishiIds != null && pishiIds.contains(affair.getMemberId())){
					pishiToName = currentMember.getName();
					pishiToId = currentMember.getId();
				}
			}
		}
 		...
 		if(null!=modelType && !"".equals(modelType) && "toSend".equals(modelType)){
			...
			******************/
            ...
            try {
				...
				// 在已发中和待签收产生数据
				Map<String, String> sendEdoc = edocExchangeManager.sendEdoc(record, currentMember!=null?currentMember.getId():user.getId() ,currentMember!=null?currentMember.getName():user.getName(),agentToId != null? agentToId : pishiToId,isResend,exchangeModeValues);
				...
				conditions.put("subApp", ApplicationSubCategoryEnum.old_exSend.key());
	        	...
	        	long memberId = agentToId==null ? user.getId() : agentToId != null? agentToId : pishiToId;
	        	CtpAffair currentAffair = null;
	        	CtpAffair cloneAffair = null;
	        	Date currentDate = new Date();
	        	List<CtpAffair> aList =  new ArrayList<CtpAffair>();
				if(null!=affairList && affairList.size()>0){
					for(CtpAffair af : affairList) {
	            		...
	            		if(af.getMemberId().longValue() != user.getId()){// 检查代理和代录
	            			if(agentIds != null && agentIds.contains(af.getMemberId().longValue())){
	            				af.setDelete(false);
	            			}
	            			if(pishiIds != null&& pishiIds.contains(af.getMemberId().longValue())){
	            				af.setDelete(false);
		            			AffairUtil.addExtProperty(af,AffairExtPropEnums.dailu_pishi_mark, "pishi"+AppContext.currentUserId());
	            			}
	            		}
	            		af.setState(StateEnum.edoc_exchange_sent.getKey());
						//af.setDelete(true);
						af.setFinish(true);
						af.setCompleteTime(currentDate);
						cloneAffair = (CtpAffair)af.clone();
						aList.add(af);
					}
					affairManager.updateAffairs(aList);
				}
				if(currentAffair==null && cloneAffair!=null) {
					...
					currentAffair.setCompleteTime(currentDate);
					affairManager.save(currentAffair);
				}
				
				//GWSJ-232 【升级】5.6sp1待发送数据，发送后数据未进入已办和已分送列表
				if(!"true".equals(reSend) && currentAffair != null) {
					EdocSummary summary = edocSummaryManager.findById(currentAffair.getObjectId());
					summary.setExchangeSendAffairId(currentAffair.getId());
					edocManager.update(summary);
				}
				
				EdocSummary newsummary = new EdocSummary();
				newsummary.setId(record.getId());
				newsummary.setSubject(record.getSubject());
				newsummary.setEdocType(99);
				newsummary.setDeadline(-1L);
				newsummary.setHasArchive(false);
				newsummary.setGovdocType(0);
				newsummary.setState(EdocConstant.flowState.oldexchange.ordinal());
				newsummary.setDocMark(record.getDocMark());
				newsummary.setSecretLevel(record.getSecretLevel());
				newsummary.setUrgentLevel(record.getUrgentLevel());
				newsummary.setCreateTime(record.getCreateTime());
				try {
					V3xOrgMember mem = orgManager.getMemberById(record.getSendUserId());
					if(null != mem){
						newsummary.setCreatePerson(mem.getName());
					}
				} catch (BusinessException e) {}
				newsummary.setStartUserId(record.getSendUserId());
				newsummary.setOrgAccountId(record.getExchangeOrgId());
				newsummary.setStartTime(record.getSendTime());
				edocSummaryManager.saveEdocSummary(newsummary);
	        	...
	        	if(null!=receivers && receivers.size()>0){
	        		...
	        		}else if(pishiToId !=null){
	        			userMessageManager.sendSystemMessage(new MessageContent(sendKey, affairList.get(0).getSubject(), pishiToName,affairList.get(0).getApp()).add("edoc.pishi.deal", user.getName()), ApplicationCategoryEnum.edocSend,pishiToId, receivers,EdocMessageFilterParamEnum.exchange.key);
	        		}else{
        ...
        /***********************收文签收*********************/
			...
			try {
				...
				baseVo.setOldSignRecord(record);
				baseVo.setCurrentDate(new Date());
				baseVo.setCurrentUser(AppContext.getCurrentUser());
				govdocMarkManager.saveSignMarkOld(baseVo);
				record = baseVo.getOldSignRecord();*/
				...
				Map<String, Object> conditions = new HashMap<String, Object>();
				conditions.put("app", ApplicationCategoryEnum.edoc.key());
				conditions.put("subApp", ApplicationSubCategoryEnum.old_exSign.key());
	            conditions.put("objectId", record.getEdocId());
	            conditions.put("subObjectId", record.getId());
	          ...
	          //每个版本都需要录入登记日志
				...
				if(!EdocHelper.isG6Version()||isOpenRegister.equals("1")){
				...
				//当G6版本关闭登记开关时，那么签收时，还要生成登记数据，并生成待分发affair
			      ...
			      //v3x_affair表中增加待分发事项
			        distributeAffair(register.getSubject(), register.getDocMark(), register.getSendUnit(), register.getSendUnitId(), register.getRegisterUserId(),register.getDistributerId(),register.getId(),
					...
					//2015年8月28日 修改isOpenRegister判断  简化公文交换环节，选择合并签收、登记、分发操作 xiex
			        if(!EdocHelper.isG6Version() || isOpenRegister.equals("3")) {
			    		if(null != request.getParameter("isListClick")) {
			    			out.println("<script>");
			                if ((Strings.isNotBlank(request.getParameter("isOpenByToReceive"))) &&  ("true".equals(request.getParameter("isOpenByToReceive")))) {
			                	out.println("window.top.opener.toNewRegister('"+register.getId()+"');");
			                } else {
			                    out.println("window.top.opener.parent.parent.document.getElementById('mainIframe').src = '/seeyon/edocController.do?method=entryManager&entry=recManager&resourceCode=F07_recManager&toNewRegister=" + register.getId() + "'");
			                }
						...
						//给分发人的代理人发消息
						sendRegisterMessage(key , subject, recieverName, agentToName, register.getRegisterUserId(), registerId, distributerId, "", "", url, linkOpenType, registerId, "");
						...
						if(agentMemberId != null) {
							sendRegisterMessage(key , subject, recieverName, agentToName, register.getRegisterUserId(), registerId, agentMemberId, "col.agent", "", url, linkOpenType, registerId, agentApp);
						}
					 }
				}
			} catch(Exception e) {
	...
	private void copyRegisterData(EdocSummary edocSummary,EdocRegister edocRegister){
    	//复制部分信息到summary中
		edocSummary.setId(edocRegister.getId());
		edocSummary.setSubject(edocRegister.getSubject());
		edocSummary.setEdocType(99);
...
//		sendMessageToRegister(user, agentToId, reAffair);//待登记人不需要消息了
      ...
      //装载公文正文
        ...
        //同时在edoc_summary表中增加数据
        EdocSummary summary2 = new EdocSummary();
        copyRegisterData(summary2,register);
        edocSummaryManager.saveEdocSummary(summary2);
...
//        	record.setRegisterTime(new java.sql.Timestamp(register.getRegisterDate().getTime()));
        ...
        //v3x_affair表中增加待分发事项
        distributeAffair(register.getSubject(),register.getDocMark(), register.getSendUnit(), register.getSendUnitId(), register.getRegisterUserId(),register.getDistributerId(),register.getId(),summary.getId(),
		...
		out.println("<script>");
		if (f.booleanValue()) {
			out.println("parent.doEndSign_exchange('','"+affairId+"','toReceive');"); 
		} else {
			out.println("window.getA8Top().close();");
		}
		out.println("</script>");
...
//			userMessageManager.sendSystemMessage(messageContent, ApplicationCategoryEnum.edocRec, fromUserId, receiver);
	...
	public boolean isExchangeEdoc(String modelType,String id) throws BusinessException{
		...
		/**
		 * 获得我代录的人，他有交换权限依然可以打开
		 */
		List<Long> pishiToList = govdocPishiManager.getEdocUserLeaderId(user.getId());
		if(Strings.isNotEmpty(pishiToList)){
			exchangeUsers.addAll(pishiToList);
		}
		...
		for(Long userId : exchangeUsers){
			...
			if(exchangeType == 2){
				...
				flag = GovdocRoleHelper.isDepartmentExchange(userId,departmentId,accountId);
			}
			//单位交换
			else{
				accountId = departmentId;
				flag = GovdocRoleHelper.isAccountExchange(userId,accountId);//用户是否为单位公文收发员
			}
			if(flag)break;
		}
	...
	public ModelAndView edit(HttpServletRequest request,HttpServletResponse response)throws Exception{
		...
		if(!isExchangeEdoc(modelType,id)){
			...
			jsBuffer.append("    parent.doEndSign_exchange('"+hasNoRoleAlert+"','"+affairId+"','"+modelType+"');");
		...
		if("sent".equals(modelType)){
			...
			*/
			List<EdocSendDetail> sendDetailList = new ArrayList<EdocSendDetail>();
			List<EdocSendRecord> sendRecordList = sendEdocManager.getEdocSendRecordOnlyByEdocId(bean.getEdocId());
			Map<String, String> sendOrgIdMap = new HashMap<String, String>();
			if(Strings.isNotEmpty(sendRecordList)) {
				for(int i = sendRecordList.size() - 1; i >= 0; i--) {
					EdocSendRecord recordBean =  sendRecordList.get(i);
					List<EdocSendDetail> list = sendEdocManager.getDetailBySendId(recordBean.getId());
					if(Strings.isNotEmpty(list)) {
						for(EdocSendDetail detailBean : list) {
							if(!sendOrgIdMap.containsKey(detailBean.getRecOrgId())) {
								sendOrgIdMap.put(detailBean.getRecOrgId(), detailBean.getRecOrgId());
								sendDetailList.add(detailBean);
							}
						}
					}
				}
...
//	        	 mav.addObject("recEdocId",bean.getEdocId());
		...
		/***********************收文签收*********************/			
            ...
            if(bean.getRecUserId() != 0){
            	...
            	if(affair != null) {
            		recUserId = affair.getMemberId();
            		V3xOrgMember member = orgManager.getMemberById(recUserId);
            		recUserName = member == null ? "" : member.getName();
            	} else {
	                ...
	                if(type == EdocRecieveRecord.Exchange_Receive_iAccountType_Org){
	                    if(!GovdocRoleHelper.isAccountExchange()){
	                        //所有单位收发员
	                        List<V3xOrgMember> members = GovdocRoleHelper.getAccountExchangeUsers();
	                        //获得我代理的人
	                    ...
	                    if(!GovdocRoleHelper.isDepartmentExchange(user.getId(),bean.getExchangeOrgId(),user.getLoginAccount())){
	                        List<V3xOrgMember> members = GovdocRoleHelper.getDepartmentExchangeUsers(bean.getExchangeOrgId());
	                        Map<String,Object> recUserMap = getAgentExchangeUserName(members);
	                        recUserName = String.valueOf(recUserMap.get("recUserName"));
	                        recUserId = Long.parseLong(String.valueOf(recUserMap.get("recUserId")));
	                    }
			...
			if(EdocHelper.isG6Version()) {
				String isOpenRegister = EdocSwitchHelper.isOpenRegister(exchangeAccountId);
				mav.addObject("isAutoRegister", isOpenRegister.equals("1") ? "false":"true");
				//2015年8月28日 修改isOpenRegister判断  简化公文交换环节，如果合并签收、登记、分发操作，则不显示分发人 
		...
		mav.addObject("isOpenByToReceive", request.getParameter("isOpenByToReceive"));
		return mav;
	}
		...
		try {
				...
				if(!CollectionUtils.isEmpty(agentIdList)) {
					...
					for(int i=0; i<agentIdList.size(); i++) {
						if(null != orgAccountId){
							agentIsExchangeRole = GovdocRoleHelper.isExchangeRole(agentIdList.get(i), orgAccountId);
						}else{
							agentIsExchangeRole = GovdocRoleHelper.isExchangeRole(agentIdList.get(i));
						}
						if(agentIsExchangeRole) {
							break;
						}
			...
			if (null != record
				...
				if(!isExchangeRole) {
					...
					params.put("app", ApplicationCategoryEnum.edoc.key());
					params.put("subApp", ApplicationSubCategoryEnum.old_exSend.key());
		...
		if(affairforcheck !=null &&  Integer.valueOf(StateEnum.edoc_exchange_received.getKey()).equals(affairforcheck.getState())){
			try {
				...
				if(Strings.isNotBlank(alertNote)) {
					StringBuffer jsBuffer = new StringBuffer();
					jsBuffer.append("if(parent.doEndSign_exchange) {");
					jsBuffer.append("	parent.doEndSign_exchange('"+alertNote+"','','"+modelType+"');");
					jsBuffer.append("} else {");
	...
	public ModelAndView receiveDetail(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		if(Strings.isNotBlank(affairIdStr)){
			...
			if(affair != null && SubStateEnum.col_pending_unRead.getKey() != affair.getSubState()){
		...
		if(Strings.isNotBlank(affairIdStr)){
			CtpAffair affair = affairManager.get(Long.valueOf(affairIdStr));
			boolean isTrue = WFComponentUtil.checkAgent(affair.getMemberId(), affair.getSubject(),ModuleType.edoc, true, request);
			if(!isTrue) {
				if(!GovdocRoleHelper.checkPishi(affair.getMemberId())) {
					return null;
				}
			}
			checkAclMemberId = affair.getMemberId();
		}
		boolean falg = GovdocRoleHelper.isExchangeRole(checkAclMemberId);
		if(!falg){
			...
			if(_agentModelList!=null && _agentModelList.size()>0){
				...
				for(AgentModel agent : _agentModelList){
						...
						Long accountIdOfagentId = agentMember.getOrgAccountId();
						agentIsExchangeRole = GovdocRoleHelper.isExchangeRole(Long.valueOf(agentMemberId), accountIdOfagentId);
			...
			List<Long> _pishiModelList = govdocPishiManager.getEdocUserLeaderId(user.getId());//我代录谁
			if(!falg && _pishiModelList != null && _pishiModelList.size()>0){
				boolean agentIsExchangeRole = false;
				for(Long leaderId : _pishiModelList){
					V3xOrgMember agentMember=orgManager.getMemberById(leaderId);
					Long accountIdOfagentId = agentMember.getOrgAccountId();
					agentIsExchangeRole = GovdocRoleHelper.isExchangeRole(leaderId,accountIdOfagentId);
					if(agentIsExchangeRole){
						falg = true;
						break;
					}
		...
		if(!falg){
			...
			if(user.getLoginAccount().longValue() != user.getAccountId().longValue()) {
				falg = GovdocRoleHelper.isExchangeRole(user.getId(),user.getAccountId());
			}
		}
		
		// 判断当前人是否有处理公文所在单位的公文交换权限
		if (!falg && null != record) {
			if (record.getExchangeType() == 1) {
				falg = GovdocRoleHelper.isExchangeRole(user.getId(), Long.valueOf(record.getExchangeOrgId()));
			} else if (record.getExchangeType() == 2) {
				long exchangeOrgId = record.getExchangeOrgId();
				V3xOrgDepartment v3xOrg = this.orgManager.getDepartmentById(exchangeOrgId);
				falg = GovdocRoleHelper.isExchangeRole(user.getId(), v3xOrg.getOrgAccountId());
			}
		}
		//判断当前人是否还有公文交换的权限，如果没有，点击历史消息不可以进行发送和签收处理，包括回退后的数据
		if(falg){
			boolean isCanDeal = true;
			boolean isFromMessageOpen = false;
			String _isModalDialog = request.getParameter("_isModalDialog");
			if("true".equals(_isModalDialog)){
				isCanDeal = false;
				isFromMessageOpen = true;
			}
		    ...
		    if(!checkEdocRecieveRecord(response, record,false,isCanDeal,isFromMessageOpen)) { 
                return null;
            }
...
//	@CheckRoleAccess(roleTypes={Role_NAME.Accountexchange,Role_NAME.Departmentexchange})
	public ModelAndView stepBack(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			out.println("<script>");
			if (f.booleanValue()) {
				out.println("parent.doEndSign_exchange('','','toReceive');"); 
			} else {
				out.println("window.getA8Top().close();");
			}
			out.println("</script>");
	...
	public ModelAndView takeBack(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (null != affairList && affairList.size() > 0) {
			...
			affairManager.update(columns, new Object[][] {
					{ "app", ApplicationCategoryEnum.edoc.key() },
					{ "subApp", ApplicationSubCategoryEnum.old_exSign.key() },
	...
	public boolean checkEdocSendRecord(HttpServletResponse response, EdocSendRecord record, boolean isDialogArguments, boolean isLock) {
		try {
			...
			if(Strings.isBlank(alertNote) && isLock) {
				Long lockUserId = edocLockManager.canGetLock(record != null ? record.getId() : null, AppContext.currentUserId());
			...
			if(Strings.isNotBlank(alertNote)) {
				StringBuffer jsBuffer = new StringBuffer();
				jsBuffer.append("if(parent.doEndSign_exchange) {");
				jsBuffer.append("	parent.doEndSign_exchange('"+alertNote+"','', 'toSend');");
				jsBuffer.append("} else {");
				jsBuffer.append(	doEndSign_exchange_Java(alertNote));
				jsBuffer.append("}");
				super.rendJavaScript(response, jsBuffer.toString());
...
//				}
				jsBuffer.append("if(parent.doEndSign_exchange) {");
				jsBuffer.append("	parent.doEndSign_exchange('"+alertNote+"', '', 'toReceive');");
				jsBuffer.append("} else {");
