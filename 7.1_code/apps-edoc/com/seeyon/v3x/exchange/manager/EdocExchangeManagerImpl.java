...
public class EdocExchangeManagerImpl implements EdocExchangeManager {
	...
	public boolean isEdocCreateRole(Long userId,Long exchangeAccountId,String resCode){
	    ...
	    boolean ret=false; 
	    try {
	    	if("F07_recListFenfaing".equals(resCode) || "F07_recListRegistering".equals(resCode)) {//兼容新公文登记角色
	    		resCode = GovdocResCodeEnum.F20_newDengji.name();
	    	}
	...
	public String checkExchangeRole(String typeAndIds)
	{
		...
		try
		{
			...
			for(String item:items)
			{
				...
				{
					roles=GovdocRoleHelper.getAccountRecExchangeUsers(Long.parseLong(data[1]));//单位公文收文员					
				}
				else if(V3xOrgEntity.ORGENT_TYPE_DEPARTMENT.equals(data[0]))
				{
					tempEntity=orgManager.getEntity(item);
					roles=GovdocRoleHelper.getDepartmentRecExchangeUsers(tempEntity.getId());//部门公文收文员				
				}
				else
				{	
					tempEntity=null;
	...
	public void recEdoc(long id, long recUserId, long registerUserId, String recNo, String remark,String keepPeriod,Long agentToId) throws Exception {
		...
		if (edocRecieveRecord.getFromInternal()) {
			this.replyEdoc(Long.valueOf(replyId), remark, recUserName,edocRecieveRecord.getRecNo(),agentToId);
		} else { // 来自外部系统的公文
	...
	public List<EdocSendRecord> getSendEdocs(long userId, long orgId, int status) throws Exception{
		...
		String depIds=null;
		accountIds=GovdocRoleHelper.getUserExchangeAccountIds();
...
//		}
		...
		conditions.put("subApp", ApplicationSubCategoryEnum.old_edocRecDistribute.key());
		...
		if (null != affairList && affairList.size() > 0) {
			...
			if (oldDistributeUserId.equals(af.getMemberId())&& !af.isDelete()) {
				...
				if(register != null && register.getRegisterBody() != null) {
					reAffair.setBodyType(String.valueOf(GovdocUtil.getContentType(register.getRegisterBody().getContentType())));
				}
				...
				MessageReceiver receiver = new MessageReceiver(reAffair.getId(), reAffair.getMemberId(), url, com.seeyon.ctp.common.usermessage.Constants.LinkOpenType.href, String.valueOf(EdocEnum.edocType.recEdoc.ordinal()),String.valueOf(register.getId()),"");
				MessageContent messageContent = new MessageContent(key,reAffair.getSubject(), changeOperUserName);
				messageContent.setResource("com.seeyon.v3x.edoc.resources.i18n.EdocResource");
				userMessageManager.sendSystemMessage(messageContent, ApplicationCategoryEnum.edocRegister, Long.parseLong(changeOperUserID), receiver,EdocMessageFilterParamEnum.exchange.key);
			}
		}
	...
	public boolean changeRegisterEdocPerson(String edocRecieveRecordId,
		...
		Map<String, Object> conditions = new HashMap<String, Object>();
		conditions.put("app", ApplicationCategoryEnum.edoc.key());
		conditions.put("subApp", ApplicationSubCategoryEnum.old_edocRegister.getKey());
		...
		if (null != affairList && affairList.size() > 0) {
			...
			String bodyType = String.valueOf(MainbodyType.HTML.getKey());
			if(register != null && register.getRegisterBody()!=null) { 
				bodyType = register.getRegisterBody().getContentType();
			} else {
				if(edocRecieveRecord.getEdocId() != 0 && edocRecieveRecord.getEdocId() != -1) {
					EdocSummary summary = edocSummaryManager.getEdocSummaryById(edocRecieveRecord.getEdocId(), false, false);
					if(summary != null) {
			...
			reAffair.setBodyType(String.valueOf(GovdocUtil.getContentType(bodyType)));
	...
	public void stepBackRegisterEdoc(EdocRegister edocRegister, long id, long referenceAffairId, String stepBackInfo,String competitionAction) throws Exception {
		...
		if(!"no".equals(competitionAction)){
			if(edocRecieveRecord.getExchangeType() == EdocRecieveRecord.Exchange_Receive_iAccountType_Org){
				List<V3xOrgMember> members = GovdocRoleHelper.getAccountSendExchangeUsers(edocRecieveRecord.getExchangeOrgId());
				...
				List<V3xOrgMember> members = GovdocRoleHelper.getDepartmentSendExchangeUsers(dep.getId());
				membersToMessageReceiver(members,referenceAffairId,receivers);
			}
		}else{
			//发出消息，被回退（只发给签收人）	
...
//			edocRegisterManager.updateEdocRegisterState(edocRegister.getId(), EdocNavigationEnum.RegisterState.Register_StepBacked.ordinal());
		...
		if(edocRegister != null && edocRegister.getRegisterBody()!=null) { 
			bodyType = edocRegister.getRegisterBody().getContentType();
		} else {
			if(edocRecieveRecord.getEdocId() != 0 && edocRecieveRecord.getEdocId() != -1) {
				EdocSummary summary = edocSummaryManager.getEdocSummaryById(edocRecieveRecord.getEdocId(), false, false);
				if(summary != null) {
		...
		if("no".equals(competitionAction)){
			...
			affair.setSubApp(ApplicationSubCategoryEnum.old_exSign.getKey());
			affair.setNodePolicy(OldExchangeNodePolicyEnum.oldqianshou.name());
			affair.setBodyType(String.valueOf(GovdocUtil.getContentType(bodyType)));
			...
			Map<String, Object> extParam = GovdocAffairHelper.createExtParam(edocRecieveRecord.getDocMark(), edocRecieveRecord.getSendUnit(), null, edocRecieveRecord.getRegisterUserId());
	...
	public void stepBackRecievedEdoc(long id, long referenceAffairId, String stepBackInfo) throws Exception {
		...
		if (edocSendDetail != null && edocSendDetail.getRecOrgType() == 1) {
			// 内部单位
			member = GovdocRoleHelper.getAccountRecExchangeUsers(user.getLoginAccount());
		} else if (edocSendDetail != null && edocSendDetail.getRecOrgType() == 2) {
			// 内部部门
			member = GovdocRoleHelper.getDepartmentRecExchangeUsers(user.getDepartmentId());
		}
		...
		for (V3xOrgMember m : member) {
			...
			affair.setApp(ApplicationCategoryEnum.edoc.getKey());
			affair.setSubApp(ApplicationSubCategoryEnum.old_exSign.getKey());
			affair.setNodePolicy(OldExchangeNodePolicyEnum.oldfasong.name());
			affair.setBodyType(String.valueOf(GovdocUtil.getContentType(bodyType)));
			affair.setCreateDate(new Timestamp(System.currentTimeMillis()));
			affair.setReceiveTime(new Timestamp(System.currentTimeMillis()));
			affair.setSubject(edocRecieveRecord.getSubject());
	...
	public void takeBackEdoc(Long edocId,
		...
		// 取得待取回公文的recode
		EdocSendRecord edocSendRecord = sendEdocManager.getEdocSendRecord(edocSendDetail != null ? edocSendDetail.getSendRecordId() : null);
		...
		affair.setIdIfNew();
		affair.setApp(ApplicationCategoryEnum.edoc.getKey());
		affair.setSubApp(ApplicationSubCategoryEnum.old_exSend.getKey());
		affair.setNodePolicy(GovdocEnum.OldExchangeNodePolicyEnum.oldfasong.name());
		affair.setBodyType(String.valueOf(GovdocUtil.getContentType(stepBackEdocSummary.getBodyType())));
		affair.setCreateDate(new Timestamp(System.currentTimeMillis()));
		affair.setReceiveTime(new Timestamp(System.currentTimeMillis()));
		...
		Map<String, Object> extParam = GovdocAffairHelper.createExtParam(stepBackEdocSummary);
		AffairUtil.setExtProperty(affair, extParam);
	...
	public boolean stepBackEdoc(Long stepBackEdocId, String stepBackInfo, boolean oneself) throws BusinessException {
		...
		if (null != affairList && affairList.size() > 0) {
			...
			wheres[0][0] = "app"; wheres[0][1] = ApplicationCategoryEnum.edoc.key();
			wheres[1][0] = "subApp"; wheres[1][1] = ApplicationSubCategoryEnum.old_exSign.key();
			wheres[2][0] = "objectId"; wheres[2][1] = edocRecieveRecord.getEdocId();
			wheres[3][0] = "subObjectId"; wheres[3][1] = edocRecieveRecord.getId();
		...
		if(Strings.isNotEmpty(memberList)) {
			...
			for(V3xOrgMember v3xOrgMember : memberList) {
				...
				if(newAffair == null) {
					...
					newAffair.setApp(ApplicationCategoryEnum.edoc.getKey());
					newAffair.setSubApp(ApplicationSubCategoryEnum.old_exSend.getKey());
					newAffair.setNodePolicy(GovdocEnum.OldExchangeNodePolicyEnum.oldfasong.name());
					newAffair.setBodyType(String.valueOf(GovdocUtil.getContentType(summary.getBodyType())));
	...
	public String withdraw(String sendRecordId, String detailId, String accountId, String sendCancelInfo) throws BusinessException {
		try {
			...
			if(sendRecord != null) {
				...
				/** 5、生成待发送新数据 */
				CtpAffair cloneAffair = null;
				List<CtpAffair> sendAffairList = affairManager.getAffairs(ApplicationCategoryEnum.edoc, ApplicationSubCategoryEnum.old_exSend, newSendRecord.getEdocId());
				...
				if(newAffair == null) {
					...
					Map<String, Object> extParam = EdocUtil.createExtParam(newSendRecord.getDocMark(),  newSendRecord.getSendUnit(), summary.getSendUnitId());
					AffairUtil.setExtProperty(newAffair, extParam);
				}
				newAffair.setId(UUIDLong.longUUID());
				newAffair.setDelete(Boolean.FALSE);
				newAffair.setState(StateEnum.edoc_exchange_send.key());
	...
	public String[] checkEdocSendMember(String recieveRecordId) throws BusinessException {
		...
		if(edocSendRecord.getExchangeType() == EdocSendRecord.Exchange_Send_iExchangeType_Dept) {//部门收发员
			boolean flag = GovdocRoleHelper.isDepartmentExchange(edocSendRecord.getSendUserId(), edocSendRecord.getExchangeOrgId());
			strs[0] = String.valueOf(flag);
			if(!flag) {
				String hasMember = GovdocRoleHelper.hasDepartmentRecExchangeMember(edocSendRecord.getExchangeOrgId());
				strs[1] = hasMember;
			}
		} else if(edocSendRecord.getExchangeType() == EdocSendRecord.Exchange_Send_iExchangeType_Org) {//内部收发员
			boolean flag = GovdocRoleHelper.isAccountExchange(edocSendRecord.getSendUserId(), edocSendRecord.getExchangeOrgId());
			strs[0] = String.valueOf(flag);
			if(!flag) {
				String hasMember = GovdocRoleHelper.hasAccountRecExchangeMember(edocSendRecord.getExchangeOrgId());
				strs[1] = hasMember;
			}
		}
		return strs;
	...
	public String[] checkEdocRecieveMember(String recieveRecordId) throws BusinessException {
		...
		if(edocRecieveRecord.getExchangeType() == EdocRecieveRecord.Exchange_Receive_iAccountType_Dept) {//部门收发员
			boolean flag = GovdocRoleHelper.isDepartmentExchange(edocRecieveRecord.getRecUserId(), edocRecieveRecord.getExchangeOrgId());
			strs[0] = String.valueOf(flag);
			if(!flag) {
				String hasMember = GovdocRoleHelper.hasDepartmentRecExchangeMember(edocRecieveRecord.getExchangeOrgId());
				strs[1] = hasMember;
			}
		} else if(edocRecieveRecord.getExchangeType() == EdocRecieveRecord.Exchange_Receive_iAccountType_Org) {//内部收发员
			boolean flag = GovdocRoleHelper.isAccountExchange(edocRecieveRecord.getRecUserId(), edocRecieveRecord.getExchangeOrgId());
			strs[0] = String.valueOf(flag);
			if(!flag) {
				String hasMember = GovdocRoleHelper.hasAccountRecExchangeMember(edocRecieveRecord.getExchangeOrgId());
				strs[1] = hasMember;
			}
		}
		return strs;
	...
	public String cuiban(String detailIdStr,String cuibanInfo)throws BusinessException{
	    ...
	    try{
	        ...
	        List<CtpAffair> affairs = affairManager.getAffairs(ApplicationCategoryEnum.edoc, ApplicationSubCategoryEnum.old_exSign, send.getEdocId());
	...
	private void copyRecieve2EdocSummary(Long recieveId) {
		...
		if(edocRecieveRecord != null){
			...
			edocSummary.setId(edocRecieveRecord.getId());
			edocSummary.setSubject(edocRecieveRecord.getSubject());
			edocSummary.setEdocType(99);
			edocSummary.setDeadline(-1L);
			edocSummary.setHasArchive(false);
			edocSummary.setGovdocType(0);
			//edocSummary.setState(edocRecieveRecord.getStatus());
			edocSummary.setState(EdocConstant.flowState.oldexchange.ordinal());
			edocSummary.setDocMark(edocRecieveRecord.getDocMark());
			edocSummary.setSecretLevel(edocRecieveRecord.getSecretLevel());
			edocSummary.setUrgentLevel(edocRecieveRecord.getUrgentLevel());
			edocSummary.setCreateTime(edocRecieveRecord.getCreateTime());
			edocSummary.setCreatePerson(edocRecieveRecord.getSender());
			edocSummary.setStartUserId(edocRecieveRecord.getRecUserId());
			edocSummary.setOrgAccountId(edocRecieveRecord.getExchangeOrgId());
