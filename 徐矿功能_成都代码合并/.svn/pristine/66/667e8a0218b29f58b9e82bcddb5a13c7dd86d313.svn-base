...
import com.seeyon.v3x.services.form.impl.FormServiceImpl;
...
public class AffairResource extends BaseResource {
    ...
    public Response countPendingAffairs(Map<String, Object> params) {
		...
		String showAll = ParamUtil.getString(params, "showAll");
		if ("true".equals(showAll) && Strings.isNotBlank(appKeys)) {
			appKeys += ",999";// 如果是999证明是前端传递的查询条件
			log.info("best:查询所有待办数量（公文和协同）开始，appKeys：" + appKeys);
		}
        ...
        try {
            ...
            map = getAffairManager().countPendingAffairs(memberIdL, appKeyList);
        } catch (Exception e) {
            String keys = Strings.join(",", appKeyList);
            log.error("获取用户当前待办的数量出错！memberId:" + memberId + ",appKeyList:" + keys, e);
            return ok("false");
        }
        return ok(map);
    }
...
//        wfContext.setBusinessData(WorkFlowEventListener.CtpAffairConstant, affair);
    ...
    public Response canDealWithForm(@QueryParam("formOperationId")Long formOperationId, @QueryParam("formAppId")Long formAppId, @QueryParam("userId") String userId,
        ...
        FormBean fb = getFormManager().getForm(formAppId);
        ...
        if(isExecuteFinished){
            ...
            bpmProcess= getWapi().getBPMProcessForM1(colSummary.getProcessId());
            if(bpmProcess != null){
