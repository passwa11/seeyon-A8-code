...
import com.seeyon.v3x.edoc.manager.EdocMarkLockManager;
...
import com.seeyon.v3x.edoc.manager.RecRelationHandlerByG6;
...
public class EdocController extends BaseController {
	...
	public ModelAndView entryManager(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        modelAndView.addObject("app", request.getParameter("app"));
        modelAndView.addObject("sub_app", request.getParameter("sub_app"));
        modelAndView.addObject("openFrom", request.getParameter("openFrom"));
        //2015年8月28日 修改isOpenRegister判断  简化公文交换环节，签收之后直接跳转新建页面  添加一个参数 xiex start
        modelAndView.addObject("toNewRegister", request.getParameter("toNewRegister"));
        //2015年8月28日 修改isOpenRegister判断  简化公文交换环节，签收之后直接跳转新建页面  添加一个参数 xiex end
...
/*                modelAndView = new ModelAndView("common/redirect");
        ...
        }else if(Strings.isNotBlank(toFrom) && ("newEdocRegister").equals(toFrom) && !EdocHelper.isG6Version()){/* xiangfan 添加修复在兼职单位没有登记权限，点击登记的消息框，页面无限跳转的错误 GOV-5018 Start*/
            boolean isEdocRegister = EdocRoleHelper.isEdocCreateRole(AppContext.getCurrentUser().getLoginAccount(), AppContext.getCurrentUser().getId(), EdocEnum.edocType.recEdoc.ordinal());
            if(!isEdocRegister){
                modelAndView = new ModelAndView("common/redirect");
                String msgKey="alert_not_edocregister";
        		//G6的登记是分发
        ...
        if("register".equals(comm)&&"1".equals(request.getParameter("edocType"))){
            ...
            if(Strings.isNotBlank(recieveId)){
                ...
                if(record == null || record.getStatus()==com.seeyon.v3x.exchange.util.Constants.C_iStatus_Retreat||
                    ...
                    if(record != null){
                    	szJs = "alert('"+ResourceUtil.getString("edoc.alert.flow.edocStepBack", record.getSubject())+"');";//公文《"+record.getSubject()+"》已经被退回。
                    }
                    szJs += "   history.back();";
        ...
        modelAndView.addObject("isFromHome", request.getParameter("isFromHome"));
        return modelAndView ;
	}
	...
	public ModelAndView fullEditor(HttpServletRequest request,
		...
		modelAndView.addObject("connotEdit", request.getParameter("connotEdit"));
		return modelAndView;
	...
	public ModelAndView recManager(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String isFromHome = request.getParameter("isFromHome");
		...
		if(Strings.isNotBlank(request.getParameter("objectId"))) {
			...
			}else if(summary != null && summary.getProcessType()==1){//办文
			  //取我的Affair，用于访问控制权限校验
                List<CtpAffair> myAffairs = affairManager.getAffairs(ApplicationCategoryEnum.edoc, summary.getId(), user.getId());
                ...
                if(affair != null && affair.getSubState() != null && affair.getSubState().intValue() == SubStateEnum.col_pending_ZCDB.getKey()){
                    toFrom = "listZcdb";
                }
			}
		}
		...
		if(Strings.isNotBlank(isFromHome)) {
			url.append("&isFromHome="+isFromHome);
		}
		...
		if(Strings.isNotBlank(request.getParameter("sub_app"))) {
			url.append("&sub_app="+request.getParameter("sub_app"));
		}
		...
		String listType = Strings.isBlank(request.getParameter("listType"))?"listPending":request.getParameter("listType");
		if(Strings.isNotBlank(from)) {
			listType = from;
		}
		url.append("&listType="+listType);
		if(Strings.isNotBlank(request.getParameter("subState")) && request.getParameter("subState").equals(String.valueOf(SubStateEnum.col_pending_specialBacked.getKey()))){
			url.append("&backBoxToEdit=true");
		}
		...
		//2015年8月28日 修改isOpenRegister判断  简化公文交换环节，签收之后直接跳转新建页面  添加一个参数 xiex start
		String toNewRegister = request.getParameter("toNewRegister");
		if (Strings.isNotBlank(toNewRegister)) {
			url.append("&toNewRegister="+toNewRegister);
		}
		if (Strings.isNotBlank(request.getParameter("_resourceCode"))) {
			url.append("&_resourceCode="+request.getParameter("_resourceCode"));
		}
	...
	public ModelAndView sendManager(HttpServletRequest request,
		...
		String edocId = request.getParameter("edocId");
		String summaryId = Strings.isBlank(request.getParameter("summaryId"))?"":(request.getParameter("summaryId"));
		...
		return super.redirectModelAndView("/edocController.do?method=listIndex&comm="+comm+"&from="+from+"&toFrom="+toFrom+"&track="+track+"&controller=edocController.do&edocType="+EdocEnum.edocType.sendEdoc.ordinal()
				...
				+"&edocId="+edocId+"&transmitSendNewEdocId="+transmitSendNewEdocId+(Strings.isBlank(sub_app)?"":"&sub_app="+sub_app)+(Strings.isBlank(app)?"":"&app="+app)
				+(Strings.isBlank(modelType)?"":"&modelType="+modelType)+"&summaryId="+summaryId+"&listType="+listType+"&backBoxToEdit="+backBoxToEdit
				+(Strings.isBlank(openFrom)?"":"&openFrom="+openFrom)
				+(Strings.isBlank(request.getParameter("_resourceCode"))?"":"&_resourceCode="+request.getParameter("_resourceCode")));
	}
	...
	public ModelAndView signReport(HttpServletRequest request,
		...
		String openFrom = Strings.isBlank(request.getParameter("openFrom"))?"":request.getParameter("openFrom");
		return super.redirectModelAndView("/edocController.do?method=listIndex&from="+from+"&toFrom="+toFrom+"&track="+track+"&controller=edocController.do&edocType="+EdocEnum.edocType.signReport.ordinal()+"&templeteId="+templeteId+"&affairId="+affairId+"&summaryId="+summaryId+"&listType="+listType+"&backBoxToEdit="+backBoxToEdit+"&openFrom="+openFrom+"&_resourceCode=" + request.getParameter("_resourceCode"));
	}
    ...
    public ModelAndView newEdoc(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        //陈枭 -- 2015-9-10
        String fileType = QwqpUtil.getNowFileType();
        modelAndView.addObject("fileType",fileType);
        //
        String register = request.getParameter("register");
        String meetingSummaryId = request.getParameter("meetingSummaryId");//会议纪要转公文 获得会议纪要ID
        String comm = request.getParameter("comm");
        ...
        modelAndView.addObject("app", request.getParameter("app"));
        modelAndView.addObject("sub_app", request.getParameter("sub_app"));
        modelAndView.addObject("canSelfCreateFlow", EdocSwitchHelper.canSelfCreateFlow());//是否允许自建流程
		// 发文、收文、签报各自设置默认节点权限
        ...
        //判断是否是从首页进入的，用于提交处理之后刷新页面
        String isFromHome = request.getParameter("isFromHome");
        modelAndView.addObject("isFromHome",isFromHome);
        /**
         * 参数定义
         */
         ...
         */
        	...
        	modelAndView.addObject("processId",ctpTemplate.getWorkflowId());
        	 WorkflowApiManager wapi  = (WorkflowApiManager) AppContext.getBean("wapi");
        	 if(ctpTemplate.getWorkflowId() == null){
        		 modelAndView.addObject("process_xml","");
        	 }else{
        		 String process_xml = wapi.selectWrokFlowTemplateXml(String.valueOf(ctpTemplate.getWorkflowId().longValue()));
         ...
         */
        	...
        	//默认模板设置  dy 2015-9-15
        	User user = AppContext.getCurrentUser();
            Long department = user.getLoginAccount();
            Long templateType1 = null;
            ...
            if(AppContext.hasPlugin("qwqp")) {
	            Long idDefault = qwqpManager.getExistDefaultTemplate(department, templateType1);
	            List<QwqpDefaultTemplate> list = qwqpManager.getDefaultTemplateById(idDefault);
	            if (list.size()>0){
	            	QwqpDefaultTemplate defaultTemplate = new QwqpDefaultTemplate();
	            	defaultTemplate = list.get(0);
	            	idTemplate = defaultTemplate.getTemplate();
	            	String templeteId1 =Long.toString(idTemplate);
	            	//权限判断
					boolean isTemplate=templeteManager.isTemplateEnabled(idTemplate, user.getId());
					if(isTemplate==true){
						modelAndView.addObject("templeteId1", templeteId1);
	            	}
	            }
            }
        ...
        //不是调用模板和待发编辑时
        if(modelAndView != null && StringUtils.isEmpty(templeteId)&&Strings.isBlank(s_summaryId)){
        ...
        String _trackValue = customizeManager.getCustomizeValue(AppContext.getCurrentUser().getId(),CustomizeConstants.TRACK_SEND);
        ...
        if(handle.getSummary() != null) {
        	modelAndView.addObject("NPSAppName", GovdocUtil.getNPSAppName(0, handle.getSummary().getEdocType()));
        }
        return modelAndView;
    }
	...
	public ModelAndView showTime(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Date begin = Datetimes.parseDatetime(time);
		String etime = "2013-12-29 00:00:00";
		Date end = Datetimes.parseDatetime(etime);
	...
	public ModelAndView newEdocRegister(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        long recieveId = (Strings.isBlank(request.getParameter("recieveId"))||"null".equals(request.getParameter("recieveId"))) ? -1 : Long.parseLong(request.getParameter("recieveId"));
        ...
        if("create".equals(comm)) {//新建
        	...
        	if(edocRegister == null && recieveId!=-1) {
        		edocRegister = edocRegisterManager.findRegisterByRecieveId(recieveId);
        	}
        	if(edocRegister != null && edocRegister.getRecieveId()!=-1 && recieveId == -1) {
        		recieveId = edocRegister.getRecieveId();
        	}
	        //签收单
        	if(recieveId != -1 && recieveId!=0) {
        		record = recieveEdocManager.getEdocRecieveRecord(recieveId);	
        	}
        	if(record == null && edocRegister==null && affair != null) {
				if(affair.getObjectId() != null) {
					edocRegister = edocRegisterManager.findRegisterById(affair.getObjectId());
				}

				if(edocRegister == null)
				{
					if(affair.getSubObjectId() != null) {
						record = recieveEdocManager.getEdocRecieveRecord(affair.getSubObjectId());
						if(record != null) {
							recieveId = record.getId();
						}
					}
				}

        	}
        	//G6 老公文保保存待登记的记录升级上来，正文没法获取到,需要获取edocRegister对应的正文
	        if(edocRegister != null && edocRegister.getRegisterBody() == null){
	        ...
	        if(registerUserId != 0 && registerUserId != user.getId()) {
	            ...
	            String pishiFalg = govdocPishiManager.checkLeaderPishi(user.getId(), registerUserId);
		            ...
		            if(user.getId() != agentId) {//如果当前用户与登记人的代理人不同，则表示公文登记人的代理人已转换
		                ...
		                if("agent".equals(request.getParameter("app"))) {
		                	...
		                	if(EdocRoleHelper.isEdocCreateRole(EdocEnum.edocType.recEdoc.ordinal())) {
		                	    sb.append("	parent.parent.location.href='edocController.do?method=listIndex&from=listRegister&edocType="+edocType+"';");
		                	} else {
		                	    sb.append("	parent.parent.location.href='edocController.do?method=listIndex&from=listPending&edocType="+edocType+"';");
		                	}
		                }
		                sb.append("}");
		                rendJavaScript(response, sb.toString());
	            ...
	            } else if(!"pishi".equals(pishiFalg)) {
	            	String errMsg = ResourceBundleUtil.getString("com.seeyon.v3x.edoc.resources.i18n.EdocResource", "alert_hasChanged_register");
	            	StringBuilder sb = new StringBuilder();
	            	sb.append("alert(\"" + errMsg + "\");");
	            	sb.append("if(window.dialogArguments){"); // 弹出
	        	...
	        	if(!EdocHelper.isG6Version() && !EdocRoleHelper.isEdocCreateRole(EdocEnum.edocType.recEdoc.ordinal())) {
	        ...
	        //如果公文回退了
	        if(record!=null
	        		&& (record.getStatus()==EdocRecieveRecord.Exchange_iStatus_Torecieve
	            ...
	            sb.append("}else{");
	            if(EdocRoleHelper.isEdocCreateRole(EdocEnum.edocType.recEdoc.ordinal())) {
	                sb.append("	parent.parent.location.href='edocController.do?method=listIndex&from=listRegister&edocType=1&listType=listPending';");
            	} else {
            	    sb.append("	parent.parent.location.href='edocController.do?method=listIndex&from=listPending&edocType=1&listType=listPending';");
            	}
	            sb.append("}");
	            sb.append("");
	            rendJavaScript(response, sb.toString());
	        ...
	        if(affair!=null) {
	        	...
	        	if(affair.getState() != StateEnum.col_pending.key()) {
			        ...
			        if (record!=null && record.getStatus() == EdocRecieveRecord.Exchange_iStatus_Registered && edocRegister.getState().intValue()==2) {
			        	hasRegisted = true;
			        }
	        	}
	        } else {
	        ...
	        if(hasRegisted) {
	            ...
	            if(("agent".equals(request.getParameter("app")) || user.getId().equals(agentMember)) && user.getId()!=record.getRegisterUserId()) {//代理人跳转到代理事项
	                sb.append("parent.parent.parent.location.href='collaboration/pending.do?method=morePending&from=Agent';");
	            } else {
	            	//String isOpenFromHome = request.getParameter("isFromHome");//该参数暂时不用在跳转页面
	            	if(user.hasResourceCode(GovdocResCodeEnum.F20_govdocPending.name())) {//列表权限
            			sb.append("window.location.href = 'govdoc/govdoc.do?method=index&listType=listPendingAllRoot&_resourceCode=F20_govdocPending';");
                	}else if(user.hasResourceCode(GovdocResCodeEnum.F20_receiveManage.name())) {//列表权限
                		sb.append("window.location.href = 'govdoc/govdoc.do?method=index&listType=listPending&govdocType=2,4&_resourceCode=F20_receiveManage';");
                	}else {
                		sb.append("parent.window.location.href = parent.window.location.href;");
                	}
	            }
	            sb.append("}");
	            sb.append("");
	        ...
	        if(edocRegister.isNew()) {
		      ...
		      /*  if(agentToId != -1){
		       ...
		       // }
		        edocRegister.setCreateTime(new Timestamp(new Date().getTime()));
		        edocRegister.setUpdateTime(new Timestamp(new Date().getTime()));
	
		        Long exchangeOrgId = null;
		        if(record == null){
		            exchangeOrgId = user.getLoginAccount();
		        }else {
		            exchangeOrgId = ExchangeUtil.getAccountIdOfRegisterByOrgIdAndOrgType(record.getExchangeOrgId(), record.getExchangeType());
	            }
	
		        edocRegister.setOrgAccountId(exchangeOrgId);
	
		        edocRegister.setState(RegisterState.WaitRegister.ordinal());
		        //设置登记人
		        if(registerUserId == 0){
		       ...
		       if(registerUserId != user.getId()) {
		        ...
		        if(record!=null){//第一次登记时从签收单获取信息，非新建则直接获取登记单中的信息
		        ...
		        //G6 升级后 老公文分发的待登记数据，进行保存待发后查看附件没有读取到
		        if(attachmentListCopy.size()<=0){
		        	attachmentListCopy = attachmentManager.getByReference(edocRegister.getId(), edocRegister.getId());
		        }
	        ...
	        } else {//end if(edocRegister.isNew())
	        	if(Strings.isNotBlank(edocRegister.getSerialNo())) {
	    			...
	    			if(emTemp==null) {
	    				emTemp = GovdocMarkHelper.parseMarkstrOld(edocRegister.getSerialNo(), "1");
	    				if(emTemp != null) {
	    					emTemp.setWordNo(emTemp.toString());
	    				}
	        ...
	        if(edocRegister.getRegisterBody() == null){
		        //装载公文正文
		        ...
		        }else if(summary ==null && record!=null) {//G6 老公文升级上来直接为待登记的记录，正文没法获取到,需要获取原发文的
		        	EdocSummary sendSummary = edocManager.getEdocSummaryById(record.getEdocId(), true);
		        	if(sendSummary != null){
			        	EdocBody edocBody = sendSummary.getBody(record.getContentNo().intValue());
			        	registerBody = new RegisterBody();
			        	edocBody = edocBody==null?sendSummary.getFirstBody() : edocBody;
						if(null != edocBody){
							registerBody.setId(-1L);
							registerBody.setContent(edocBody.getContent());
							registerBody.setContentNo(edocBody.getContentNo());
        	...
        	if(edocRegister == null) {//该登记不存在
                ...
                sb.append("   window.close();");
                sb.append("} else {");
                sb.append("   parent.location.href='edocController.do?method=edocFrame&from=listRegister&edocType="+edocType+"';");
                sb.append("}");
                rendJavaScript(response, sb.toString());
                return null;
	        }
        ...
        String deptIds = orgManager.getUserIDDomain(user.getId(),user.getLoginAccount(), V3xOrgEntity.ORGENT_TYPE_DEPARTMENT, V3xOrgEntity.ORGENT_TYPE_ACCOUNT);
        ...
        boolean canUpdateContent = (edocRegister.getRegisterType() != null && (edocRegister.getRegisterType()==2||edocRegister.getRegisterType()==3)) ? true : EdocSwitchHelper.canUpdateAtOutRegist();;//是否能修改正文
        ...
        //待登记数据保存待发后，再次处理判断是否从首页进入
        modelAndView.addObject("fromHome",request.getParameter("fromHome"));
        modelAndView.addObject("appType", EdocUtil.getSubAppCategoryByEdocType(iEdocType).getKey());
        return modelAndView;
    }
	...
	public ModelAndView saveRegister(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String comm = Strings.isBlank(request.getParameter("comm"))? "create" : request.getParameter("comm");//新建 /修改
        String listType = Strings.isBlank(request.getParameter("recListType"))? "listRegister" : request.getParameter("recListType");//新建 /修改
        ...
        long affairId = Strings.isBlank(request.getParameter("affairId"))? -1 : Long.parseLong(request.getParameter("affairId"));//当前待登记的事项ID
        User user = AppContext.getCurrentUser();
        String registerState = request.getParameter("state");//0 保存待发  2 发送
        ...
        //String isOpenFromHome = request.getParameter("isFromHome");//该参数暂时不用在跳转页面
        //保存待发后 跳转到待办列表
        if("2".equals(registerState)) {//发送
        	if(user.hasResourceCode(GovdocResCodeEnum.F20_govdocPending.name())) {//列表权限
        		refreshDraftScript.append("window.location.href = 'govdoc/govdoc.do?method=index&listType=listPendingAllRoot&_resourceCode=F20_govdocPending';");
        	}else if(user.hasResourceCode(GovdocResCodeEnum.F20_receiveManage.name())) {//列表权限
        		refreshDraftScript.append("window.location.href = 'govdoc/govdoc.do?method=index&listType=listPending&govdocType=2,4&_resourceCode=F20_receiveManage';");
        	}else {
        		refreshDraftScript.append("parent.window.location.href = parent.window.location.href;");
        	}
    	} else {//保存待发
    		if(user.hasResourceCode(GovdocResCodeEnum.F20_govdocPending.name())) {//列表权限
        		refreshDraftScript.append("window.location.href = 'govdoc/govdoc.do?method=index&listType=listPendingAllRoot&_resourceCode=F20_govdocPending';");
       ...
       //权限与状态验证
        ...
        if(tempState != null && tempState == RegisterState.Registed.ordinal()) {//状态为已登记
        	edocRegister.setIsRetreat(RegisterRetreatState.NotRetreat.ordinal());//非退回
        	edocRegister.setDistributeState(EdocNavigationEnum.EdocDistributeState.WaitDistribute.ordinal());
        }
        /*serialNo = edocMarkManager.registDocMark(edocRegister.getId(), serialNo, 3, 1, false, EdocEnum.MarkType.edocInMark.ordinal());
...
*/
        ...
        baseVo.setRegister(edocRegister);
        baseVo.setCurrentUser(user);
        baseVo.setCurrentDate(new Date());
        if("2".equals(registerState)) {//发送
        	govdocMarkManager.saveRegisterSendMarkOld(baseVo);
        } else {
        	govdocMarkManager.saveRegisterDraftMarkOld(baseVo);
        }
        edocRegister = baseVo.getRegister();
        ...
        if("create".equals(comm) && oldRegister == null) {
            ...
            //电子公文 (为1)
            if(edocRegister.getRegisterType() != null && edocRegister.getRegisterType() == EdocNavigationEnum.RegisterType.ByAutomatic.ordinal()) {
            	if(edocRecieveRecord != null){
        	...
        	//更新edocsummary表数据
        	EdocSummary summary = edocSummaryManager.getEdocSummaryById(edocRegister.getId(), false, false);
        	copyData(summary,edocRegister);
            edocSummaryManager.saveOrUpdateEdocSummary(summary);//不能保证老数据一定存在该edocsummary 所以用saveorupdate
        }
        //创建或者更新的时候 创建或更新edoc_summary 和 affair表中数据
        try{
        	saveAffairAndSummary4Register(edocRegister,registerState,affairId);
        } catch (Exception e) {
			LOGGER.error("老公文升级新公文  处理登记时插入数据异常 ："+e.getMessage());
		}
        
        //电子公文保存待发，修改affair的状态
        if("0".equals(registerState) && edocRegister.getRegisterType() != null && edocRegister.getRegisterType() == EdocNavigationEnum.RegisterType.ByAutomatic.ordinal()){
        ...
        if(edocRecieveRecord!=null) {
	        ...
	        edocRecieveRecord.setRegisterTime(new Timestamp(edocRegister.getRegisterDate().getTime()));
	        ...
	        if("2".equals(registerState)) {
		        if(edocRegister.getState() != null && edocRegister.getState() == RegisterState.Registed.ordinal()) {
		        	//去掉待登记
		        	Map<String, Object> params = new HashMap<String, Object>();
		        	params.put("app", ApplicationCategoryEnum.edoc.getKey());
		        	params.put("subApp", ApplicationSubCategoryEnum.old_edocRegister.getKey());
			        ...
			        if(affairList!=null) {
			        	for(int i=0; i<affairList.size(); i++) {
			        		...
			        		affairList.get(i).setFinish(true);
        ...
        //待分发数据回退后， 待分发的affair数据被置为删除状态，这一条edocRegister数据被再次登记，进入待分发状态，
        //affair状态没有该回来，在合并签收登记分发操作时，待办列表不能查询出来数据为删除状态的数据，
        //所以在这里就把affair的删除状态改回来。  xiex  2015年10月23日
        List<CtpAffair> affairs = affairManager.getAffairs(ApplicationCategoryEnum.edoc, ApplicationSubCategoryEnum.old_edocRecDistribute, edocRegister.getId());
        for (CtpAffair ctpAffair : affairs) {
			if (ctpAffair.getState() == 3 && ctpAffair.isDelete()) {
				ctpAffair.setDelete(false);
				affairManager.updateAffair(ctpAffair);
			}
		}
        if("2".equals(registerState)){
	        affairs = affairManager.getAffairs(ApplicationCategoryEnum.edoc, ApplicationSubCategoryEnum.old_edocRegister, edocRegister.getId());
	        if(Strings.isNotEmpty(affairs)) {
		        for (CtpAffair ctpAffair : affairs) {
					if (ctpAffair.getState() == 3) {
						//代理人登记
		        		if(!user.getId().equals(edocRegister.getRegisterUserId())){
		        			ctpAffair.setTransactorId(user.getId());
		        		}
						ctpAffair.setFinish(true);
						ctpAffair.setState(StateEnum.col_done.key());
						ctpAffair.setCompleteTime(new Timestamp(new Date().getTime()));
						affairManager.updateAffair(ctpAffair);
					}
				}
	        }
    ...
    private void copyData(EdocSummary edocSummary,EdocRegister edocRegister){
    	//复制部分信息到summary中
		edocSummary.setId(edocRegister.getId());
		edocSummary.setSubject(edocRegister.getSubject());
		edocSummary.setEdocType(99);
		edocSummary.setDeadline(-1L);
		edocSummary.setHasArchive(false);
		edocSummary.setGovdocType(0);
		//int state = edocRegister.getState();
		edocSummary.setState(EdocConstant.flowState.oldexchange.ordinal());
		edocSummary.setDocMark(edocRegister.getDocMark());
		edocSummary.setSecretLevel(edocRegister.getSecretLevel());
		edocSummary.setUrgentLevel(edocRegister.getUrgentLevel());
		edocSummary.setCreateTime(edocRegister.getCreateTime());
		edocSummary.setCreatePerson(edocRegister.getRegisterUserName());
		edocSummary.setStartUserId(edocRegister.getRegisterUserId());
		if(edocRegister.getRegisterUserId() == null || "".equals(edocRegister.getRegisterUserId())){
			edocSummary.setCreatePerson(edocRegister.getDistributer());
			edocSummary.setStartUserId(edocRegister.getDistributerId());
		}
		edocSummary.setOrgAccountId(edocRegister.getSendUnitId());
		//将 edoc_id  写入 edoc_summary中的caseid  保存 没实际意义，只是为了前台 打开以前界面参数
		//量receive_id  写入 edoc_summary中的processid保存 没实际意义，只是为了前台 打开以前界面参数
		edocSummary.setCaseId(edocRegister.getEdocId());
		edocSummary.setProcessId(edocRegister.getRecieveId()+"");
		edocSummary.setDocMark(edocRegister.getDocMark());
		edocSummary.setSerialNo(edocRegister.getSerialNo());
		edocSummary.setBodyType(String.valueOf(GovdocUtil.getContentType(edocRegister.getRegisterBody().getContentType())));
    }
	...
	private String checkValid(long registUserId, EdocRegister oldreRegister, EdocRecieveRecord edocRecieveRecord)
		...
		if (oldreRegister != null && oldreRegister.getState() != null) {// 已有登记数据
			...
			if (oldreRegister.getState() == RegisterState.deleted.ordinal()) {// 竞争执行看是否被删除了
				hasDelete = true;
			} else if (oldreRegister.getState() == RegisterState.Registed.ordinal()) {
			...
			} else if (oldreRegister.getState() == RegisterState.DraftBox.ordinal()) {
		...
		try {
			...
			if ("modify".equals(comm)) {
				...
				if (distributerId != oldDistributerId) {
					// 给分发人发送修改消息
					distributeAffair(subject, edocRegister.getDocMark(), edocRegister.getSendUnit(), edocRegister.getSendUnitId(), registerUserId, oldDistributerId, edocRegister.getId(),
							edocRegister.getDistributeEdocId(), "delete", importantLevel, bodyType);
					
					EdocSummary newsummary = new EdocSummary();
					copyData(newsummary, edocRegister);
					edocSummaryManager.deletePhysicalEdocSummary(newsummary.getId());
					edocSummaryManager.saveOrUpdateEdocSummary(newsummary);//不能保证老数据一定存在该edocsummary 所以用saveorupdate
					...
					distributeAffair(subject, edocRegister.getDocMark(), edocRegister.getSendUnit(), edocRegister.getSendUnitId(), registerUserId, distributerId, edocRegister.getId(),
							edocRegister.getDistributeEdocId(), "create", importantLevel, bodyType);
					
					newsummary = new EdocSummary();
					copyData(newsummary, edocRegister);
					edocSummaryManager.deletePhysicalEdocSummary(newsummary.getId());
					edocSummaryManager.saveOrUpdateEdocSummary(newsummary);//不能保证老数据一定存在该edocsummary 所以用saveorupdate
	...
	private void distributeAffair(String subject, String docMark, String sendUnit, Long sendUnitId, long senderId, long memberId, Long objectId, Long subObjectId,
			String comm, int importantLevel, String... bodyType) throws BusinessException {
		CtpAffair reAffair = null;
		if ("delete".equals(comm)) {
			affairManager.deleteByObjectId(ApplicationCategoryEnum.edoc, objectId);
		} else if ("create".equals(comm)) {// 这里注意，登记ojbectId, subState // 分发：subObjectId, state
			List<CtpAffair> affairs = affairManager.getAffairs(ApplicationCategoryEnum.edoc, ApplicationSubCategoryEnum.old_edocRecDistribute, objectId);
			if (Strings.isNotEmpty(affairs)) {
			...
			if (bodyType != null && bodyType.length == 1) {
				reAffair.setBodyType(String.valueOf(GovdocUtil.getContentType(bodyType[0])));
			}
			// reAffair.setApp(31);
			// wangjingjing end
			...
			if (registerList != null) {
				...
				for (EdocRegister edocRegister : registerList) {
					if (edocRegister.getRegisterType() != 1 && edocRegister.getRecieveId() == -1) {// 纸质/二维码登记公文允许撤销
						edocRegister.setState(RegisterState.DraftBox.ordinal());
						...
						wheres[0] = new Object[] { "app", ApplicationCategoryEnum.edoc.key() };
						wheres[1] = new Object[] { "subApp", ApplicationSubCategoryEnum.old_edocRegister.key() };
						wheres[2] = new Object[] { "objectId", edocRegister.getId() };
						wheres[3] = new Object[] { "state", StateEnum.col_pending.getKey() };
	...
	public ModelAndView doComment(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		senderOpinion.setOpinionType(OpinionType.senderOpinion.ordinal());
	...
	private Map<String, Object> checkSendRecEdoc(EdocRegister edocRegister, long registerId, String recieveIdStr,
		...
		// 电子收文
			...
			Long recordRegisterUserId = edocRegister != null ? edocRegister.getDistributerId():0L;
			...
			String pishiFlag = govdocPishiManager.checkLeaderPishi(user.getId(), recordRegisterUserId.longValue());
			if (agentId != null) {
			...
			if (recordRegisterUserId.longValue() != user.getId()) {
				agentToId = edocRegister !=null ? edocRegister.getDistributerId() : null;
				msgMap.put("agentToId", agentToId);
			}
			if (recordRegisterUserId.longValue() != user.getId() && isNotAgent && !"pishi".equals(pishiFlag)) {
				// 公文登记人已经转换
			...
			if(recordRegisterUserId.longValue() != user.getId() && isNotAgent && "pishi".equals(pishiFlag)){
				msgMap.put("pishiType", "true");
			}
			// 登记公文，判断指定的登记人是否可以登记此公文
			String errorMsg = canSendRecEdoc(edocRegister != null ? edocRegister.getOrgAccountId() : null, recordRegisterUserId, roleType, app);
			if (Strings.isNotBlank(errorMsg)) {
				msgMap.put("errMsg", errorMsg);
				return msgMap;
			}

			if (edocRegister != null && edocRegister.getIsRetreat() == 1) {
				// 公文已经回退
				String sb = createSendRecEdocErrMsg("alert_hasBeStepBack_already", app);
				msgMap.put("errMsg", sb);
				return msgMap;
			} else if (edocRegister != null && edocRegister.getDistributeState() == EdocNavigationEnum.EdocDistributeState.Distributed.ordinal()
...
//    	long starttime = System.currentTimeMillis();
		...
		Integer processType = GovdocProcessTypeEnum.govdocRecHandle.key();
		Boolean pishiFlag = false;
		if (processTypeStr != null && !"".equals(processTypeStr)) {
			processType = Integer.parseInt(processTypeStr);
		}
    	/**
    	 * 表单ID
    	 */
		...
		if("1".equals(edocType)){
			...
			Map<String,Object> msgMap = checkSendRecEdoc(edocRegister,registerId,recieveIdStr,request.getParameter("app"),user
		    		,backBoxToEdit);
			pishiFlag =  "true".equals(msgMap.get("pishiType"))? true : false;
			String errMessage = (String)msgMap.get("errMsg");
			//如果合并了签收登记分发操作，则不判断是否具有分发权  xiex
			if(Strings.isNotBlank(errMessage) && !"3".equals(EdocSwitchHelper.isOpenRegister())) {
				if("redirect".equals(errMessage)){
					ModelAndView modelAndView = new ModelAndView("common/redirect");
					String errMsg = ResourceBundleUtil.getString("com.seeyon.v3x.exchange.resources.i18n.ExchangeResource", "edoc.distributed");
					modelAndView.addObject("redirectURL", BaseController.REDIRECT_BACK);
					modelAndView.addObject("errMsg", errMsg);
					return modelAndView;
				}else{
        ...
        //兼容待发的老公文编辑后发送，老公文会将bodyType设置为“OfficeWord”样的字符串，清空
        //edocSummary.setBodyType(null);
        if(edocSummary.getGovdocType() == null) {
        	edocSummary.setGovdocType(0);
        }
        ...
        //交换类型
        int edocExchangeType = request.getParameter("edocExchangeType")==null?-1:Integer.parseInt(request.getParameter("edocExchangeType"));
        ...
        if(Strings.isNotBlank(templeteId)){
            ...
            if(null != _curTemplate && !_curTemplate.isSystem() && null == _curTemplate.getFormParentid()){
            	templeteManager.updateTempleteRecent(user.getId(), _curTemplate.getId());
        ...
        senderOninion.setOpinionType(OpinionType.senderOpinion.ordinal());
        ...
        edocSummary.setBodyType(String.valueOf(GovdocUtil.getContentType(tempStr)));		
...
//        }
        ...
        if(edocRegister != null && (!"HTML".equals(tempStr))) {
        	Long fileId = Long.parseLong(body.getContent());
        	V3XFile v3xfile = fileManager.getV3XFile(fileId);
        	if(v3xfile == null) {
        		LOGGER.info("该公文收发时无正文内容，需登记表中拷贝正文：registerId=" + edocRegister.getId() + " fileId=" + body.getContent());
        		RegisterBody registerBody = edocRegisterManager.findRegisterBodyByRegisterId(edocRegister.getId());
        		if(registerBody != null) {
        			try {
        				Long registerFileId = Long.parseLong(registerBody.getContent());
            			v3xfile = fileManager.clone(registerFileId);
            			if(v3xfile != null) {
            				fileManager.save(v3xfile);
            				body.setContent(String.valueOf(v3xfile.getId()));
            			}
            			LOGGER.info("该公文收发时无正文内容，拷贝正文成功。。。");
         ...
         */
        recieveIdStr = RecRelationHandlerFactory.getHandler().getRecieveIdBeforeSendRec(edocSummary, recieveIdStr, waitRegister_recieveId, isNewSent);
		...
		if(sendAffair != null){
			...
			if (subState != SubStateEnum.col_pending_specialBacked.getKey() && subState != SubStateEnum.col_pending_specialBackToSenderCancel.getKey()) {
	            //this.superviseManager.deleteRemindersLog(edocSummary.getId());//删除催办日志
				//TODO 老公文兼容
	        }
		}
		//----------------------  end  ----------------------
        ...
        if(isQuickSend){//快速发文后，summary的状态为结束。
        	edocSummary.setState(EdocConstant.flowState.finish.ordinal());
        	...
        	edocSummary.setState(EdocConstant.flowState.run.ordinal());
        }
        Timestamp nowTime = new Timestamp(System.currentTimeMillis());
        edocSummary.setCreateTime(nowTime);
        edocSummary.setStartTime(nowTime);
        //流程期限具体时间需要做版本控制
        ...
        if(!isNew && Strings.isNotBlank(hasSummaryId)){
            ...
            if(AppContext.hasPlugin("qwqp")) {
	            QwqpEdocSendFormRelation edocSendFormRelation = qwqpManager.existsSign(Long.parseLong(hasSummaryId));
	            String saveAip = request.getParameter("saveAip");
	            if(edocSendFormRelation!=null&&Strings.isNotBlank(saveAip)&&"false".equals(saveAip)){
	            	qwqpManager.deleteByEdocId(Long.parseLong(hasSummaryId));
	            }
            }
        }
        Long affairId = 0L;
       ...
       // edocInnerMarkDefinitionManager.getInnerMark(edocSummary.getEdocType(), user.getAccountId(), true);
        ...
        GovdocBaseVO baseVo = new GovdocBaseVO();
        baseVo.setSummary(edocSummary);
        baseVo.setRegister(edocRegister);
        baseVo.setCurrentDate(DateUtil.currentDate());
        baseVo.setCurrentUser(AppContext.getCurrentUser());
        if(edocSummary.getEdocType() == 0) {
        	baseVo.setQuickSend(edocSummary.getIsQuickSend());
        }
        baseVo.setNewBusiness(isNewSent ? "1" : "0");
        govdocMarkManager.saveSendMarkOld(baseVo);
        edocSummary = baseVo.getSummary();
        
        if (edocSummary != null && !edocSummary.isNew()) {
        	try {
        		edocSummaryManager.saveOrUpdateEdocSummary(edocSummary);
        	} catch(Exception e) {
    			LOGGER.error("保存公文号前保存公文出错", e);
    		}
        }

        if(edocSummary.getEdocType() == 1) {
        	govdocRegisterManager.saveBySummary(edocSummary);
        }
        
        String process_xml = request.getParameter("process_xml");
        String ssss = request.getParameter("process_info");
        String templeteProcessId= request.getParameter("templeteProcessId");
        try {
        	affairId = edocManager.runCase(edocSummary, body, senderOninion, sendType,
                 options,comm,agentToId,isNewSent,process_xml,workflowNodePeoplesInput,workflowNodeConditionInput,templeteProcessId);
        } catch (Exception e) {
        ...
        //this.saveColSupervise(request, response, edocSummary, isNew,true,affairId);

//        if("distribute".equals(comm) && recieveIdStr!=null && !"".equals(recieveIdStr))
//        {
//			if (isCanBeRegisted) {
//				Long exchangeId = Long.parseLong(recieveIdStr);
...
//        }
        ...
        if(edocSummary!=null && edocSummary.getEdocType()==1){
             ...
             */
			new RecRelationHandlerByG6().transAfterSendRec(param);
        }
        	...
        	try{
           	 ...
           	 if(edocSummary.getEdocType() == 0 || edocSummary.getEdocType() == 1){  //发文
                    if(edocSummary.getArchiveId()!= null  && !edocSummary.getHasArchive()){
                        ...
                        quickSendPigholeInfo = ResourceBundleUtil.getString("com.seeyon.v3x.edoc.resources.i18n.EdocResource","edoc.quickSend.pigholeInfo");
                    }
                }
           	//快速发文——归档。end
        	}catch(Exception e){
		...
		if("listReaded".equals(pageview)){
		    ...
		    sb.append("  	window.close();");
		    sb.append("} else {");
		    sb.append("	parent.parent.location.href='edocController.do?method=listIndex&controller=edocController.do&from=listReaded&listType=listReaded&edocType="+edocSummary.getEdocType()+"'");
		    sb.append("}");
		    ...
		    sb.append("  	window.close();");
		    sb.append("} else {");
		    sb.append("	parent.parent.location.href='edocController.do?method=listIndex&controller=edocController.do&from=listReading&listType=listReading&edocType="+edocSummary.getEdocType()+"'");
		    sb.append("}");
		}
...
//		}
			...
			if("agent".equals(request.getParameter("app")) && edocRegister!=null && !user.getId().equals(edocRegister.getRegisterUserId())) {//代理人跳转到代理事项
				...
				sb.append(" parent.parent.parent.window.location.href = 'govdoc/govdoc.do?method=leaderPishiList&_resourceCode=F20_leaderPishi';");
			}else if(Strings.isNotBlank(openFrom) && "ucpc".equals(openFrom) && !pishiFlag){
				...
				//保存待发后 跳转到待办列表
				//String isFromHome = request.getParameter("isFromHome");//该参数暂时不用在跳转页面
				if (("0").equals(edocType)) {//发文
    				sb.append(" parent.parent.parent.window.location.href = 'govdoc/govdoc.do?method=index&govdocType=1&listType=listSent&_resourceCode=F20_govDocSendManage';");
				}else if (("1").equals(edocType)) {//收文
					sb.append(" parent.parent.parent.window.location.href = 'govdoc/govdoc.do?method=index&govdocType=2,4&listType=listSent&_resourceCode=F20_receiveManage';");
				}else if (("2").equals(edocType)) {//签报
					sb.append(" parent.parent.parent.window.location.href = 'govdoc/govdoc.do?method=index&govdocType=3&listType=listSent&_resourceCode=F20_signReport';");
				}else if (track) {
					sb.append(" parent.parent.parent.window.location.href = 'govdoc/govdoc.do?method=index&govdocType=2,4&listType=listSent&_resourceCode=F20_receiveManage';");
				}
			}
...
//		System.out.println("公文发送总耗时："+(endtime-starttime)+" ms");
		return null;
    }
	...
	private void isUpdateRecRelation(EdocSummary edocSummary) throws BusinessException {
		...
		if (edocSummary.getEdocType() == 1 && edocSummary.getId() != null) {
			...
			if (Strings.isNotEmpty(relationList)) {
				List<CtpAffair> affList = affairManager.getValidAffairs(ApplicationCategoryEnum.edoc, ApplicationSubCategoryEnum.old_edocRec, edocSummary.getId());
	...
	public ModelAndView readBatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
    	...
    	if(registerId!=null) {//判断页面是否选择，Js有验证，加一层错误提示保险
    		for(int i=0; i<registerId.length; i++) {//批量处理需要办理的待发文单
    	        ...
    	        summary.setState(EdocConstant.flowState.run.ordinal());//将此流程设为运行状态
    	        ...
    	        summary.setProcessType(GovdocProcessTypeEnum.govdocRecRead.key());
    	        ...
    	        if(summary !=null && edocRegister!=null) {
    				//增加正文文件和v3x_file
        	        try {
        	        	if(registerBody != null && registerBody.getContent()!=null){
    					...
    					//增加附件
        	        	if(fromSummary!=null){
        	        		this.attachmentManager.copy(fromSummary.getId(), fromSummary.getId(), summary.getId(), summary.getId(), ApplicationCategoryEnum.edocRegister.ordinal());//附件
        	        	}
        	        	if(edocRegister!=null){
        	        		//登记上传的附件
        	        		this.attachmentManager.copy(edocRegister.getId(), edocRegister.getId(), summary.getId(), summary.getId(), ApplicationCategoryEnum.edocRegister.ordinal());
        	        	}
            	        ...
            	        docMark=edocMarkManager.registDocMark(summary, summary.getId(), docMark, 2,summary.getEdocType(),false,EdocEnum.MarkType.edocMark.ordinal());
            	        if(docMark!=null){summary.setDocMark2(docMark);}
            	        //内部文号
            	        String serialNo = summary.getSerialNo();
            	        if(serialNo==null){summary.setSerialNo(edocMarkManager.registDocMark(summary, summary.getId(), serialNo, 3,summary.getEdocType(),false,EdocEnum.MarkType.edocInMark.ordinal()));}
        	            ...
        	            try {
        	                ...
        	                affairId = edocManager.runCase(summary, edocBody, senderOninion,sendType, options,comm,agentToId,false,process_xml,"","");
        	            } catch (Exception e) {
        	                LOGGER.error("发起公文流程异常",e);
        	            }
        		...
        		wheres[2] = new Object[]{"objectId", edocRegister.getId()};
        		wheres[3] = new Object[]{"memberId", edocRegister.getDistributerId()};
...
//    		System.out.println(DateUtil.get19DateAndTime()+" msg="+msg);
    		StringBuilder sb = new StringBuilder();
    		sb.append("alert('"+msg+"');");
    		sb.append("location.href = 'edocController.do?method=listDistribute&edocType=1&list=aistributining&btnType=2';");
    		rendJavaScript(response, sb.toString());
    		return null;
    	}
    	PrintWriter out = response.getWriter();
    	String listType = "listSent";
    	if ("3".equals(EdocSwitchHelper.isOpenRegister())) {
			listType = "listPending";
		}
		out.println("<script>");
		out.println("window.parent.parent.location.href='edocController.do?method=listIndex&from=listSent&edocType=1&listType="+listType+"'");
    	out.println("</script>");
    	out.close();
		return null;
     }
	...
	public ModelAndView readBatchRegist(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		String[] registerId = registerIdsStr.split("&");
		String process_xml = request.getParameter("processXml");
		defaultEdocForm = edocFormManager.getDefaultEdocForm(
				user.getLoginAccount(), 1);
		...
		if (registerId != null) {// 判断页面是否选择，Js有验证，加一层错误提示保险
			for (int i = 0; i < registerId.length; i++) {// 批量处理需要办理的待发文单
				...
				summary.setProcessType(GovdocProcessTypeEnum.govdocRecRead.key());
				StringBuilder notPassMsg = new StringBuilder();
				Object[] objects = edocFormManager
						.getEdocFormElementRequiredMsg(defaultEdocForm, summary);
				notPassMsg.append(((String) objects[1]));
				if (!(Boolean) objects[0]) {
					continue;
				}
				...
				edocBody.setIdIfNew();
				if (edocBody.getCreateTime() == null) {
					edocBody.setCreateTime(new Timestamp(System
							.currentTimeMillis()));
				}
				edocBody.setContentType(registerBody == null ? "HTML"
						: registerBody.getContentType());
				edocBody.setLastUpdate(new Timestamp(System.currentTimeMillis()));
				if (summary != null && edocRegister != null) {
					// 增加正文文件和v3x_file
					try {
						if (registerBody != null && registerBody.getContent() != null) {
							if (!"HTML".equals(registerBody.getContentType())) {
								...
								V3XFile file = null;
								try {
									file = fileManager.clone(Long
											.parseLong(registerBody
													.getContent()), true);
									file.setFilename(file.getId().toString());
									fileManager.update(file);
									edocBody.setContent(file.getId().toString());
						...
						// 增加附件
						if(fromSummary!=null){
							this.attachmentManager.copy(fromSummary.getId(),
									fromSummary.getId(), summary.getId(),
									summary.getId(),
									ApplicationCategoryEnum.edocRegister.ordinal());// 附件
						}
						...
						senderOninion.setIsHidden(false);
						senderOninion.setCreateUserId(user.getId());
						senderOninion.setCreateTime(new Timestamp(System
								.currentTimeMillis()));
						senderOninion.setPolicy(request.getParameter("policy"));
						senderOninion
								.setOpinionType(OpinionType.senderOpinion
										.ordinal());
						senderOninion.setNodeId(0);
						String comm = "new_form";
						Long agentToId = null;
						...
						// 处理文号
						docMark = summary.getDocMark2();
						docMark = edocMarkManager.registDocMark(summary, summary.getId(), docMark, 2,
								summary.getEdocType(), false,
						...
						String serialNo = summary.getSerialNo();
						if (serialNo == null) {
							summary.setSerialNo(edocMarkManager.registDocMark(summary, summary.getId(),
									serialNo, 3, summary.getEdocType(), false,
						...
						try {
							...
							affairId = edocManager.runCase(summary, edocBody,
									senderOninion, sendType, options, comm,
				...
				edocRegister
						.setDistributeState(EdocNavigationEnum.EdocDistributeState.Distributed
								.ordinal());// 改成已分发
				edocRegister.setDistributeEdocId(summary.getId());
				...
				edocRegister.setState(RegisterState.Registed.ordinal());
				edocRegisterManager.update(edocRegister);// 修改登记表状态
				// 更新待登记记录
				recieveEdocManager.registerRecieveEdoc(recieveId,
						summary.getId());
		...
		PrintWriter out = response.getWriter();
		out.println("<script>");
		out.println("window.parent.parent.location.href='edocController.do?method=listIndex&from=listSent&edocType=1&listType=listSent'");
    	out.println("</script>");
    	out.close();
		return null;
	}
    ...
    private EdocRegister createEdocRegister(long recieveId) throws BusinessException {
        ...
        edocRegister.setRegisterDate(new java.sql.Date(new Date().getTime()));
        //来文信息
        Long edocId=record != null? record.getEdocId():null;
        edocRegister.setEdocId(edocId);
        edocRegister.setRecieveId(recieveId);
        if(record != null){
        	record.getSendUnit();
	...
	private void add2Index(int edocType, Long affairId) {
		...
		if (AppContext.hasPlugin("index")) {
			try {
			    indexApi.add(affairId, ApplicationCategoryEnum.edoc.getKey());
...
//            boolean portalIsTrack=false;
    		successUrl="location.href ='collaboration/collaboration.do?method=moreWaitSend&fragmentId="+fragmentId+"&ordinal="+ordinal+"&rowStr="+rowStr+"&columnsName="+columnsName+"';";
    		failUrl=successUrl;
    		if("19".equals(app)){
    		...
    		successUrl="parent.location.href ='edocController.do?method=listIndex&edocType="+edocType+"&listType=listSent';";
    		failUrl="location.href = 'edocController.do?method=edocFrame&edocType="+edocType+"&listType=listWaitSend';";
    	}
    	...
    	for (int i = 0; i < _summaryIds.length; i++)
    	{
            ...
            if("1".equals(edocType)) {
                ...
                if(edocRegister != null && !EdocHelper.isG6Version()) {
                	...
                	edocRegister.setIsRetreat(RegisterRetreatState.NotRetreat.ordinal());
            ...
            //如果有全文签批单则删除
            if(AppContext.hasPlugin("qwqp")) {
            	qwqpManager.deleteByEdocId(summaryId);
            }
    		...
    		boolean isEdocCreateRole=EdocRoleHelper.isEdocCreateRole(user.getLoginAccount(),user.getId(),edocTypeRole);
    		//没有发起权
    		if(!isEdocCreateRole){
    			String errMsg1=ResourceBundleUtil.getString("com.seeyon.v3x.edoc.resources.i18n.EdocResource","alert_not_edocCreatePower",edocSummary.getSubject());
    		...
    		if (edocSummary.getTempleteId() != null ) {
    			String errMsg2=ResourceBundleUtil.getString("com.seeyon.v3x.edoc.resources.i18n.EdocResource","alert_not_edocHaveTemplate",edocSummary.getSubject());
    		...
    		if (!isQuickSend && (processId == null || "".equals(processId.trim()))) {
    			String errMsg3=ResourceBundleUtil.getString("com.seeyon.v3x.edoc.resources.i18n.EdocResource","alert_not_edocHaveWorkFlow",edocSummary.getSubject());
            ...
            if(isQuickSend) {
            	...
            	if(edocSummary.getEdocType() == EdocEnum.edocType.sendEdoc.ordinal()){
    	            ...
    	            if(edocExchangeType!=-1){
    		        	...
    		        	try{
    						sendEdocManager.create(edocSummary, unitId, edocExchangeType, edocMangerID==null?null:String.valueOf(edocMangerID), a,false);
    		        		//更新公文统计表为封发
    		        		edocStatManager.setSeal(edocSummary.getId());
    		        	}catch(Exception e)
    		        	{
    	...
    	//ModelAndView mv = super.redirectModelAndView("/edocController.do?method=edocFrame&listType=listSent&edocType="+edocType);
    	if (message.length() > 0) {
/*    		WebUtil.saveAlert("alert.sendImmediate.nowf", message.toString());
    		mv = super.redirectModelAndView("/edocController.do?method=edocFrame&listType=listWaitSent&edocType="+edocType);*/
    ...
    public ModelAndView updateFormData(HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        try{
        	...
        	edocSummary.setCanTrack(isTrack);
 	        ...
 	        GovdocBaseVO baseVo = new GovdocBaseVO();
 	        baseVo.setSummary(edocSummary);
 	        //baseVo.setQuickSend(("fengfa".equals(affair.getNodePolicy())||signOpinion.exchangeType>=0));
 	        //baseVo.setLastNode(summary.getState() == EdocConstant.flowState.finish.ordinal() || baseVo.getIsQuickSend());
 	        baseVo.setCurrentDate(new Date());
 	        baseVo.setCurrentUser(AppContext.getCurrentUser());
 	       baseVo.setAction(request.getParameter("operationAction"));
 	        govdocMarkManager.saveDealMarkOld(baseVo);
 	       edocSummary = baseVo.getSummary();
          ...
          if(docMark!=null && !"".equals(docMark))
          {
        	 // edocMarkManager.disconnectionEdocSummary(edocSummary.getId(),1);
        	  try {
        		  docMark=edocMarkManager.registDocMark(edocSummary, edocSummary.getId(),docMark, 1, edocSummary.getEdocType(), true,EdocEnum.MarkType.edocMark.ordinal());
        	  }catch(EdocMarkHistoryExistException e) {
        		//(5.0sprint3)-FIXED(yangfan)
        		  response.setContentType("text/html;charset=UTF-8");
        		  String errMsg=ResourceBundleUtil.getString("com.seeyon.v3x.edoc.resources.i18n.EdocResource","edoc.docmark.exist");
          ...
          if(docMark!=null && !"".equals(docMark))
          {
        	 // edocMarkManager.disconnectionEdocSummary(edocSummary.getId(),2);
        	  docMark=edocMarkManager.registDocMark(edocSummary, edocSummary.getId(), docMark, 2,edocSummary.getEdocType(),true,EdocEnum.MarkType.edocMark.ordinal());
          ...
          if(serialNo!=null && !"".equals(serialNo))
          {
        	//  edocMarkManager.disconnectionEdocSummary(edocSummary.getId(),3);
        	  serialNo=edocMarkManager.registDocMark(edocSummary, edocSummary.getId(), serialNo, 3,edocSummary.getEdocType(),true,EdocEnum.MarkType.edocInMark.ordinal());
         ...
         // operationlogManager.insertOplog(summaryId, formId, ApplicationCategoryEnum.edoc, EactionType.LOG_EDOC_UPDATE_CONTENT, EactionType.LOG_EDOC_UPDATE_CONTENT_DESCRIPTION, user.getName(), edocSummary.getSubject());
          ...
          if(isChangeEdocForm(edocSummary,copyEdocSummary)){
              //OA-34156 应用检查：待办节点有效果文单的权限，处理后查看日志记录，一直会有：修改文单，很久之前报了，但是关了，需求要求可以挂起这个bug作为再放开修改的bug
              if(!"true".equals(isOnlyModifyWordNo)){
                  this.processLogManager.insertLog(user, Long.valueOf(edocSummary.getProcessId()), affair.getActivityId(),
          ...
          Map<String, Object> extParam = EdocUtil.createExtParam(edocSummary.getDocMark(), edocSummary.getSendUnit(), edocSummary.getSendUnitId());
	...
	public ModelAndView save(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if("1".equals(edocType)){
			...
			Map<String,Object> msgMap = checkSendRecEdoc(edocRegister,registerId,recieveId,request.getParameter("app"),user
		    		,backBoxToEdit);
			String errMessage = (String)msgMap.get("errMsg");
        ...
        summary.setBodyType(String.valueOf(GovdocUtil.getContentType(tempStr)));
        ...
        if(edocRegister != null && (!"HTML".equals(tempStr))) {
        	...
        	if(v3xfile == null) {
        		...
        		if(registerBody != null) {
        			try {
        				Long registerFileId = Long.parseLong(registerBody.getContent());
        				v3xfile = fileManager.getV3XFile(registerFileId);
        				if(v3xfile == null){
        					LOGGER.error("V3XFile : " + registerFileId + "不存在。");
        					throw new java.io.FileNotFoundException("该公文正文丢失 ，发送失败！ : ");
        				}
...
//        FlowData flowData = FlowData.flowdataFromRequest();
        ...
        if(isAgent){
        	Long summaryStartUId = agentToId != null?agentToId:edocRegister==null?null:edocRegister.getDistributerId();
        	if(summaryStartUId == null)
        		throw new EdocException("获取不到被代理人ID");
        ...
        baseVo.setCurrentDate(DateUtil.currentDate());
        baseVo.setCurrentUser(AppContext.getCurrentUser());
        baseVo.setNewBusiness(isNew ? "1" : "0");
        govdocMarkManager.saveDraftMarkOld(baseVo);
        summary = baseVo.getSummary();

        if (summary != null && !summary.isNew()) {
        	try {
        		if(summary.getGovdocType() == null){
        			summary.setGovdocType(0);
        		}
        		edocSummaryManager.saveOrUpdateEdocSummary(summary);
        	} catch(Exception e) {
    			LOGGER.error("保存公文号前保存公文出错", e);
        ...
        docMark=edocMarkManager.registDocMark(summary, summary.getId(), docMark, 1,summary.getEdocType(),false,EdocEnum.MarkType.edocMark.ordinal());
        if(docMark!=null){summary.setDocMark(docMark);}
        ...
        //内部文号
        String serialNo = summary.getSerialNo();
        serialNo=edocMarkManager.registDocMark(summary, summary.getId(), serialNo, 3,summary.getEdocType(),false,EdocEnum.MarkType.edocInMark.ordinal());
        if(serialNo!=null){summary.setSerialNo(serialNo);}
         */
        
        this.zcdbSupervise(request, response, summary, isNew,SuperviseEnum.superviseState.waitSupervise.ordinal(),false);
        ...
        if (edocRegister != null) {
        	List<CtpAffair> affairs = affairManager.getAffairs(ApplicationCategoryEnum.edoc,ApplicationSubCategoryEnum.old_edocRecDistribute,edocRegister.getId());
        	for (CtpAffair ctpAffair : affairs) {
        		ctpAffair.setState(StateEnum.col_done.getKey());
			}
       ...
       //跟踪
        ...
        //if(!"true".equals(backBoxToEdit)){//TODO
        	edocManager.setTrack(affairId, track, trackMembers);
        //}
		...
		if (Strings.isNotBlank(relationRec) && "haveYes".equals(relationRec)) {
			...
			edocSummaryRelation.setIdIfNew();
			edocSummaryRelation.setSummaryId(sendSummaryId);// 发文Id
			if(Strings.isNotBlank(relationRecIdStr) && !"null".equals(relationRecIdStr)) {
		...
		/*
        待登记 选择一个  到分发，保存草稿箱后，  在待发中发送后，待登记没消失        （解决办法，保存草稿箱时，就需要将签收记录中的 state设为2，表示已登记了）
*/
        String waitRegister_recieveId = request.getParameter("waitRegister_recieveId");
...
//			out.println("window.close();");
//		}else
		if ("3".equals(EdocSwitchHelper.isOpenRegister()) && "1".equals(edocType)) {
			PrintWriter out = response.getWriter();
			out.println("<script>");
			out.println("window.parent.parent.parent.location.href='govdoc/govdoc.do?method=index&govdocType=2,4&listType=listWaitSend';");
	    	out.println("</script>");
	    	out.close();
			return null;
		}
		//发送后 跳转到待办列表
		//String isFromHome = request.getParameter("isFromHome");//该参数暂时不用在跳转页面
		if(GovdocResCodeEnum.F20_govDocWaitSend.name().equals(request.getParameter("_resourceCode"))) {//列表权限
			sb.append("parent.parent.parent.window.location.href = 'govdoc/govdoc.do?method=index&listType=listWaitSendAllRoot';");
		} else {
			if (("0").equals(edocType)) {
				sb.append("parent.parent.parent.window.location.href = 'govdoc/govdoc.do?method=index&govdocType=1&listType=listWaitSend&_resourceCode=F20_govDocSendManage';");
			} else if (("2").equals(edocType)) {
				sb.append("parent.parent.parent.window.location.href = 'govdoc/govdoc.do?method=index&govdocType=3&listType=listWaitSend&_resourceCode=F20_signReport';");
			} else {
				sb.append("parent.parent.parent.window.location.href = 'govdoc/govdoc.do?method=index&govdocType=2,4&listType=listWaitSend&_resourceCode=F20_receiveManage';");
			}
		}
		rendJavaScript(response, sb.toString());
		return null;
	...
	public ModelAndView saveArchived(HttpServletRequest request,
		...
		long formId=Long.parseLong(request.getParameter("edoctable"));
		try{
		//String lastEdocSummaryContent=XMLCoder.encoder(edocSummary); //把上一版本数据保存为xml格式（留痕）
		
		DataUtil.requestToSummary(request,edocSummary,formId);
		
		//sGOV-4527 公文已办已归档修改公文，发起者附言可以修改但是保存不成功
		/*          String note = request.getParameter("note");
		if(Strings.isBlank(note))note = "";*/
		//GOV-4538 公文发文，已办，已办结公文的修改历史不显示发起者附言的修改
		//是否修改了附言
		/*          boolean isModifyNote = false;*/
		
		//GOV-4671 公文管理-发文管理-已办-已办结-已归档，修正一条记录点击修改，不管有没有修改内容点击保存，一直就在处理中了。只能重起浏览器   start
		/*          if(edocSummary.getSenderOpinion() == null){
		if(Strings.isNotBlank(note))
		 isModifyNote = true;
		}else{
		if(!note.equals(edocSummary.getSenderOpinion().getContent())){
		 isModifyNote = true;
		}
		}
		if(edocSummary.getSenderOpinion() == null){
		...
		this.edocManager.saveOpinion(senderOpinion, false);
		}else{
		edocSummary.getSenderOpinion().setContent(note);
		}*/
		//GOV-4671 公文管理-发文管理-已办-已办结-已归档，修正一条记录点击修改，不管有没有修改内容点击保存，一直就在处理中了。只能重起浏览器 end
		...
		String listType = Strings.isBlank(request.getParameter("listType"))?"listDoneAll":request.getParameter("listType");
		sb.append("location.href='edocController.do?method=listDone&edocType="+edocSummary.getEdocType()+"&listType="+listType+"';");
		rendJavaScript(response, sb.toString());
		}
	...
	public ModelAndView edocFrame(HttpServletRequest request,
		...
		String comm = request.getParameter("comm");
		String from = request.getParameter("from");
		String listType = Strings.isBlank(request.getParameter("listType"))?"":Strings.toHTML(request.getParameter("listType"));
		...
		String toNewRegister = request.getParameter("toNewRegister");
		if (Strings.isNotBlank(toNewRegister)) {
		modelAndView.addObject("toNewRegister", toNewRegister);
		}
		//2015年8月28日 修改isOpenRegister判断  简化公文交换环节，签收之后直接跳转新建页面  添加一个参数 xiex end
		String edocType=request.getParameter("edocType");
	...
	public ModelAndView upOptionState(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// 4退回时办理人选择意见覆盖方式,其他情况保留所有意见，被退回人选择保存所有意见
			...
			List<CtpAffair> affairList = affairManager.getAffairsByNodeId(affair.getActivityId());
			for(CtpAffair aff : affairList){
    	    	//BUG_紧急_V5_V5.1sp1_2015年8月修复包_文单定义-意见元素设置-被回退者选择意见覆盖方式，其他人意见保留，回退后，被回退者选择保留最后一次意见，提交，打开公文，之前的意见仍然保留_20150928012595_唐桂林_2015-10-12
    	    	if(!aff.isDelete()) {
    	    		affairIds.add(aff.getId());
    	    	}
    	    }
	...
	public ModelAndView listFinish(HttpServletRequest request, HttpServletResponse response) throws Exception {
...

		{
			...
			if (comb_condition != null && "1".equals(comb_condition)) {
				...
				queryList = edocManager.combQueryByCondition(iEdocType, em, StateEnum.col_done.key(), params,
						new int[] { EdocConstant.flowState.finish.ordinal(), ihasArchive });
				modelAndView.addObject("combQueryObj", em); // 设置的查询条件还原到页面
				modelAndView.addObject("combCondition", "1");
			} else
			/*************** 组合查询组件调用结束 ************/
			if (condition != null) {
				...
				queryList = edocManager.queryByCondition(iEdocType, condition, textfield, textfield1,
						StateEnum.col_done.key(), params, EdocConstant.flowState.finish.ordinal(), ihasArchive);
			}
			if (queryList != null) {
				modelAndView.addObject("pendingList", queryList);
			} else {
	...
	public ModelAndView listPending(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int roleEdocType = iEdocType == 1 ? 3 : iEdocType;
		boolean isEdocCreateRole = EdocRoleHelper.isEdocCreateRole(roleEdocType);
		boolean isExchangeRole = GovdocRoleHelper.isExchangeRole();
		modelAndView.addObject("isEdocCreateRole", isEdocCreateRole);
		modelAndView.addObject("isExchangeRole", isExchangeRole);
		// 发文的拟文权限
		boolean isSendEdocCreateRole = EdocRoleHelper.isEdocCreateRole(EdocEnum.edocType.sendEdoc.ordinal());
	...
	public ModelAndView listIndex(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        //GOV-4722.公文管理-代理人；A设置B为代理人，某公文中A的节点权限是封发（A有全部公文权限，B没有收文相关权限），那么B在代处理该公文时到 收文分发 这一步，在待办事项内点击该待办，页面会一直处在跳转中 start
        if(!isEdocCreateRole){
            if(!"agent".equals(request.getParameter("app")) && null !=request.getParameter("app")) {//没有公文发起权不能发送
                ...
                if("newEdoc".equals(model) && edocType != 1) {
                    sb.append("alert('"+errMsg+"');");
                    sb.append("location.href='edocController.do?method=listIndex&edocType="+edocType+"&list=listSent&from=listSent';");
                    rendJavaScript(response, sb.toString());
                    return null;
                } else if("listDistribute".equals(model) && edocType==1) {
                    sb.append("alert('"+errMsg+"');");
                    sb.append("location.href='edocController.do?method=listIndex&edocType="+edocType+"&list=listPending&from=listPending';");
                    rendJavaScript(response, sb.toString());
                    return null;
                }
            }
	...
	public ModelAndView edocRegisterDetail(HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			...
			Long recieveId = null;
			if (!Strings.isBlank(request.getParameter("recieveId"))) {
				recieveId = Long.parseLong(request.getParameter("recieveId"));
			}
			if (registerId == null) {
	...
	public ModelAndView edocRegister(HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			...
			long registerId = Strings.isBlank(request.getParameter("registerId")) ? -1L
					: Long.parseLong(request.getParameter("registerId"));
			long recieveId = Strings.isBlank(request.getParameter("recieveId")) ? -1L
			...
			if(edocRegister == null &&recieveId != -1) {
				EdocRecieveRecord bean = edocExchangeManager.getReceivedRecord(recieveId);
				if(bean!=null) {
					edocRegister = edocRegisterManager.findRegisterByRecieveId(bean.getId());
	        		if(edocRegister == null) {
	        			edocRegister = edocRegisterManager.findRegisterByDistributeEdocId(bean.getReciveEdocId());
	        		}
				}
			}
			if (edocRegister == null) {
			...
			if (registerBody != null) {
				...
				// GOV-4861.公文管理，收文登记节点，标准格式的正文，如果没有内容，查看正文时，显示NULL start
				modelAndView.addObject("content", Strings.isBlank(registerBody.getContent()) ? "" : contentBuffer.toString());
				// GOV-4861.公文管理，收文登记节点，标准格式的正文，如果没有内容，查看正文时，显示NULL end
				if (!"HTML".equals(registerBody.getContentType())) {
	...
	public ModelAndView batchRegister(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (recieveIds != null) {
			for (int i = 0; i < recieveIds.length; i++) {
				...
				if (summary != null && edocRecieveRecord != null) {
					...
					try {
						...
						this.attachmentManager.copy(summary.getId(), summary.getId(), edocRegister.getId(),
								edocRegister.getId(), ApplicationCategoryEnum.edocRegister.key());// 附件
						attachmentList = attachmentManager.getByReference(edocRegister.getId(), edocRegister.getId());
						/*
				...
				edocRecieveRecord.setStatus(EdocRecieveRecord.Exchange_iStatus_Registered);
				recieveEdocManager.update(edocRecieveRecord);
				if (edocRegister.getState() == RegisterState.Registed.ordinal()) {
					// 去掉待登记z
					Map<String, Object> params = new HashMap<String, Object>();
					params.put("app", ApplicationCategoryEnum.edoc.getKey());
					params.put("subApp", ApplicationSubCategoryEnum.old_edocRegister.getKey());
				...
				// 发送消息
				if (edocRegister.getState() == RegisterState.Registed.ordinal()) {
					String bodyType = MainbodyType.HTML.name();
					if(registerBody != null) {
						bodyType = String.valueOf(GovdocUtil.getContentType(registerBody.getContentType()));
					}
					sendMessageToRegister(user, -1, edocRegister, "create", -1, bodyType);
				}
			}
		}
...
//		}else
		...
		//String isFromHome = request.getParameter("isFromHome");//该参数暂时不用在跳转页面
		if(GovdocResCodeEnum.F20_govDocWaitSend.name().equals(request.getParameter("_resourceCode"))) {//列表权限
			sb.append(" parent.window.location.href = 'govdoc/govdoc.do?method=index&listType=listWaitSendAllRoot';");
		} else {
			if ("0".equals(edocType))  {
				sb.append("parent.window.location.href = 'govdoc/govdoc.do?method=index&listType=listWaitSend';");
			} else if ("2".equals(edocType)) {//签报
				sb.append("parent.window.location.href = 'govdoc/govdoc.do?method=index&listType=listWaitSend';");
			} else {
				sb.append(" parent.window.location.href = 'govdoc/govdoc.do?method=index&listType=listWaitSend';");
	...
	public ModelAndView listRegister(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		List<EdocRegister> list = new ArrayList<EdocRegister>();
		if (state == RegisterState.DraftBox.ordinal()
				|| state == RegisterState.Registed.ordinal()
				|| state == RegisterState.retreat.ordinal()) {
	...
	public ModelAndView distributeRetreat(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(edocRegister != null) {
			if(summary != null) {
				...
				//删除收文(暂逻辑删除)
				edocSummaryManager.updateEdocSummaryState(summary.getId(), EdocConstant.flowState.deleted.ordinal());
			}
			...
			/**
			 * EdocRecieveRecord record = recieveEdocManager.getEdocRecieveRecordByEdocId(edocId);
			...
			String affairStr = request.getParameter("affairId");
			Long affairId = -1L;
			Object[][] objects = new Object[3][2];
			if (affairStr != null && !"".equals(affairStr)) {
				affairId = Long.valueOf(affairStr);
			}
			/*objects[0] = {"app", ApplicationCategoryEnum.edocRecDistribute.getKey()};
			objects[1] = {"state", StateEnum.edoc_exchange_register.key()};*/
			objects[0] = new Object[]{"app", ApplicationCategoryEnum.edoc.getKey()};
			objects[1] = new Object[]{"subApp", ApplicationSubCategoryEnum.old_edocRecDistribute.getKey()};
			objects[2] = new Object[]{"state", StateEnum.edoc_exchange_register.key()};
			if (affairStr != null && !"".equals(affairStr)) {
				objects[3] = new Object[]{"id",affairId};
			}else{
				objects[3] = new Object[]{"objectId",registerId};
			}
			
			//删除分发待办
			Map<String, Object> columns = new HashMap<String, Object>();
			columns.put("delete", true);
			affairManager.update(columns, objects);

			//自动登记的数据 或者当 登记关闭时，分发直接退回到签收
			//2015年8月28日 修改isOpenRegister判断  简化公文交换环节，以前开关只有两个选项，是boolean类型，现在有三个选项，修改判断 xiex
			if((edocRegister.getAutoRegister() != null && edocRegister.getAutoRegister().intValue() == EdocRegister.AUTO_REGISTER) || !"1".equals(EdocSwitchHelper.isOpenRegister())) {
				EdocExchangeManager edocExchangeManager = (EdocExchangeManager)AppContext.getBean("edocExchangeManager");
				edocExchangeManager.stepBackRegisterEdoc(edocRegister, record.getId(), record.getId(), stepBackInfo, competitionAction);
			}
			//按照以前的回退流程
			else{
				//给登记人发送退回消息
				...
				if(!isRegisterOk){
					...
					userMessageManager.sendSystemMessage(new MessageContent("exchange.stepback", subject,distributeName,stepBackInfo),
						ApplicationCategoryEnum.edocRegister,
						sendId,
						receivers,
						EdocMessageFilterParamEnum.recQita.key);
				}else{
				...
				if(edocRegister.getRegisterType()==1) {//电子登记
					if(edocRegister.getEdocId()!=-1 && edocRegister.getRecieveId()!=-1) {
						...
						affairManager.update(columns, new Object[][]{
								{"app", ApplicationCategoryEnum.edoc.getKey()},
								{"subApp", ApplicationSubCategoryEnum.old_edocRegister.getKey()},
		...
		if ("3".equals(EdocSwitchHelper.isOpenRegister())) {
			PrintWriter out = response.getWriter();
			out.println("<script>");
			out.println("window.parent.parent.location.href='edocController.do?method=listIndex&from=listSent&edocType=1&listType=listPending'");
	    	out.println("</script>");
	    	out.close();
			return null;
		}else{
			if(registerId != -1) {//待分发列表
				...
				if(summary != null){//G6分发待发，A8待发列表
					...
					toView = super.redirectModelAndView("/edocController.do?method=listDistribute&edocType=1&list=aistributining&btnType=2");
				}
				return toView;
			} else {//退件箱列表
	...
	private void saveAffair(EdocRegister edocRegister, Long affairMemberId, Map<String, Object> extParam,
		...
		affair.setSubApp(ApplicationSubCategoryEnum.old_edocRegister.getKey());
		affair.setNodePolicy(OldExchangeNodePolicyEnum.olddengji.name());
		if(edocRegister != null && edocRegister.getRegisterBody() != null) {
			affair.setBodyType(String.valueOf(GovdocUtil.getContentType(edocRegister.getRegisterBody().getContentType())));	
		}
		if(edocRegister != null){
	...
	public ModelAndView listPendingRetreat(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		long id = register != null ? register.getId() : null ;
	...
	public ModelAndView detailIFrame(HttpServletRequest request, HttpServletResponse response) throws Exception {
		String openFrom = Strings.toHTML(request.getParameter("openFrom"));
		String from = Strings.toHTML(request.getParameter("from"));
		if(Strings.isBlank(openFrom) && Strings.isNotBlank(from)) {
			if("Done".equals(from)) {
				openFrom = "listDone";
			} else if("Sent".equals(from)) {
				openFrom = "listSent";
		...
		if(Strings.isNotEmpty(request.getParameter("affairId")) && !"undefined".equals(request.getParameter("affairId"))
				&& !"null".equals(request.getParameter("affairId"))) {
			Long affairId = Long.valueOf(request.getParameter("affairId"));
			CtpAffair affair = affairManager.get(affairId);
			...
			if (affair == null) {
				GovdocUtil.webAlertAndClose(response, ResourceUtil.getString("govdoc.summary.cannotacc.msg2"));
				return null;
			}
			if("listPending".equals(openFrom)&&affair.getState() == StateEnum.col_done.getKey()){
				openFrom = "listDone";
			}
			
			if (affair.getApp() == ApplicationCategoryEnum.edoc.getKey()) {
				if(affair.getSubApp().intValue() == ApplicationSubCategoryEnum.edoc_fawen.key()
						|| affair.getSubApp().intValue() == ApplicationSubCategoryEnum.edoc_shouwen.key()
						|| affair.getSubApp().intValue() == ApplicationSubCategoryEnum.edoc_qianbao.key()
						|| affair.getSubApp().intValue() == ApplicationSubCategoryEnum.edoc_jiaohuan.key()) {
					String govdocDetailURL = "/govdoc/govdoc.do?method=summary&openFrom=" + openFrom;
					if(Strings.isNotEmpty(request.getParameter("summaryId"))&& !request.getParameter("summaryId").equals("0")) {
						Long summaryId = Long.valueOf(request.getParameter("summaryId"));
						govdocDetailURL += "&summaryId=" + summaryId;
					}else{
						Long summaryId = affair.getObjectId();
						govdocDetailURL += "&summaryId=" + summaryId;
					}
					if(Strings.isNotEmpty(request.getParameter("trackTypeRecord"))) {
						int trackTypeRecord = Integer.valueOf(request.getParameter("trackTypeRecord"));
						govdocDetailURL += "&trackTypeRecord=" + trackTypeRecord;
					}
					return super.redirectModelAndView(govdocDetailURL + "&affairId="+ affairId);
				}
			}
		} else if(Strings.isNotEmpty(request.getParameter("baseObjectId")) || Strings.isNotEmpty(request.getParameter("summaryId"))) {
			Long summaryId = null;
			if(Strings.isNotEmpty(request.getParameter("baseObjectId"))) {
				summaryId = Long.valueOf(request.getParameter("baseObjectId"));
			} else if(Strings.isNotEmpty(request.getParameter("summaryId"))) {
				summaryId = Long.valueOf(request.getParameter("summaryId"));
			}
			EdocSummary summary = edocManager.getEdocSummaryById(summaryId, false);
			if(summary == null) {
				GovdocUtil.webAlertAndClose(response, ResourceUtil.getString("govdoc.summary.cannotacc.msg2"));
			...
			if (summary.getGovdocType() != null && summary.getGovdocType().intValue() != 0) {
				String govdocDetailURL = "/govdoc/govdoc.do?method=summary&openFrom=" + openFrom;
				return super.redirectModelAndView(govdocDetailURL + "&summaryId="+ summaryId);
			}
		}
	...
	public ModelAndView detail(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			String leaderPishiType = request.getParameter("leaderPishiType");
			String from = request.getParameter("from");
			String openFrom = request.getParameter("openFrom");
			
			//OA-35933在处理公文时，从关联文档中打开一个公文，应该只能查看，而这里居然可以处理公文--如果是关联文档打开，则是以已办的形式打开，不允许处理。
			boolean isglwgFlag = "glwd".equals(openFrom) || "glwd".equals(from) || "F8Reprot".equals(openFrom);
			...
			if(Strings.isNotBlank(sendSummaryId)) {//从关联文点击进来的
				...
				List<CtpAffair> affairs = affairManager.getValidAffairs(ApplicationCategoryEnum.edoc,Long.parseLong(sendSummaryId));
				for(int i=0;i<affairs.size();i++) {
					if(Strings.equals(user.getId(), affairs.get(i).getMemberId())) {
						showType = true;
					}
				}
			}// end Strings.isNotBlank(sendSummaryId)
			...
			if(Strings.isNotBlank(isCollCube) && "1".equals(isCollCube) && Strings.isNotBlank(_affairId)) {//协同立方过来查看的判断
				...
				if(null != validAffair && StateEnum.col_done.getKey() != validAffair.getState().intValue()){
					...
					rendJavaScript(response, "alert('"+errStr+"');try{parent.window.close();parent.window.parentDialogObj.url.closeParam.handler();}catch(e){}");
					return null;
				}
			}
			if(Strings.isNotBlank(isColl360) && "1".equals(isColl360) && Strings.isNotBlank(_affairId)){//协同360过来查看的判断 ps:协同360记录了两种关系一是发送者二是处理者
				CtpAffair validAffair = affairManager.get(Long.parseLong(_affairId));
				if(null != validAffair &&
						((StateEnum.col_sent.getKey() == validAffair.getState().intValue() && validAffair.isDelete()) ||
						(StateEnum.col_done.getKey() != validAffair.getState().intValue() && validAffair.isDelete())
						||(StateEnum.col_waitSend.getKey() ==  validAffair.getState().intValue() && null !=validAffair.getSubState() && SubStateEnum.col_waitSend_cancel.getKey() == validAffair.getSubState().intValue())
			...
			if(Strings.isNotBlank(_affairId)) {
				//来自代领导批示编号
//				if("2".equals(leaderPishiType)) {
//					List<CtpAffair> list = affairManager.getAffairs(Long.valueOf(summaryId), StateEnum.col_pending);
//					if(Strings.isNotEmpty(list)) {
//						affair=list.get(0);
//					}
//				} else {
			        Long _affairIdLong = Long.parseLong(_affairId); 
					affair = affairManager.get(_affairIdLong);
					if(affair == null) {
					    affair = affairManager.getByHis(_affairIdLong);
					}
//				}
				if("2".equals(leaderPishiType) && affair != null){
					leaderPishiFlag = govdocPishiManager.checkLeaderPishi(AppContext.currentUserId(), affair.getMemberId());
				}
				//来自关联文档，但流程状态为已撤销
				if( isglwgFlag && null != affair && null != affair.getSubState() && SubStateEnum.col_waitSend_cancel.getKey() == affair.getSubState()) {
					rendJavaScript(response, "alert('"+ResourceUtil.getString("edoc.alert.already.repeal")+"');window.top.close();");
					return null;
				}
				//来自老公文收文分发查看
				if(affair!=null && affair.getSubApp() == ApplicationSubCategoryEnum.old_edocRecDistribute.key()) {
				...
				}// end affair!=null && affair.getSubApp() == ApplicationSubCategoryEnum.old_edocRecDistribute.key()
				//这段代码真奇葩 打开待办取得是已办的affair
				...
				//这段代码真奇葩 打开待办取得是已办的affair
				if(!showType && !"transEvent".equals(isTransFrom) && !"pishi".equals(leaderPishiFlag)){
				...
				if(affair != null && affair.getState() == StateEnum.col_pending.key()) {  //
				 	...
				 	if(affair_sender!=null && affair_sender.getSubState()!=null &&affair_sender.getSubState() == SubStateEnum.col_waitSend_sendBack.key()) {
				 		String errMsg = "《"+affair_sender.getSubject()+"》"+ResourceBundleUtil.getString("com.seeyon.v3x.edoc.resources.i18n.EdocResource", "alert_not_openReturn");
				         rendJavaScript(response, "alert('"+errMsg+"');window.top.close();");
				     	return null;
				 	}
				     //OA-63298节点到期后转给指定人后，还能通过历史消息点击转之前的公文，弹出空白页面刷新提示null
				 	if(summaryId == -1){
				 		String errMsg =ResourceBundleUtil.getString("com.seeyon.v3x.edoc.resources.i18n.EdocResource", "alert_already_deleted",affair.getSubject());
				         rendJavaScript(response, "alert('"+errMsg+"');window.top.close();");
				     	return null;
				 	}
				}// end affair != null && affair.getState() == StateEnum.col_pending.key()
				
				//待办事项
				if(affair != null && Strings.isNotBlank(from)) {
					if("Pending".equals(from) || "Done".equals(from)) {//撤销消息点击查看时是Down
					   //获得发起人的affair
					   if(affair!=null && affair.getSubState()!=null && affair.getSubState() == SubStateEnum.col_waitSend_cancel.key()) {
						   String errMsg = "《"+StringEscapeUtils.escapeJavaScript(affair.getSubject())+"》"+ResourceBundleUtil.getString("com.seeyon.v3x.edoc.resources.i18n.EdocResource", "edoc.waitSend.stepBack");
					        rendJavaScript(response, "alert('"+errMsg+"');window.top.close();");
					        return null;
					    }
					   //撤销
					   if(affair!=null && affair.getState()!=null && affair.getState() == StateEnum.col_cancel.key()) {
						   	String errMsg = "《"+StringEscapeUtils.escapeJavaScript(affair.getSubject())+"》"+ResourceBundleUtil.getString("com.seeyon.v3x.edoc.resources.i18n.EdocResource", "edoc.waitSend.revocation");
					        rendJavaScript(response, "alert('"+errMsg+"');window.top.close();");
					        return null;
					   }
					   //回退
					   if(affair!=null && affair.getState()!=null && Integer.valueOf(StateEnum.col_stepBack.getKey()).equals(affair.getState())) {
							   String errMsg = "《"+StringEscapeUtils.escapeJavaScript(affair.getSubject())+"》"+ResourceBundleUtil.getString("com.seeyon.v3x.edoc.resources.i18n.EdocResource", "edoc.waitSend.stepBack");
					        rendJavaScript(response, "alert('"+errMsg+"');window.top.close();");
					        return null;
					    }
					}//"Pending".equals(from) || "Done".equals(from)
					
					//如果是從列表打開已被回退的信息，此處進行驗證
					//OA-43845 公文待发列表中单击/双击流程下游节点回退的公文，弹出的的界面中提示公文已经被回退
					//OA-50622  已经被指定回退给发起人的公文，查看以前的回退的消息还能设置督办，且督办设置生效，上一条消息的链接from的参数是sended
					...
					if(!"listWaitSend".equals(from)) {
						if(null != affair.getSubState()) {
						   ...
						   * 当前数据是否为回退数据
						   * affair!=null 非空验证
						   * affair.getSubState().intValue() == SubStateEnum.col_waitSend_stepBack.key() 当前数据是否为已回退数据
						   * affair.getState().intValue() != StateEnum.col_waitSend.key() 当前数据状态为待发状态
						   * 验证逻辑为：affair不等于空，当前数据substate是已回退状态并且state不等于待发，才进行提示
						   * OA-48472待发栏目更多页面，查看被人回退回来的发文流程，提示已被回退。应该可以查看的
						   */
							if(affair!=null && affair.getSubState().intValue() == SubStateEnum.col_waitSend_stepBack.key() && affair.getState().intValue() != StateEnum.col_waitSend.key()){
					    		String errMsg = "《"+affair.getSubject()+"》"+ResourceBundleUtil.getString("com.seeyon.v3x.edoc.resources.i18n.EdocResource", "edoc.affair.stepBack");
					            rendJavaScript(response, "alert('"+errMsg+"');window.top.close();");
					        	return null;
						   }
						}
					}// end !"listWaitSend".equals(from)
				}// end affair != null && Strings.isNotBlank(from)
			} else {
				//取我的Affair，用于访问控制权限校验
				List<CtpAffair> myAffairs = affairManager.getAffairs(ApplicationCategoryEnum.edoc, summaryId, user.getId());
				if(myAffairs!=null && !myAffairs.isEmpty()) {
					affair = myAffairs.get(0);
				} else {
					affair = affairManager.getSenderAffair(summaryId);
					if(affair == null){//有可能是流程追溯的
						List<CtpAffair> affairsList = affairManager.getAffairs(summaryId);
						if(affairsList != null && !affairsList.isEmpty()){
							affair = affairsList.get(0);
							openFrom = "stepBackRecord";
						}
					}
				}
			}//end Strings.isNotBlank(_affairId)
			...
			if (affair == null || (affair.getState().intValue() != StateEnum.col_pending.key()
				...
				if (("true".equals(isG6) && affair != null && user.getId() == affair.getSenderId().longValue())
			...
			if(summaryId == null||0L == summaryId) {
				summaryId=affair.getObjectId();
			}
			...
			if("1".equals(edocType) && "listSent".equals(detailType)) {
				...
				if(edocRegister != null) {
					List<EdocSummary> newEdocList1 = this.edocSummaryRelationManager.findNewEdocByRegisteredOrWaitSent(edocRegister.getId(), user.getId(), 1,2);
					if(newEdocList1 != null) {
						allEdocList.addAll(newEdocList1);
					}
				}
				List<EdocSummary> newEdocList2 = this.edocSummaryRelationManager.findNewEdocByRegisteredOrWaitSent(relationSummaryId, user.getId(), 1,2);
			...
			/**
			* 收文转发文，收文关联发文，进行催办按钮屏蔽控制
			* from不为空时，from值不变化，不影响其它链接的设置
			...
			*/
			...
			if((!showType && !"transEvent".equals(isTransFrom)) && !isglwgFlag && !"pishi".equals(leaderPishiFlag)) {
				if(!SecurityCheck.isLicit(request, response, ApplicationCategoryEnum.edoc, user, summaryId, affair, null)){
					if((Strings.isNotBlank(isCollCube) && "1".equals(isCollCube)) || (Strings.isNotBlank(isColl360) && "1".equals(isColl360))) {
				...
				AccessControlBean.getInstance().addAccessControl(ApplicationCategoryEnum.edoc, String.valueOf(summaryId), user.getId());
			}
			
			//G6BUG_G6_v1.0_中国证券监督管理委员会_自定义IE标题，打开公文单后，IE标题显示为默认的名称，打开其他的事项显示的都是自定义的标题_20120731011964
			String subject = affair.getSubject() ;
			modelAndView.addObject("affairId", affair.getId());
			...
			modelAndView.addObject("openFrom", request.getParameter("openFrom"));
			modelAndView.addObject("controller", "edocController.do");
			modelAndView.addObject("leaderPishiType", leaderPishiType);
			...
			rendJavaScript(response, "alert('" + errMsg + "');try{parent.window.close();parent.window.parentDialogObj.url.closeParam.handler();}catch(e){}");
			return null;
		}
	}

		public ModelAndView edocDetailHistory(HttpServletRequest request, HttpServletResponse response) throws Exception {
			...
			// lijl添加是否从文档中心点进来的false是,true否
			String docEntrance = request.getParameter("docEntrance");
			mav.addObject("docEntrance", docEntrance);
			...
			mav.addObject("historyData", request.getParameter("historyData"));
			mav.addObject("controller", "edocController.do");
			if (affair != null) {
				mav.addObject("affairId", affair.getId());
			}
			// 客开 项目名称：贵州省政府 ，作者：dengyd，修改日期：2015-5-18
			// 上午10:49:52，功能[授权的归档员归档文件后对文档的查看]，start
			else {
				...
				if (summary != null) {
					...
					params.put("app", app);
					params.put("objectId", Long.parseLong(summaryId));
					List<CtpAffair> currentAffairs = affairManager.getByConditions(null, params);
					if (currentAffairs != null && currentAffairs.size() > 0) {
						for (CtpAffair affair2 : currentAffairs) {
							if (affair2.getSubObjectId() == null && affair2.getActivityId() == null) {
								mav.addObject("affairId", affair2.getId());
								mav.addObject("zhihuiStr", "true");
								break;
							}
						}
					}
			...
			// 客开 项目名称：贵州省政府 ，作者：dengyd，修改日期：2015-5-18
			// 上午10:49:52，功能[授权的归档员归档文件后对文档的查看]，end
			return mav;
		}
	...
	public ModelAndView edocDetailInDoc(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String summaryId = Strings.toHTML(request.getParameter("summaryId"));
		String openFrom = Strings.toHTML(request.getParameter("openFrom"));
		String lenPotent = Strings.toHTML(request.getParameter("lenPotent"));
		CtpAffair affair = null;
		if (Strings.isNotBlank(summaryId)) {
		    Long summaryIdLong = Long.valueOf(summaryId);
			//如果是新公文，传入的summaryId，可能是affairId，这里判断下，走不通的打开链接
            CtpAffair govdocAffair = affairManager.get(summaryIdLong);
			if(govdocAffair != null &&  
				(govdocAffair.getSubApp().intValue()== 1 || govdocAffair.getSubApp().intValue() == 2
				|| govdocAffair.getSubApp().intValue() ==3 || govdocAffair.getSubApp().intValue()==4 )){
				return super.redirectModelAndView("/govdoc/govdoc.do?method=summary&affairId=" + summaryId + "&openFrom="
								+ openFrom + "&lenPotent=" + lenPotent+"&isGovArchive=1");
			}
			
			affair = affairManager.getSenderAffair(summaryIdLong);
			if(affair == null) {
			    affair = affairManager.getSenderAffairByHis(summaryIdLong);
			}
			if (affair == null) {
				// 获得从已发删除的affair
				DetachedCriteria criteria = DetachedCriteria.forClass(CtpAffair.class)
						.add(Restrictions.eq("objectId", summaryIdLong)).add(Restrictions.eq("delete", true))
				...
				if (Strings.isNotEmpty(list)) {
					...
					affair = affairManager.get(summaryIdLong);
					if(affair == null) {
					    affair =  affairManager.getByHis(summaryIdLong);
					}
					if (null != affair) {
						if (affair.getObjectId() != null) {
		...
		mav.addObject("lenPotent", lenPotent);
		mav.addObject("docId", request.getParameter("docId"));
		mav.addObject("summaryId", summaryId);
		mav.addObject("isLibOwner", request.getParameter("isLibOwner"));
		mav.addObject("controller", "edocController.do");
		...
		if (affairId != null) {
			...
			if (tracks != null) {
				for (CtpTrackMember ctpTrackMember : tracks) {
					if(ctpTrackMember.getTrackMemberId() != null){
						if (ids.length() > 0) {
							ids.append(",");
						}
	...
	public ModelAndView summary(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
        	...
        	String archiveModifyId=request.getParameter("archiveModifyId");  //归档修改日志id
        	if(Strings.isNotBlank(archiveModifyId)){
        		//EdocArchiveModifyLog archiveModifyLog=edocArchiveModifyLogManager.getById(Long.parseLong(archiveModifyId));
        	...
        	String historyData = request.getParameter("historyData");
			if ("historyData".equals(historyData)) {// 贵州省政府数据迁移 查看绕过安全验证
													// 2015年7月25日 xiachh
				ModelAndView modelAndView = new ModelAndView("edoc/edocSummary1");
				...
				if (attachments != null) {
					...
					String desc = "";
					Date date = new Date(107, 0, 2);
					Date date2 = new Date(107, 0, 1);
					for (Attachment attachment : attachments) {
						if (attachment.getCreatedate().before(date)) {
							String folder = fileManager.getFolder(date2, false);
							File file = new File(folder + File.separatorChar + attachment.getFileUrl());
							if (!file.exists()) {
								if (!"".equals(desc)) {
									desc += ",";
								}
								desc += "《" + attachment.getFilename() + "》";
							}
							attachments2.add(attachment);
						}
					}
					modelAndView.addObject("attachments", attachments2);
					if (!"".equals(desc)) {
						desc += ResourceUtil.getString("govdoc.data.migrated");
					}
        	...
        	//获取pdf  aip ID    陈枭
			if(AppContext.hasPlugin("qwqp")) {
	        	mav.addObject("PDFOrAipId","");
	        	List<QwqpEdocSendFormRelation> edocSendFormRelationList = qwqpManager.getByEdocId(summaryId);
	        	if(null!=edocSendFormRelationList&&edocSendFormRelationList.size()>0){
	        		//公文如果存在pdfid或者aipid
					mav.addObject("PDFOrAipId",edocSendFormRelationList.get(0).getFileId());
					mav.addObject("fileType", edocSendFormRelationList.get(0).getFileType());
	        	}
			}
        	//
        	...
        	boolean _flag = ("Pending".equals(_from)
        					|| "Done".equals(_from)
        					|| "listSent".equals(_from)
        					|| "supervise".equals(_from)
        					|| "supervise".equals(_openFrom)
        					|| "sended".equals(_openFrom)
        					|| "sended".equals(_from))
        					&& !"glwd".equals(_openFrom)
        					&& !"recRegisterResult".equals(_openFrom)//收文登记薄
    		...
    		//验证当前流程是否是指定回退流程
            boolean specialStepBack = (summary.getCaseId()==null||Long.valueOf(0).equals(summary.getCaseId()))?false:wapi.isInSpecialStepBackStatus(summary.getCaseId());
          ...
          /*  CtpEnum remindMetadata = enumManager.getEnum(EnumNameEnum.common_remind_time.name());
            CtpEnum  deadlineMetadata= enumManager.getEnum(EnumNameEnum.collaboration_deadline.name());
            String remindLabel=remindMetadata.getItemLabel(summary.getAdvanceRemind().toString());
            String deallineLabel= "";
            if(summary.getDeadline()!= null ){
            	deallineLabel = deadlineMetadata.getItemLabel(summary.getDeadline().toString());
            }
            ...
            //mav.addObject("allowUpdateAttachment",true);
            //只查找正文的附件。
            ...
            String trimFileName = "";
	            for(Attachment att : attachments){
	            	...
	            	//客户bug--国家质量监督检验检疫总局国际检验检疫标准与技术法规研究中心
	            	if(att.getFileUrl()!=null){
		            	CtpAffair aff = affairManager.get(att.getFileUrl());
		            	if(aff != null && aff.getApp() > 4){ //区分老公文还是新公文 然后设置类型  让关联文档正常打开
        	...
        	try {
        		if(Strings.isNotBlank(_affairId)) {
        			...
        			if("2".equals(leaderPishiType)){
        				List<CtpAffair> list = affairManager.getAffairs(affairId, StateEnum.col_pending);
    					if(Strings.isNotEmpty(list)) {
    						affair=list.get(0);
    					}
        			}else {
        		...
        		} else if(Strings.isNotBlank(s_summaryId) && "glwd".equals(request.getParameter("openFrom"))) {
        			affair = affairManager.getSenderAffair(summaryId);
        			if(affair == null){
        		...
        		mav.addObject("senderId",affair.getSenderId());
                if(affair != null && !affair.getMemberId().equals(currentNodeOwner)){
                    currentNodeOwner = affair.getMemberId();
                }
                mav.addObject("fromId",affair.getFromId());
                /**
                 * OA-50342督办人员查看处于指定回退的督办公文流程时，无修改流程按钮
                 *  此处验证督办详细是否可以修改流程，需要加上指定回退的验证
                 */
                if(affair != null &&  affair.getState() == SubStateEnum.col_waitSend_draft.getKey() && affair.getSubState() != SubStateEnum.col_pending_specialBacked.key()){
                	flageState = false;
                }
                ...
                if(ColOpenFrom.stepBackRecord.name().equals(request.getParameter("openFrom"))){
    	    		if(null != summary && null!=affair && null !=summary.getCaseId() && null!= affair.getActivityId()){
    	        		...
    	        		if(nodeDelete) {
    	                     ...
    	                     if(obj != null) {
    	                         nodeDelete = wapi.isNodeDelete(summary.getCaseId(), obj.toString());
    	                     }
    	                 }
    	        		...
    	        		if(nodeDelete || !effectiveAffair){
    	        			rendJavaScript(response,"alert('"+ResourceUtil.getString("collaboration.alert.notExistInWF")+"');");
    	        			return null;
    	        		}
    	        	}
    	    		//获取关联发文信息
...
//        	EdocSummary summary = edocManager.getEdocSummaryById(summaryId, true);
            ...
            if(affair != null){
                 ...
                 * 加签 减签   知会  js 调用修改
                                                        需要加   默认节点权限   和   单位id参数
                                                        单位id 对于自由流程取  发起人所在单位id
                                                                 对于模板流程取  模板所在单位id
                                                        所以在调用方法之前，要先判断 当前处理的  是自由流程发起的还是由  模板流程发起
                 */
                ...
                //传阅还需要 departmentId
                //mav.addObject("departmentId",summary.getOrgDepartmentId());
            ...
            wapi.getBPMCaseByProcessId(summary.getProcessId());
            mav.addObject("scene", 3);//查看已发出去的流程图
            if(Strings.isBlank(summary.getProcessId()) || "0".equals(summary.getProcessId()) || "-1".equals(summary.getProcessId())){
        	...
        	if(!showCaseId){
        		if((null == summary.getCaseId() || -1l == summary.getCaseId() || 0l == summary.getCaseId()) && (Strings.isNotBlank(summary.getProcessId()) && !"0".equals(summary.getProcessId()))){
        			summary.setCaseId(wapi.getBPMCaseByProcessId(summary.getProcessId()).getId());
        		}
        	...
        	if(summary.getCaseId() == null){
        		StringBuilder sb=new StringBuilder();
        		sb.append(ResourceUtil.getString("govdoc.edoc.caseId.null"));
        		if(summary!=null)	sb.append( summary.getId());
        		if(affair!=null)	sb.append(", " + affair.getId());
        		sb.append(", " + user.getName());
        		LOGGER.warn(sb.toString());
        		/*try{//TODO 老公文兼容
	        		CaseManager caseManager = (CaseManager)AppContext.getBean("caseManager");
	        		List<BPMCase> caselist = (List<BPMCase>)caseManager.getCaseByProcessId(summary.getProcessId());
	        		if(caselist != null && caselist.size() > 0){
	        			summary.setCaseId(caselist.get(0).getId());
	        			edocManager.update(summary);
	        		}
        		}catch(Exception ee){
        			LOGGER.error("更新公文summary对象的caseid出错", ee);
        		}*/
        	}
            ...
            // show1 重新流转 show2 提交回退者
             ...
             if("olddengji".equals(configItem)){
            	 configItem = "dengji";
            ...
            PermissionVO permissionVO = permissionManager.getPermissionVO(category,
                    configItem, flowPermAccout);
            String _updateNodeFlag = "";//OA-66328 update by libing
            if(configItem.startsWith("fenban_zhihui")){
            	_updateNodeFlag ="2";
            	//如果是分办知会则修改为已读 zhangdong start
        		CtpAffair zAffair = affairManager.get(Long.valueOf(_affairId));
        		zAffair.setState(StateEnum.col_done.key());
        		zAffair.setSubState(SubStateEnum.col_normal.key());
            	Timestamp now = new Timestamp(System.currentTimeMillis());
            	zAffair.setCompleteTime(now);
            	zAffair.setUpdateDate(now);
                affairManager.updateAffair(zAffair);	
            	//如果是分办知会则修改为已读 zhangdong end   
            }else{
	            //用于判断当前节点权限是否存在，如果不存在则给知会的提示，inform是协同的节点权限，处理时做知会操作产生的错误数据
	            if (!"inform".equals(configItem) && permissionVO != null && !configItem.equals(permissionVO.getName()) && affair.getState() == StateEnum.col_pending.getKey()) {
	            	mav.addObject("noFindPermission",true);
	            	if(EnumNameEnum.edoc_send_permission_policy.name().equals(category)|| EnumNameEnum.edoc_qianbao_permission_policy.name().equals(category) ){
      	...
      	if(!docResourceExist) {
            ...
            String from = request.getParameter("from");
            if(("lenPotent".equals(openFrom)||"docLib".equals(openFrom)) && "2".equals(lenPotent))
            {
            	onlySeeContent="true";
            }
        	...
        	if(!"WaiSend".equals(request.getParameter("from"))){ //待发不用判断权限
        		...
        		if(affair != null && affair.getActivityId() !=null)
        		    nodePolicyFromWorkflow= wapi.getNodePolicyIdAndName(ApplicationCategoryEnum.edoc.name(), summary.getProcessId(), String.valueOf(affair.getActivityId()));
        		...
        		if(affair != null)
        		{
        			if("Pending".equals(request.getParameter("from")))
        			{
        				...
        				if(Integer.valueOf(StateEnum.col_pending.getKey()).equals(affair.getState())
        					&&(Integer.valueOf(SubStateEnum.col_pending_ZCDB.getKey()).equals(affair.getSubState())
        							||Integer.valueOf(SubStateEnum.col_pending_takeBack.getKey()).equals(affair.getSubState()))){
    	            ...
    	            if(summary.getState()!=EdocConstant.flowState.finish.ordinal() && affair.getSubObjectId() == null && affair.getState()==StateEnum.col_sent.getKey() && user.getId().longValue() == affair.getSenderId().longValue() && !("lenPotent").equals(openFrom) && !("haveNot").equals(from)){
...
/* OA-36989一个流程中的多人执行节点，节点权限审核绑定了文单，但是第一个人处理时意见框显示在文单中，第二个人处理时，意见框显示在下面
 * 这个缓存有问题，导致意见的位置出现偶发性变化，需要检查这个缓存
 *        			Hashtable<String, String> locHs = SharedWithThreadLocal.getLocHs();
        			if(locHs == null){
        				locHs=edocFormManager.getOpinionLocation(summary.getFormId(),EdocHelper.getFlowPermAccountId(summary, user.getLoginAccount(), templeteManager));
        			}
        			...
        			*/
        			...
        			//OA-45168 上传一个文单，审批意见框默认绑定了审批节点权限，当取消审批节点后，处理节点处理时，意见框仍显示在文单中，但是处理后意见显示在文单下面
        			//当disPosition为null时，表示当前节点权限 没有绑定意见框，那么意见就应该显示在文单最下面(其他意见)，如果再设置为当前节点节点就有问题了
        		...
        		// 这里需要用老的枚举 兼容升级7.1之前的数据
        		EnumNameEnum edocTypeEnumOld = EdocUtil.getEdocMetadataNameEnumOld(summary.getEdocType());
        		EnumNameEnum edocTypeEnumnew = EdocUtil.getEdocMetadataNameEnum(summary.getEdocType());
...
//        			fpm = flowPermManager.getFlowPerm(edocTypeEnum.name(), nodePermissionPolicy, flowPermAccountId);
        			permission = permissionManager.getPermission(edocTypeEnumOld.name(), nodePermissionPolicy, flowPermAccountId);
        			...
        			if(permission!=null){
        				...
        				mav.addObject("nodePermissionPolicyName",ResourceUtil.getString("govdoc.edoc.read"));
        			}
        			int docMarkAccess = 1;
...
//        			if(null!=fpm){
        			if(null!=permission){
        				...
        				Hashtable <Long,EdocElementFlowPermAcl> actorsAcc = edocElementFlowPermAclManager.getEdocElementFlowPermAclsHs(permission.getFlowPermId());
        				EdocElement docMarkElement = edocElementManager.getEdocElement("004");
	        			if(docMarkElement != null) {
        					...
        					logger.error("节点权限无公文文号元素编辑数据：" + permission.getFlowPermId());
        				}

            			EdocElement docMark2Element = edocElementManager.getEdocElement("021");
            			if(docMark2Element != null) {
        					...
        					logger.error("节点权限无公文文号2元素编辑数据：" + permission.getFlowPermId());
        				}
            			...
            			if(serialNoElement != null) {
        					...
        					logger.error("节点权限无内部文号元素编辑数据：" + permission.getFlowPermId());
        				}
        				...
        				if(permission.getType()==0){ //com.seeyon.v3x.flowperm.util.Constants.F_type_system
        					tempMitem = enumManagerNew.getEnumItem(edocTypeEnumnew, nodePermissionPolicy);
        				}
        			}
                ...
                List<String> baseActions  = permissionManager.getBasicActionList(edocTypeEnumOld.name(), nodePermissionPolicy, flowPermAccountId);
                if(Strings.isEmpty(baseActions)){
        			if(CollectionUtils.isEmpty(baseActions) && "chuanyue".equals(nodePermissionPolicy)){
            			mav.addObject("nodePermissionPolicyName",ResourceUtil.getString("govdoc.edoc.read"));
            		}
        			LOGGER.error("edocController.summary : baseActions is null! [edocTypeEnum.name():"+edocTypeEnumOld.name()+" nodePermissionPolicy:"+nodePermissionPolicy +" flowPermAccountId:"+flowPermAccountId+" summary.id:"+summary.getId());
        		}
        		//常用操作
                List<String> commonActions  = permissionManager.getCommonActionList(edocTypeEnumOld.name(), nodePermissionPolicy, flowPermAccountId);
                //GOV-1342 屏蔽转发 start
                String forward ="";
                for(String str :commonActions){
                	if("Forward".equals(str)){
                		forward = str;
                	}
                }
              //兼容G6 1.0，回退/终止/撤销放到baseActions中
                if(Strings.isNotEmpty(baseActions)){
                	if(baseActions.contains("Return") && !commonActions.contains("Return")) {
                		commonActions.add("Return");
                	}
                	if(baseActions.contains("Terminate") && !commonActions.contains("Terminate")) {
                		commonActions.add("Terminate");
                	}
                	if(baseActions.contains("Cancel") && !commonActions.contains("Cancel")) {
                		commonActions.add("Cancel");
                	}
                }
                if(Strings.isNotBlank(forward)){
                    commonActions.remove(forward);
                }
                //高级操作
                List<String> advancedActions  = permissionManager.getAdvaceActionList(edocTypeEnumOld.name(), nodePermissionPolicy, flowPermAccountId);
                for(String str :advancedActions){
                	if("Forward".equals(str)){
                		forward = str;
                	}
                }
    			...
    			if (("fengfa".equals(nodePermissionPolicy))
    				...
    				mav.addObject("defaultExchangeType", EdocSwitchHelper.getDefaultExchangeType(summary.getOrgAccountId()));
    				...
    				String exchangeDeptTypeSwitchValue = EdocSwitchHelper.getDefaultExchangeDeptType(summary.getOrgAccountId());
    			...
    			if((commonActions != null && commonActions.contains("Return")) || (advancedActions !=null && advancedActions.contains("Return"))){
    				isdealStepBackShow = true ;
    			}
    			if((commonActions != null && commonActions.contains("Terminate")) || (advancedActions !=null && advancedActions.contains("Terminate"))){
    				isdealStepStopShow= true;
    			}
    			if((commonActions != null && commonActions.contains("Cancel")) || (advancedActions !=null && advancedActions.contains("Cancel"))){
                ...
                //G6.1 新公文没有转发策略进行屏蔽--start
				if( AppContext.hasPlugin("govdoc") && commonActions.contains("Forward")){
					commonActions.remove("Forward");
				}
				if( AppContext.hasPlugin("govdoc") && advancedActions.contains("Forward")){
					advancedActions.remove("Forward");
				}
				//G6.1 新公文没有转发策略进行屏蔽--end
    	        ...
    	        EdocExchangeTurnRec turnRec = edocExchangeTurnRecManager.findEdocExchangeTurnRecByEdocId(summary.getId());
    	        ...
    	        if(turnRec != null){
	    	        ...
	    	        if(summary.getEdocType() == EdocEnum.edocType.recEdoc.ordinal()){
	    	        	...
	    	        	for(CtpAffair af : affairs){
	    	        		//首页需要是收文流程的 affair
	    	        		if(af.getSubApp().intValue() == ApplicationSubCategoryEnum.old_edocRec.key() &&
			...
			CtpEnumBean attitude = enumManagerNew.getEnum(EnumNameEnum.collaboration_attitude.name()); // 处理意见 
			colMetadata.put(EnumNameEnum.collaboration_attitude.toString(), attitude);// attitude
			CtpEnumBean deadline = enumManagerNew.getEnum(EnumNameEnum.collaboration_deadline.name()); // 处理期限
			colMetadata.put(EnumNameEnum.collaboration_deadline.toString(), deadline);// attitude
        	...
        	mav.addObject("NPSAppName", GovdocUtil.getNPSAppName(0, summary.getEdocType()));
        	...
        	//OA-36876 weblogic环境：q1在兼职单位gwgw下新建公文，节点人员q1封发节点，gwgw下有收发员，q1待办处理时，能看到单位收发员有人，但是提交的时候却提示请设置单位收发员
    		...
    		StringBuilder supervisorId = new StringBuilder(); // supervisorId : all the ids of supervise detail
    		StringBuilder tempIds = new StringBuilder(); // tempIds : all the ids of superviseTemplate
    		CtpSuperviseDetail detail = this.superviseManager.getSupervise(SuperviseEnum.EntityType.edoc.ordinal(),summaryId);
    		if(detail != null) {
    			...
    			if(null!=summary && null!=summary.getTempleteId()){
    				CtpSuperviseDetail tempDetail = superviseManager.getSupervise(SuperviseEnum.EntityType.template.ordinal(),summary.getTempleteId());
    				if(null!=tempDetail){
    					...
    					List<CtpSuperviseTemplateRole> roleList = superviseManager.findRoleByTemplateId(summary.getTempleteId());
    					...
    					for (CtpSuperviseTemplateRole role : roleList) {
    						...
    						if (role.getRole().toLowerCase().equals(
    						        EdocOrgConstants.ORGENT_META_KEY_SEDNER.toLowerCase() + EdocOrgConstants.ORGENT_META_KEY_DEPMANAGER.toLowerCase())) {
    							orgRole = orgManager.getRoleByName(EdocOrgConstants.ORGENT_META_KEY_DEPMANAGER, summary.getOrgAccountId());
    							if (null != orgRole) {
    						...
    						if (role.getRole().toLowerCase().equals(EdocOrgConstants.ORGENT_META_KEY_SEDNER.toLowerCase() + EdocOrgConstants.ORGENT_META_KEY_SUPERDEPMANAGER.toLowerCase())) {
    							orgRole = orgManager.getRoleByName(EdocOrgConstants.ORGENT_META_KEY_SUPERDEPMANAGER, summary.getOrgAccountId());
    							if (null != orgRole) {
            ...
            if("true".equals(request.getParameter("isLibOwner"))){  //文档库相关
            	...
            	printEdocTable="false";
                    ...
                    "listSent".equals(from)/*从首页和普通列表打开*/|| "Sent".equals(from)/*首页更多打开*/) { //已发送和保存待发
            	printEdocTable="true";
            }else if(isSupervisor && !"Pending".equals(from)){//督办节点都能打印
            	printEdocTable = "true";
            }else if(Strings.isNotBlank(_openFrom) && ("recRegisterResult".equals(_openFrom)//收文登记薄
					|| "sendRegisterResult".equals(_openFrom))){   //发文登记薄
            	printEdocTable = "true";
            }else {
        	...
        	/**
        	 * 由于协同督办传递的参数值为openFrom，公文统一全部是from,为了影响改动范围，先调整公文的参数接受方式
        	 * wangwei
        	 */
        	...
        	if("supervise".equals(superviseFrom) && summary.getState()!=EdocConstant.flowState.finish.ordinal()) {
    			mav.addObject("showHastenButton", "true");
    		}
            ...
            if(null != dataByParams && dataByParams.size() > 0 && ("listPending".equals(superviseFrom)||("Pending").equals(superviseFrom))){
     ...
     * @throws Exception
     */
    public ModelAndView getAIPContent(HttpServletRequest request, HttpServletResponse response) throws Exception {
    	ModelAndView modelAndView = new ModelAndView("edoc/edocTopicAIP");
    	String summaryIdstr = request.getParameter("summaryId");
        long summaryId = Long.parseLong(summaryIdstr);
        modelAndView.addObject(summaryId);
        
        if(AppContext.hasPlugin("qwqp")) {
	        List<QwqpEdocSendFormRelation> edocSendFormRelations = qwqpManager.getByEdocId(summaryId);
	        if(null!=edocSendFormRelations&&edocSendFormRelations.size()>0){
	        	modelAndView.addObject("fileType",edocSendFormRelations.get(0).getFileType());
	        }
        }
    	return modelAndView;
    }
    ...
    public ModelAndView getContent(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        boolean showType = false;
        if("lenPotent".equals(openFrom) || "glwd".equals(openFrom) || "edocDocLib".equals(openFrom)|| "F8Reprot".equals(openFrom))
        {
        	showType = true;
        	if("2".equals(lenPotent)){
        	   onlySeeContent=true;
...
//        EdocSummary summary = (EdocSummary)request.getSession().getAttribute(EdocConstant.SUMMARY_TO_SESSEION);
...
//        }
    	...
    	if(Strings.isNotBlank(archiveModifyId)){
    		...
    		if(summary.getTempleteId() != null){
    		    ...
    		    if(null!=template){
    		    	...
    		    	docMarkByTemplateJs = EdocHelper.exeTemplateMarkJs(templateSummary.getDocMark(), templateSummary.getDocMark2(), templateSummary.getSerialNo());
    		    }
    		}
    	}
    	modelAndView.addObject("docMarkByTemplateJs", docMarkByTemplateJs);
       ...
       /*	   affair = (CtpAffair)request.getSession().getAttribute(Constant.AFFAIR_TO_SESSEION);
        	if(affair == null){
        		affair=affairManager.get(affairId);
        	}else{
        		request.getSession().removeAttribute(EdocConstant.AFFAIR_TO_SESSEION);
        	}*/
        	...
        	String catType= EdocUtil.getEdocMetadataNameEnumOld(summary != null ? summary.getEdocType() : null).name();
        	//flash bug add adjust field
        	if(summary != null && "collaboration".equalsIgnoreCase(nodePermissionPolicy))
        	{
        	...
        	try{
        		Long flowPermAccountId = EdocHelper.getFlowPermAccountId(summary, summary.getOrgAccountId(), templeteManager);
...
//        		}
        ...
        //收文关联发文不需要进行验证、文档中心的暂时不验证TODO
        if("true".equals(request.getParameter("openEdocByForward")) || "docLib".equals(openFrom)){
        	showType=true;
        }
		if(!showType){
			 boolean ifFromstepBackRecord =  ColOpenFrom.stepBackRecord.name().equals(openFrom);
		     boolean isFromrepealRecord = ColOpenFrom.repealRecord.name().equals(openFrom);
		...
		if(summary != null){
			if("fengfa".equals(nodePermissionPolicy) && summary.getPackTime()==null){
				summary.setPackTime(new Timestamp(System.currentTimeMillis()));
			}
        ...
        if(summary != null && summary.getFormId() != null) {
	        ...
	        if(firstBody != null) {
	        	...
	        	logger.error("公文查看时firstBody为空，请检查！！！" + summary.getIdentifier());
	        }
		...
		List<EdocFormFlowPermBound> policyList = edocFormManager.findBoundByFormId(summary.getFormId(), flowPermAccout, nodePermissionPolicy);
		String policy = "";
        ...
        List<V3xHtmDocumentSignature> signatuers = v3xHtmDocumentSignatManager.findBySummaryIdAndType(summary.getId(), V3xHtmSignatureEnum.HTML_SIGNATURE_DOCUMENT.getKey());
        Map<String, Object> strMap = EdocOpinionDisplayUtil.convertOpinionToString(map,displayConfig,affair,isFromPending,signatuers);
		...
		if("haveMany".equals(relRecs)){
			 EdocSummaryRelation edocSummaryRelationR = this.edocSummaryRelationManager.findRecEdoc(summaryId, 0);
			 if(edocSummaryRelationR != null){
			   //OA-33377 收文转发文，发文发送后。把收文撤销流程。然后在发文-关联收文中打开收文，查看属性状态，应该是待发，现在是已发。
                    ...
                    if(recAffairId != null){
                        ...
                        if(reAffair == null){
                        	reAffair = affairManager.getByHis(recAffairId);
                        }
	...
	public ModelAndView repeal(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
				...
				if (_summaryIds.length > 0) {
					...
					for (int i = 0; i < _summaryIds.length; i++) {
						...
						// else if(summary.getHasArchive()){result=2;}
							...
							conditions.put("subApp", EdocUtil.getSubAppCategoryByEdocType(summary.getEdocType()).key());
							...
							if ((doneList != null && doneList.size() > 0) && "docBack".equals(docBack)) {// 处理中不能取回
								...
								if (1 == summary.getEdocType()) {
									...
									distributerConditions.put("app", ApplicationCategoryEnum.edoc.key());
									distributerConditions.put("subApp", ApplicationSubCategoryEnum.old_edocRecDistribute.key());
						...
						if (result == 1 || result == -1
							...
							// 撤销流程事件
							EdocCancelEvent event = new EdocCancelEvent(this);
			...
			if (isRelieveLock) {
				...
				for (int i = 0; i < _summaryIds.length; i++) {
	    			wapi.releaseWorkFlowProcessLock(_summaryIds[i], String.valueOf(AppContext.currentUserId()));
	    	    }
				try {
					unLock(user.getId(), summary);
				} catch (Exception e) {
	...
	public ModelAndView stepBack(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (editHelper.hasEditAtt()) {// 是否修改附件
				...
				EdocHelper.updateAttIdentifier(summary, attUtil.parseProcessLog(Long.parseLong(processId), nodeId),
						true, "stepBack");
			}
	...
	public ModelAndView delete(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (affairIds != null) {
			for (String affairId : affairIds) {
				...
				summaryids.put(affair.getObjectId(), affair.getObjectId());
				int affairSubApp = affair.getSubApp();
				AppLogAction alog = null;
				if (affairSubApp == ApplicationSubCategoryEnum.old_edocSend.ordinal()) {
					alog = AppLogAction.Edoc_Delete_Send;
				} else if (affairSubApp == ApplicationSubCategoryEnum.old_edocRec.ordinal()) {
					alog = AppLogAction.Edoc_Delete_Rec;
				} else if (affairSubApp == ApplicationSubCategoryEnum.old_edocSign.ordinal()) {
		...
		//删除督办消息
        if(CollectionUtils.isNotEmpty(summaryids.keySet())){
        	for(Long key : summaryids.keySet()){
        		this.edocSuperviseManager.deleteSuperviseDetailAndSupervisors(this.edocManager.getEdocSummaryById(key, false));
        	}
        }
		return new ModelAndView("edoc/refreshWindow").addObject("windowObj", "parent");
	}
	...
	public ModelAndView pigeonhole(HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        if (affairIds != null) {
            for (String affairId : affairIds) {
                ...
                try
                {
                	//col.affairApp == 20 && col.affairState == 2
                	//add by chencm 如果是分发，则不用进行权限判断
                	boolean notAttributed = true;
                	if(affair.getApp() == 20 && affair.getState() == 2){
                		notAttributed = false;
                	}
                	//end add by chencm
                	nodePermissionPolicy  = affair.getNodePolicy();
                	//if("finish".equals(pageType))----edit by chencm
                	if("finish".equals(pageType) && notAttributed)
                	{//已办事项，归档判断处理时，是否有归档权限
                		List<String> baseActions = permissionManager.getActionList(edocTypeEnum.name(), nodePermissionPolicy, summary.getOrgAccountId(), PermissionAction.basic);
                	...
                	if(summary.getArchiveId()!=null && docResourceExist){
                		...
                		edocManager.pigeonholeAffair(pageType, _affairId, affair.getObjectId(),archiveId,pigeonholeType);
	...
	@CheckRoleAccess(roleTypes = { Role_NAME.AccountAdministrator, Role_NAME.EdocManagement })
	public ModelAndView sysCompanyMainIframe(HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        modelAndView.addObject("app",4);

        modelAndView.addObject("isAdministrator",AppContext.getCurrentUser().isAdministrator());
        return modelAndView;
    }
	...
	public ModelAndView doZCDB(HttpServletRequest request,
		...
		//设置summary附件标识,修改affair附件标识，设置附件元素，修改附件日志
		    AttachmentEditUtil attUtil = new AttachmentEditUtil("attActionLogDomain");
		    EdocHelper.updateAttIdentifier(summary, attUtil.parseProcessLog(Long.parseLong(processId), nodeId), true, "zcdb");
		}
		...
		*/
		...
		if(isRelieveLock) {
		wapi.releaseWorkFlowProcessLock(processId, String.valueOf(AppContext.currentUserId()));
		wapi.releaseWorkFlowProcessLock(_summaryId, String.valueOf(AppContext.currentUserId()));
		}
		try{
		unLock(user.getId(),summary);
		}catch(Exception e){
	...
	public ModelAndView openEdocSetting(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(isGov && !"1".equals(EdocSwitchHelper.isOpenRegister())){//未开起登记功能
	...
	public ModelAndView edocRegisterInfo(HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			...
			RegisterBody registerBody = edocRegister != null ? edocRegister.getRegisterBody() : null;
			if(edocRegister != null){
				modelAndView.addObject("edocType", edocRegister.getEdocType());
			}
			modelAndView.addObject("controller", "edocController.do");
	...
	public ModelAndView preInsertPeople(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String NPSAppName = request.getParameter("NPSAppName"); // 多级会签用，判断当前应用是发文、收文、签报，是字符串，从而取到该应用的节点权限。
		...
		mav.addObject("NPSAppName", NPSAppName);
	...
	public String relationReceive(String relationEdocIdStr, String edocTypeStr) {
		...
		if (relationEdocId != null && relationEdocType != -1) {
			...
			if(edocSummary == null) { // 转储后数据在分库
				edocSummary = this.edocSummaryRelationManager.findEdocSummaryHis(relationEdocId, relationEdocType);
			}
			...
			if (edocSummary != null) {// 收文单
				...
				EdocRegister edocRegister = this.edocSummaryRelationManager.findEdocRegister(relationEdocId,
						relationEdocType);
				if(edocRegister == null) { // 转储后数据在分库
					edocRegister = this.edocSummaryRelationManager.findEdocRegisterHis(relationEdocId, relationEdocType);
				}
				...
				if (edocRegister != null) {// 登记单
					...
					if(edocRecieveRecord == null) {
						edocRecieveRecord = this.edocSummaryRelationManager.findEdocRecieveRecordHis(relationEdocId);
					}
	...
	public ModelAndView appointStepBack(HttpServletRequest request, HttpServletResponse response)
		...
		context.setAppName(ApplicationCategoryEnum.edoc.name());
		context.getBusinessData().put("subAppName", GovdocWorkflowTypeEnum.oldedoc.name());
		...
		signOpinion.setIsHidden(request.getParameterValues("isHidden") != null);
		signOpinion.setOpinionType(OpinionType.backOpinion.ordinal());
		...
		if ("start".equals(theStepBackNodeId)) {
			if ("0".equals(submitStyle)) {
				try {// 删除归档
					...
					baseVo.setCurrentUser(user);
					baseVo.setCurrentDate(new Date());
					govdocMarkManager.saveCancelMarkOld(baseVo);
				} catch (Exception e) {
	...
	public Long getMinValueByDeadlineTime(String deadlineTime, Date createTime) {
		...
		try {
			if (Strings.isNotBlank(deadlineTime) && _createTime != null) {
				...
				if (minusDay > 0) {
					minValue = (long) Math.rint(minusDay / (1000 * 60d));
				}
			}
		} catch (Exception e) {
	...
	public ModelAndView getAttributeSettingInfo(HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		// 定义跳转目标路径
		ModelAndView mav = new ModelAndView("edoc/showAttributeSetting");
		String affairId = request.getParameter("affairId");
...
//		return super.refreshWorkspace();
	...
	private void saveAffairAndSummary4Register(EdocRegister record,String method,long affairId) throws BusinessException{
		//复制部分信息到summary中
    	EdocSummary edocSummary = null;
    	edocSummary = edocSummaryManager.findById(record.getId());
    	if(null == edocSummary)
    		edocSummary = new EdocSummary();
		...
		edocSummary.setSerialNo(record.getSerialNo());
		...
		if(record.getRegisterUserId() == null || "".equals(record.getRegisterUserId())){
			edocSummary.setCreatePerson(record.getDistributer());
			edocSummary.setStartUserId(record.getDistributerId());
		}
		...
		if(record != null && record.getRegisterBody()!=null) {
			edocSummary.setBodyType(String.valueOf(GovdocUtil.getContentType(record.getRegisterBody().getContentType())));		
		}
		edocSummaryManager.saveOrUpdateEdocSummary(edocSummary);
		//复制部分信息到affair中   待登记的 不需要在affair中添加数据
		/*if(1 != state){//TODO 老公文代码兼容
			/*List<CtpAffair> affairs = affairManager.getAffairs(ApplicationCategoryEnum.dengji, record.getId());//取出来应该只存在一个
			CtpAffair affair = null;
			boolean isnew = false;
			if(null == affairs || affairs.size() == 0){
				affair = new CtpAffair();
				affair.setId(UUIDLong.longUUID());
				affair.setSubState(0);
				affair.setReceiveTime(new Date());
				isnew = true;
			}else{
				for(CtpAffair aff : affairs){//获取当前待办的affair
					if(aff.getState() == StateEnum.col_pending.getKey()){
						affair = aff;
						break;
					}
				}
			}
			if(StringUtils.isNotBlank(record.getSecretLevel())){
				affair.setSecretLevel(Integer.valueOf(record.getSecretLevel()));
			}
			affair.setSubject(record.getSubject());
			affair.setTrack(0);
			affair.setObjectId(record.getId());
			affair.setApp(ApplicationCategoryEnum.dengji.getKey());
			...
			if("0".equals(method)){//保存待发 affair的state=3  已完成 =4
				affair.setState(3);
				User user = AppContext.getCurrentUser();
				...
				affair.setState(4);
				affair.setCompleteTime(new Date());
			}
			if(isnew)
				affairManager.save(affair);
			else
		...
		//保存待发时 当登记单中标题等信息修改时，同步到当前个人事项中
		CtpAffair affair=affairManager.get(affairId);
		if("0".equals(method) && affair!=null){//保存待发
