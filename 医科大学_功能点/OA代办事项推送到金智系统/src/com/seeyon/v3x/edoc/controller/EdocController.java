...
/*                modelAndView = new ModelAndView("common/redirect");
    ...
    public ModelAndView newEdocRegister(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        boolean canUpdateContent = (edocRegister.getRegisterType() != null && (edocRegister.getRegisterType() == 2 || edocRegister.getRegisterType() == 3)) ? true : EdocSwitchHelper.canUpdateAtOutRegist();
        ;//是否能修改正文
            ...
            for (int i = 0; i < registerId.length; i++) {//批量处理需要办理的待发文单
                ...
                if (summary != null && edocRegister != null) {
                    ...
                    try {
                        ...
                        if (serialNo == null) {
                            summary.setSerialNo(edocMarkManager.registDocMark(summary, summary.getId(), serialNo, 3, summary.getEdocType(), false, EdocEnum.MarkType.edocInMark.ordinal()));
                        }
        ...
        if (affairIds != null) {
            for (String affairId : affairIds) {
                ...
                try {
                    ...
                    if ("finish".equals(pageType) && notAttributed) {//已办事项，归档判断处理时，是否有归档权限
