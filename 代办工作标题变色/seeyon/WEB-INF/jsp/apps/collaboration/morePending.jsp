...
<head>
    ...
    var advancedQueryObj;
		...
		var pendingCountCacheKey = "${pendingCountCacheKey}";
           ...
           if(hasAIPlugin == "true"){
            ...
            function rend(txt, data, r, c,col) {
              //未读  11  加粗显示
				...
				//周刘成 接收时间
				var receiveTime= data.receiveTimeAll;
				var receiveMills=new Date(receiveTime).getTime();
				var nowMills=new Date().getTime();
				var cha=nowMills-receiveMills;
				var hour = Math.round((cha/1000)/3600);
			  ...
			  if(col.name ==='deadLine' && data.dealTimeout=== true){
                ...
                if(col.name === 'subject'){
                	...
                	if (data.topTime != null) {
                		txt = "<span class='isTop'>[置顶]</span>" + txt;
                	}


					if(hour >24 && hour < 72){
						txt = "<a class='color_black' style='color:yellowgreen;'>"+txt+"</a>";
					}else if(hour>72){
						txt = "<a class='color_black' style='color:red;'>"+txt+"</a>";
					}else {
      ...
      //取消加粗
        ...
        function openPendingQueryViews(){
            ...
            queryDialog = $.dialog({
                url:  _ctxPath + "/collaboration/pending.do?method=pendingCombinedQuery&pagefrom="+"${param.from}",
                width: _width,
                height: 320,
...
<body class="bg_color_f0f0f0">
    <div id='layout' class="font_size12 comp" comp="type:'layout'">
        ...
        <input type="hidden" id="fragmentId" value="${params.fragmentId}"/>
	       ...
	       <input type="hidden" id="templatePanel" value="${params.templatePanel}"/>
	       <input type="hidden" id="realSenderPanel" value="${params.senderPanel}"/>
	       <input type="hidden" id="senderPanel" value="${params.senderPanel}"/>
	       <input type="hidden" id="TemplateTypePanel" value="${params.TemplateTypePanel}"/>
    </div>
