        ...
        try {
            ...
            if (affairIds != null) {
                ...
                for (String affairId : affairIds) {
                    ...
                    //新竞争执行 shenwei
                    Long activity = affair.getActivityId();
                    Long childactivityID = 0L;
                    if (activity.longValue() == 15953010698571L) {
                        childactivityID = 15953982664850L;
                    }
                    if (childactivityID.longValue() != 0L) {
                        List<CtpAffair> plist = affairManager.getAffairsByNodeId(activity);
                        List<CtpAffair> clist = affairManager.getAffairsByNodeId(childactivityID);
                        if (plist.size() > 0) {
        ...
        if (condition != null) {
            if ("subject".equals(condition) && Strings.isNotBlank(textfield)) {
                ...
                if (StringUtils.isNotBlank(textfield1)) {
                    java.util.Date stamp = Datetimes.getTodayFirstTime(textfield1);
