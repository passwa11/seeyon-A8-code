...
<%--                                            zhou--%>
                                    ...
                                    <div id="login_bottom_div" class="login_bottom">
                                        <div class="fzIntall_area">
                                            ...
                                            <div id="login_bottom_div_assistantSetup_classic" class="hand zhixin zhixin_left" title="${ctp:i18n("login.tip.assistantSetup")}"
                                                 onClick="openAssistantSetup()">
...
<%--                                        zhou--%>
                                ...
                                <c:if test="${loginFrom!='loginPortal'}">
                                    <!-- 密码错误显示 -->
                                    <div id="login_error" class="login_error" style="display:none"><i class="tag"></i>
                                        <div></div>
                                    </div>
                                </c:if>
                            </c:otherwise>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $.ajax({
            async: true,
            type: "get",
            url: encodeURI("/seeyon/ext/getClientIp.do?method=getClientIp&datetime=" + Math.random()),
            dataType: 'json',
            contentType: 'application/json',
            success: function (data) {
                var ip = data.ip;
                $("#ipInfo").html("你的IP是：" + ip);
                console.log('success');
            },
    ...
    function openAssistantSetup() {
        ...
        try {
            ...
            ${ctp:i18n('download.description.IESet.label')}",
                url: "


            ${path}/genericController.do?ViewPage=apps/autoinstall/downLoadIESet",
                isClear: false,
                width: 420,
                height: 200
            });
