...
<script type="text/javascript">
	...
	var tempDetailId = "";
	function openCuiban(activityId,processId,appName){
		...
		getA8Top().$.dialog({
			...
			title: '催办3',
			url: '/seeyon/workflow/hasten.do?method=preHasten&activityld='+activityId+'&processId='+processId+'&appName='+appName,
			//url: '/seeyon/workflow/hasten.do?method=openHastenDialog&affairId='+affairId,
			width: 455,
			height: 465,
			top: 50,
			closed: true,
	...
	/*
	    getA8Top().v3x.openDialog({
			title:"催办",
			transParams:{'parentWin':window},
			url: '/seeyon/workflow/hasten.do?method=preHasten&activityld='+activityId+'&processId='+processId+'&appName=edoc',
			width:"400",
			height:"300"
	    });
  ...
  <div class="right_div_row2">
			...
			<tr>
						...
						<div class="div-float">
							<select name="condition" id= "condition" onChange="showNextSpecialCondition(this)" class="condition" style="width:90px">
							    ...
							    <option value="docInMark"><fmt:message key="edoc.element.wordinno.label" /></option>
							   	<c:if test="${edocType==1 && isG6 != 'true'}">
							    	<option value="sendUnit"><fmt:message key="edoc.edoctitle.fromUnit.label" /></option>
							    </c:if>
					  	...
					  	<div id="subjectDiv" class="div-float hidden"><input type="text" id="textfield" name="textfield" class="textfield" maxlength="100"></div>
					  	<div id="startMemberNameDiv" class="div-float hidden"><input type="text" name="textfield" class="textfield" maxlength="100"></div>
					  	<div id="docMarkDiv" class="div-float hidden"><input type="text" name="textfield" class="textfield" maxlength="100"></div>
					  	<div id="docInMarkDiv" class="div-float hidden"><input type="text" name="textfield" class="textfield" maxlength="100"></div>
					  	...
					  	<c:if test="${edocType==1}">
	                        ...
	                        <c:if test="${isG6=='true'}">
						  	...
						  	<c:if test="${isG6 != 'true'}">
						  	   <div id="sendUnitDiv" class="div-float hidden"><input type="text" name="textfield" class="textfield" maxlength="100"></div>
						  	</c:if>
						  	<div id="urgentLevelDiv" class="div-float hidden">
                                <select name="textfield" class="condition" style="width:90px">
                                    <option value=""><fmt:message key="common.pleaseSelect.label" /></option>
				...
				<form name="combForm" id="combForm" method="post" onsubmit="return false" style="margin: 0px">
				  ...
				  <div style="font-size: 15px;	">
						<a id="combinedQuery" onclick="combQueryEvent(${edocType},4)" class="combinedQuery common_button" ><fmt:message key='edoc.advanced.lable'/></a>
                   </div>
				</form>
				</td>		
			</tr>   
    ...
    <div class="center_div_row2" id="scrollListDiv">
		...
		<v3x:table htmlId="pending" data="pendingList" var="col" className="sort ellipsis">
			...
			<%--收文增加"来文单位"  --%>
			<c:if test="${edocType==1 && isG6 !='true'}">
				<v3x:column width="10%" type="String"  label="edoc.edoctitle.fromUnit.label" className="cursor-hand sort" read="${isRead}"
				onClick="${click}">
					<span title="${v3x:toHTML(col.summary.sendUnit)}">${v3x:toHTML(col.summary.sendUnit)}</span>&nbsp;
				</v3x:column>
			</c:if>
			<%--发起人--%>
			...
			<v3x:column width="12%" type="Date" label="edoc.supervise.startdate"  className="cursor-hand sort" read="${isRead}"
			 onClick="${click}">
					<span title="${v3x:formatDateByPattern(col.summary.createTime,datePattern)}">${v3x:formatDateByPattern(col.summary.createTime,datePattern)}</span>&nbsp;
			</v3x:column>	
			<%--处理时间--%>
			<v3x:column width="14%" type="Date" label="edoc.supervise.managedate"  className="cursor-hand sort" read="${isRead}"
			 onClick="${click}">
				<span title="${v3x:formatDateByPattern(col.dealTime,datePattern)}">${v3x:formatDateByPattern(col.dealTime,datePattern)}</span>&nbsp;
			</v3x:column>
			<%--当前代办人--%>
			...
			<!-- 催办 -->
			<v3x:column width="9%" type="String" align="center" label="催办" >
					 <span><a href="javascript:openCuiban('${col.affair.activityId}','${col.summary.processId}','<%=ApplicationCategoryEnum.edoc.getKey()%>')" >催办</a></span>
			</v3x:column>  
